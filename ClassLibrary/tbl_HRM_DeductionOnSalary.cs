...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_DeductionOnSalary
    {
        ...
        public Nullable<int> IDDeduction { get; set; }
        public Nullable<System.DateTime> AriseDay { get; set; }
        ...
        public Nullable<bool> IsAutomatedDeductionOnSalary { get; set; }
        //List 0:1
...
{
	using System;
	public partial class DTO_HRM_DeductionOnSalary
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_DeductionOnSalary 
    {
		...
		public static IQueryable<DTO_HRM_DeductionOnSalary> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_DeductionOnSalary";
		...
		public static DTO_HRM_DeductionOnSalary getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_DeductionOnSalary.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_DeductionOnSalary getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_DeductionOnSalary
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_DeductionOnSalary.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_DeductionOnSalary",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_DeductionOnSalary dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_DeductionOnSalary);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_DeductionOnSalary item, tbl_HRM_DeductionOnSalary dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDDeduction = item.IDDeduction;							
			dbitem.AriseDay = item.AriseDay;							
			dbitem.IDPayPeriod = item.IDPayPeriod;							
			dbitem.DeductionRate = item.DeductionRate;							
			dbitem.IsExecuted = item.IsExecuted;							
			dbitem.IsAutomatedDeductionOnSalary = item.IsAutomatedDeductionOnSalary;        }

		public static DTO_HRM_DeductionOnSalary post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_DeductionOnSalary dbitem = new tbl_HRM_DeductionOnSalary();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_DeductionOnSalary.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_DeductionOnSalary",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_DeductionOnSalary();
                    ...
                    {
                        dbitem = new tbl_HRM_DeductionOnSalary();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_DeductionOnSalary.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_DeductionOnSalary.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_DeductionOnSalary.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_DeductionOnSalary", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_DeductionOnSalary",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_DeductionOnSalary",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_DeductionOnSalary.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_DeductionOnSalary.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_DeductionOnSalary> toDTO(IQueryable<tbl_HRM_DeductionOnSalary> query)
        {
			return query
			.Select(s => new DTO_HRM_DeductionOnSalary(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				IDStaff = s.IDStaff,							
				IDDeduction = s.IDDeduction,							
				AriseDay = s.AriseDay,							
				IDPayPeriod = s.IDPayPeriod,							
				DeductionRate = s.DeductionRate,							
				IsExecuted = s.IsExecuted,							
				IsAutomatedDeductionOnSalary = s.IsAutomatedDeductionOnSalary,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_HRM_DeductionOnSalary toDTO(tbl_HRM_DeductionOnSalary dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_DeductionOnSalary()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IDDeduction = dbResult.IDDeduction,							
					AriseDay = dbResult.AriseDay,							
					IDPayPeriod = dbResult.IDPayPeriod,							
					DeductionRate = dbResult.DeductionRate,							
					IsExecuted = dbResult.IsExecuted,							
					IsAutomatedDeductionOnSalary = dbResult.IsAutomatedDeductionOnSalary,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_HRM_DeductionOnSalary> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_DeductionOnSalary> query = db.tbl_HRM_DeductionOnSalary.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query IDDeduction (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDDeduction"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDDeduction").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDDeduction));
////                    query = query.Where(d => IDs.Contains(d.IDDeduction));
//                    
            }

			//Query AriseDay (Nullable<System.DateTime>)
			if (QueryStrings.Any(d => d.Key == "AriseDayFrom") && QueryStrings.Any(d => d.Key == "AriseDayTo"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "AriseDayFrom").Value, out DateTime fromDate) && DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "AriseDayTo").Value, out DateTime toDate))
                    query = query.Where(d => fromDate <= d.AriseDay && d.AriseDay <= toDate);

            if (QueryStrings.Any(d => d.Key == "AriseDay"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "AriseDay").Value, out DateTime val))
                    query = query.Where(d => d.AriseDay != null && DbFunctions.TruncateTime(val) == DbFunctions.TruncateTime(d.AriseDay));
...
//                    
			...
			//Query DeductionRate (Nullable<decimal>)
			if (QueryStrings.Any(d => d.Key == "DeductionRateFrom") && QueryStrings.Any(d => d.Key == "DeductionRateTo"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "DeductionRateFrom").Value, out decimal fromVal) && decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "DeductionRateTo").Value, out decimal toVal))
                    query = query.Where(d => fromVal <= d.DeductionRate && d.DeductionRate <= toVal);

			//Query IsExecuted (Nullable<bool>)

			//Query IsAutomatedDeductionOnSalary (Nullable<bool>)
            return query;
        }
		
        public static IQueryable<tbl_HRM_DeductionOnSalary> _sortBuilder(IQueryable<tbl_HRM_DeductionOnSalary> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_DeductionOnSalary>;
                        ...
                        switch (item)
                        {
							...
							case "IDStaff_desc":
                                ...
                                break;
							case "IDDeduction":
								query = isOrdered ? ordered.ThenBy(o => o.IDDeduction) : query.OrderBy(o => o.IDDeduction);
								 break;
							case "IDDeduction_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDDeduction) : query.OrderByDescending(o => o.IDDeduction);
                                break;
							case "AriseDay":
								query = isOrdered ? ordered.ThenBy(o => o.AriseDay) : query.OrderBy(o => o.AriseDay);
								 break;
							case "AriseDay_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.AriseDay) : query.OrderByDescending(o => o.AriseDay);
                                break;
							case "IDPayPeriod":
							...
							case "IDPayPeriod_desc":
                                ...
                                break;
							case "DeductionRate":
								query = isOrdered ? ordered.ThenBy(o => o.DeductionRate) : query.OrderBy(o => o.DeductionRate);
								 break;
							case "DeductionRate_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.DeductionRate) : query.OrderByDescending(o => o.DeductionRate);
                                break;
							case "IsExecuted":
								query = isOrdered ? ordered.ThenBy(o => o.IsExecuted) : query.OrderBy(o => o.IsExecuted);
								 break;
							case "IsExecuted_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IsExecuted) : query.OrderByDescending(o => o.IsExecuted);
                                break;
							case "IsAutomatedDeductionOnSalary":
								query = isOrdered ? ordered.ThenBy(o => o.IsAutomatedDeductionOnSalary) : query.OrderBy(o => o.IsAutomatedDeductionOnSalary);
								 break;
							case "IsAutomatedDeductionOnSalary_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IsAutomatedDeductionOnSalary) : query.OrderByDescending(o => o.IsAutomatedDeductionOnSalary);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_DeductionOnSalary> _pagingBuilder(IQueryable<tbl_HRM_DeductionOnSalary> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
