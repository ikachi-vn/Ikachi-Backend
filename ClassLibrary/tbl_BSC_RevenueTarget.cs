...
namespace ClassLibrary
{
    ...
    public partial class tbl_BSC_RevenueTarget
    {
...
{
	using System;
	public partial class DTO_BSC_RevenueTarget
	{
		public int IDBranch { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_BSC_RevenueTarget 
    {
		...
		public static IQueryable<DTO_BSC_RevenueTarget> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS BSC_RevenueTarget";
		...
		public static DTO_BSC_RevenueTarget getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_BSC_RevenueTarget.Find(id);
		...
		public static DTO_BSC_RevenueTarget getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_BSC_RevenueTarget
        ...
        {
            bool result = false;
            var dbitem = db.tbl_BSC_RevenueTarget.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_BSC_RevenueTarget",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_BSC_RevenueTarget dbitem, string Username)
        {
            Type type = typeof(tbl_BSC_RevenueTarget);
        ...
        public static void patchDTOtoDBValue( DTO_BSC_RevenueTarget item, tbl_BSC_RevenueTarget dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDSalesTeam = item.IDSalesTeam;        }

		public static DTO_BSC_RevenueTarget post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_BSC_RevenueTarget dbitem = new tbl_BSC_RevenueTarget();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_BSC_RevenueTarget.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_BSC_RevenueTarget",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_BSC_RevenueTarget();
                    ...
                    {
                        dbitem = new tbl_BSC_RevenueTarget();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_BSC_RevenueTarget.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_BSC_RevenueTarget.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_BSC_RevenueTarget.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_BSC_RevenueTarget", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_BSC_RevenueTarget",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_BSC_RevenueTarget",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_BSC_RevenueTarget.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_BSC_RevenueTarget.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_BSC_RevenueTarget> toDTO(IQueryable<tbl_BSC_RevenueTarget> query)
        {
			return query
			.Select(s => new DTO_BSC_RevenueTarget(){							
				IDBranch = s.IDBranch,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				...
				Year = s.Year,							
				Month = s.Month,							
				IDSalesTeam = s.IDSalesTeam,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_BSC_RevenueTarget toDTO(tbl_BSC_RevenueTarget dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_BSC_RevenueTarget()
				{							
					...
					IDSalesTeam = dbResult.IDSalesTeam,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_BSC_RevenueTarget> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_BSC_RevenueTarget> query = db.tbl_BSC_RevenueTarget.AsNoTracking();//.Where(d => d.IsDeleted == false);
			...
			if (QueryStrings.Any(d => d.Key == "ValueFrom") && QueryStrings.Any(d => d.Key == "ValueTo"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ValueFrom").Value, out decimal fromVal) && decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ValueTo").Value, out decimal toVal))
                    query = query.Where(d => fromVal <= d.Value && d.Value <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "Value"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "Value").Value, out decimal val))
                    query = query.Where(d => val == d.Value);
			...
			//Query IDSalesTeam (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDSalesTeam"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDSalesTeam").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDSalesTeam));
////                    query = query.Where(d => IDs.Contains(d.IDSalesTeam));
//                    
            }
            return query;
        ...
        public static IQueryable<tbl_BSC_RevenueTarget> _sortBuilder(IQueryable<tbl_BSC_RevenueTarget> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_BSC_RevenueTarget>;
                        ...
                        switch (item)
                        {
							...
							case "Month_desc":
                                ...
                                break;
							case "IDSalesTeam":
								query = isOrdered ? ordered.ThenBy(o => o.IDSalesTeam) : query.OrderBy(o => o.IDSalesTeam);
								 break;
							case "IDSalesTeam_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDSalesTeam) : query.OrderByDescending(o => o.IDSalesTeam);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_BSC_RevenueTarget> _pagingBuilder(IQueryable<tbl_BSC_RevenueTarget> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
