...
namespace ClassLibrary
{
    ...
    public partial class tbl_LIST_PayPeriod
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_LIST_PayPeriod()
        {
...
{
	using System;
	public partial class DTO_LIST_PayPeriod
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_LIST_PayPeriod 
    {
		...
		public static IQueryable<DTO_LIST_PayPeriod> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS LIST_PayPeriod";
		...
		public static DTO_LIST_PayPeriod getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_LIST_PayPeriod.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_LIST_PayPeriod getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_LIST_PayPeriod
        ...
        {
            bool result = false;
            var dbitem = db.tbl_LIST_PayPeriod.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_LIST_PayPeriod",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_LIST_PayPeriod dbitem, string Username)
        {
            Type type = typeof(tbl_LIST_PayPeriod);
        ...
        public static void patchDTOtoDBValue( DTO_LIST_PayPeriod item, tbl_LIST_PayPeriod dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.EndDay = item.EndDay;        }

		public static DTO_LIST_PayPeriod post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_LIST_PayPeriod dbitem = new tbl_LIST_PayPeriod();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_LIST_PayPeriod.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_LIST_PayPeriod",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_LIST_PayPeriod();
                    ...
                    {
                        dbitem = new tbl_LIST_PayPeriod();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_LIST_PayPeriod.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_LIST_PayPeriod.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_LIST_PayPeriod.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_LIST_PayPeriod", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_LIST_PayPeriod",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_LIST_PayPeriod",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_LIST_PayPeriod.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_LIST_PayPeriod.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_LIST_PayPeriod> toDTO(IQueryable<tbl_LIST_PayPeriod> query)
        {
			return query
			.Select(s => new DTO_LIST_PayPeriod(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
		...
		public static DTO_LIST_PayPeriod toDTO(tbl_LIST_PayPeriod dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_LIST_PayPeriod()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					StartDay = dbResult.StartDay,							
					EndDay = dbResult.EndDay,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_LIST_PayPeriod> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_LIST_PayPeriod> query = db.tbl_LIST_PayPeriod.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query StartDay (Nullable<System.DateTime>)
			if (QueryStrings.Any(d => d.Key == "StartDayFrom") && QueryStrings.Any(d => d.Key == "StartDayTo"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "StartDayFrom").Value, out DateTime fromDate) && DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "StartDayTo").Value, out DateTime toDate))
                    query = query.Where(d => fromDate <= d.StartDay && d.StartDay <= toDate);

            if (QueryStrings.Any(d => d.Key == "StartDay"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "StartDay").Value, out DateTime val))
                    query = query.Where(d => d.StartDay != null && DbFunctions.TruncateTime(val) == DbFunctions.TruncateTime(d.StartDay));


			//Query EndDay (Nullable<System.DateTime>)
			if (QueryStrings.Any(d => d.Key == "EndDayFrom") && QueryStrings.Any(d => d.Key == "EndDayTo"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "EndDayFrom").Value, out DateTime fromDate) && DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "EndDayTo").Value, out DateTime toDate))
                    query = query.Where(d => fromDate <= d.EndDay && d.EndDay <= toDate);

            if (QueryStrings.Any(d => d.Key == "EndDay"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "EndDay").Value, out DateTime val))
                    query = query.Where(d => d.EndDay != null && DbFunctions.TruncateTime(val) == DbFunctions.TruncateTime(d.EndDay));

            return query;
        }
		
        public static IQueryable<tbl_LIST_PayPeriod> _sortBuilder(IQueryable<tbl_LIST_PayPeriod> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_LIST_PayPeriod>;
                        ...
                        switch (item)
                        {
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "StartDay":
								query = isOrdered ? ordered.ThenBy(o => o.StartDay) : query.OrderBy(o => o.StartDay);
								 break;
							case "StartDay_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.StartDay) : query.OrderByDescending(o => o.StartDay);
                                break;
							case "EndDay":
								query = isOrdered ? ordered.ThenBy(o => o.EndDay) : query.OrderBy(o => o.EndDay);
								 break;
							case "EndDay_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.EndDay) : query.OrderByDescending(o => o.EndDay);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_LIST_PayPeriod> _pagingBuilder(IQueryable<tbl_LIST_PayPeriod> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
