...
namespace ClassLibrary
{
    ...
    public partial class tbl_WMS_PutawayStrategyDetail
    {
        public int IDPutawayStrategy { get; set; }
        ...
        public bool ChecksRestrictions { get; set; }
        public int DimensionRestriction { get; set; }
        //List 0:1
        public virtual tbl_WMS_PutawayStrategy tbl_WMS_PutawayStrategy { get; set; }
    }
}
...
{
	using System;
	public partial class DTO_WMS_PutawayStrategyDetail
	{
		public int IDPutawayStrategy { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_WMS_PutawayStrategyDetail 
    {
		...
		public static IQueryable<DTO_WMS_PutawayStrategyDetail> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS WMS_PutawayStrategyDetail";
            ...
            if (workBook != null)
            {
                ...
                //var WMS_PutawayStrategyList = BS_WMS_PutawayStrategy.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_WMS_PutawayStrategyDetail getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_WMS_PutawayStrategyDetail.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_WMS_PutawayStrategyDetail getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_WMS_PutawayStrategyDetail
        ...
        {
            bool result = false;
            var dbitem = db.tbl_WMS_PutawayStrategyDetail.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_WMS_PutawayStrategyDetail",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_WMS_PutawayStrategyDetail dbitem, string Username)
        {
            Type type = typeof(tbl_WMS_PutawayStrategyDetail);
        ...
        public static void patchDTOtoDBValue( DTO_WMS_PutawayStrategyDetail item, tbl_WMS_PutawayStrategyDetail dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDPutawayStrategy = item.IDPutawayStrategy;							
			...
			dbitem.PutawayType = item.PutawayType;							
			dbitem.PutawayZone = item.PutawayZone;							
			dbitem.FromLocation = item.FromLocation;							
			dbitem.ToLocation = item.ToLocation;							
			dbitem.ChecksRestrictions = item.ChecksRestrictions;							
			dbitem.DimensionRestriction = item.DimensionRestriction;        }

		public static DTO_WMS_PutawayStrategyDetail post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_WMS_PutawayStrategyDetail dbitem = new tbl_WMS_PutawayStrategyDetail();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_WMS_PutawayStrategyDetail.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_WMS_PutawayStrategyDetail",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_WMS_PutawayStrategyDetail();
                    ...
                    {
                        dbitem = new tbl_WMS_PutawayStrategyDetail();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_WMS_PutawayStrategyDetail.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_WMS_PutawayStrategyDetail.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_WMS_PutawayStrategyDetail.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_WMS_PutawayStrategyDetail", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_WMS_PutawayStrategyDetail",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_WMS_PutawayStrategyDetail",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_WMS_PutawayStrategyDetail.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_WMS_PutawayStrategyDetail.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_WMS_PutawayStrategyDetail> toDTO(IQueryable<tbl_WMS_PutawayStrategyDetail> query)
        {
			return query
			.Select(s => new DTO_WMS_PutawayStrategyDetail(){							
				IDPutawayStrategy = s.IDPutawayStrategy,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				PutawayType = s.PutawayType,							
				PutawayZone = s.PutawayZone,							
				FromLocation = s.FromLocation,							
				ToLocation = s.ToLocation,							
				ChecksRestrictions = s.ChecksRestrictions,							
				DimensionRestriction = s.DimensionRestriction,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_WMS_PutawayStrategyDetail toDTO(tbl_WMS_PutawayStrategyDetail dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_WMS_PutawayStrategyDetail()
				{							
					IDPutawayStrategy = dbResult.IDPutawayStrategy,							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					PutawayType = dbResult.PutawayType,							
					PutawayZone = dbResult.PutawayZone,							
					FromLocation = dbResult.FromLocation,							
					ToLocation = dbResult.ToLocation,							
					ChecksRestrictions = dbResult.ChecksRestrictions,							
					DimensionRestriction = dbResult.DimensionRestriction,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_WMS_PutawayStrategyDetail> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_WMS_PutawayStrategyDetail> query = db.tbl_WMS_PutawayStrategyDetail.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query IDPutawayStrategy (int)
			if (QueryStrings.Any(d => d.Key == "IDPutawayStrategy"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDPutawayStrategy").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDPutawayStrategy));
            }
			...
			//Query PutawayType (int)
			if (QueryStrings.Any(d => d.Key == "PutawayTypeFrom") && QueryStrings.Any(d => d.Key == "PutawayTypeTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "PutawayTypeFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "PutawayTypeTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.PutawayType && d.PutawayType <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "PutawayType"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "PutawayType").Value, out int val))
                    query = query.Where(d => val == d.PutawayType);
			...
			//Query ChecksRestrictions (bool)
			if (QueryStrings.Any(d => d.Key == "ChecksRestrictions"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "ChecksRestrictions").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.ChecksRestrictions);
            }

			//Query DimensionRestriction (int)
			if (QueryStrings.Any(d => d.Key == "DimensionRestrictionFrom") && QueryStrings.Any(d => d.Key == "DimensionRestrictionTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "DimensionRestrictionFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "DimensionRestrictionTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.DimensionRestriction && d.DimensionRestriction <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "DimensionRestriction"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "DimensionRestriction").Value, out int val))
                    query = query.Where(d => val == d.DimensionRestriction);

            return query;
        }
		
        public static IQueryable<tbl_WMS_PutawayStrategyDetail> _sortBuilder(IQueryable<tbl_WMS_PutawayStrategyDetail> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_WMS_PutawayStrategyDetail>;
                        bool isOrdered = ordered.ToString().IndexOf("ORDER BY") !=-1;

                        switch (item)
                        {
							case "IDPutawayStrategy":
								query = isOrdered ? ordered.ThenBy(o => o.IDPutawayStrategy) : query.OrderBy(o => o.IDPutawayStrategy);
								 break;
							case "IDPutawayStrategy_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDPutawayStrategy) : query.OrderByDescending(o => o.IDPutawayStrategy);
                                break;
							case "Id":
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "PutawayType":
								query = isOrdered ? ordered.ThenBy(o => o.PutawayType) : query.OrderBy(o => o.PutawayType);
								 break;
							case "PutawayType_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.PutawayType) : query.OrderByDescending(o => o.PutawayType);
                                break;
							case "PutawayZone":
							...
							case "ToLocation_desc":
                                ...
                                break;
							case "ChecksRestrictions":
								query = isOrdered ? ordered.ThenBy(o => o.ChecksRestrictions) : query.OrderBy(o => o.ChecksRestrictions);
								 break;
							case "ChecksRestrictions_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.ChecksRestrictions) : query.OrderByDescending(o => o.ChecksRestrictions);
                                break;
							case "DimensionRestriction":
								query = isOrdered ? ordered.ThenBy(o => o.DimensionRestriction) : query.OrderBy(o => o.DimensionRestriction);
								 break;
							case "DimensionRestriction_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.DimensionRestriction) : query.OrderByDescending(o => o.DimensionRestriction);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_WMS_PutawayStrategyDetail> _pagingBuilder(IQueryable<tbl_WMS_PutawayStrategyDetail> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
