...
namespace ClassLibrary
{
    ...
    public partial class tbl_BANK_PaymentTerm
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_BANK_PaymentTerm()
        {
        ...
        public decimal FirstPartialPayment { get; set; }
        public string OpenIncomingPayment { get; set; }
...
{
	using System;
	public partial class DTO_BANK_PaymentTerm
	{
		public int IDBranch { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_BANK_PaymentTerm 
    {
		...
		public static IQueryable<DTO_BANK_PaymentTerm> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS BANK_PaymentTerm";
		...
		public static DTO_BANK_PaymentTerm getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_BANK_PaymentTerm.Find(id);
		...
		public static DTO_BANK_PaymentTerm getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_BANK_PaymentTerm
        ...
        {
            bool result = false;
            var dbitem = db.tbl_BANK_PaymentTerm.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_BANK_PaymentTerm",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_BANK_PaymentTerm dbitem, string Username)
        {
            Type type = typeof(tbl_BANK_PaymentTerm);
        ...
        public static void patchDTOtoDBValue( DTO_BANK_PaymentTerm item, tbl_BANK_PaymentTerm dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.DueDateBasedOn = item.DueDateBasedOn;							
			dbitem.StartFrom = item.StartFrom;							
			dbitem.NumberOfAdditionalMonths = item.NumberOfAdditionalMonths;							
			dbitem.NumberOfAdditionalDays = item.NumberOfAdditionalDays;							
			dbitem.ToleranceDays = item.ToleranceDays;							
			dbitem.NumberOfInstallments = item.NumberOfInstallments;							
			dbitem.FirstPartialPayment = item.FirstPartialPayment;							
			dbitem.OpenIncomingPayment = item.OpenIncomingPayment;							
			dbitem.IDPriceList = item.IDPriceList;        }

		public static DTO_BANK_PaymentTerm post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_BANK_PaymentTerm dbitem = new tbl_BANK_PaymentTerm();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_BANK_PaymentTerm.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_BANK_PaymentTerm",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_BANK_PaymentTerm();
                    ...
                    {
                        dbitem = new tbl_BANK_PaymentTerm();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_BANK_PaymentTerm.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_BANK_PaymentTerm.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_BANK_PaymentTerm.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_BANK_PaymentTerm", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_BANK_PaymentTerm",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_BANK_PaymentTerm",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_BANK_PaymentTerm.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_BANK_PaymentTerm.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_BANK_PaymentTerm> toDTO(IQueryable<tbl_BANK_PaymentTerm> query)
        {
			return query
			.Select(s => new DTO_BANK_PaymentTerm(){							
				IDBranch = s.IDBranch,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				...
				DueDateBasedOn = s.DueDateBasedOn,							
				StartFrom = s.StartFrom,							
				NumberOfAdditionalMonths = s.NumberOfAdditionalMonths,							
				NumberOfAdditionalDays = s.NumberOfAdditionalDays,							
				ToleranceDays = s.ToleranceDays,							
				NumberOfInstallments = s.NumberOfInstallments,							
				FirstPartialPayment = s.FirstPartialPayment,							
				OpenIncomingPayment = s.OpenIncomingPayment,							
				IDPriceList = s.IDPriceList,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_BANK_PaymentTerm toDTO(tbl_BANK_PaymentTerm dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_BANK_PaymentTerm()
				{							
					...
					DueDateBasedOn = dbResult.DueDateBasedOn,							
					StartFrom = dbResult.StartFrom,							
					NumberOfAdditionalMonths = dbResult.NumberOfAdditionalMonths,							
					NumberOfAdditionalDays = dbResult.NumberOfAdditionalDays,							
					ToleranceDays = dbResult.ToleranceDays,							
					NumberOfInstallments = dbResult.NumberOfInstallments,							
					FirstPartialPayment = dbResult.FirstPartialPayment,							
					OpenIncomingPayment = dbResult.OpenIncomingPayment,							
		...
		public static IQueryable<tbl_BANK_PaymentTerm> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_BANK_PaymentTerm> query = db.tbl_BANK_PaymentTerm.AsNoTracking();//.Where(d => d.IsDeleted == false);
			...
			//Query DueDateBasedOn (string)
			if (QueryStrings.Any(d => d.Key == "DueDateBasedOn_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "DueDateBasedOn_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "DueDateBasedOn_eq").Value;
                query = query.Where(d=>d.DueDateBasedOn == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "DueDateBasedOn") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "DueDateBasedOn").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "DueDateBasedOn").Value;
                query = query.Where(d=>d.DueDateBasedOn.Contains(keyword));
            }
            

			//Query StartFrom (string)
			if (QueryStrings.Any(d => d.Key == "StartFrom_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "StartFrom_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "StartFrom_eq").Value;
                query = query.Where(d=>d.StartFrom == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "StartFrom") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "StartFrom").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "StartFrom").Value;
                query = query.Where(d=>d.StartFrom.Contains(keyword));
            }
            

			//Query NumberOfAdditionalMonths (int)
			if (QueryStrings.Any(d => d.Key == "NumberOfAdditionalMonthsFrom") && QueryStrings.Any(d => d.Key == "NumberOfAdditionalMonthsTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "NumberOfAdditionalMonthsFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "NumberOfAdditionalMonthsTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.NumberOfAdditionalMonths && d.NumberOfAdditionalMonths <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "NumberOfAdditionalMonths"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "NumberOfAdditionalMonths").Value, out int val))
                    query = query.Where(d => val == d.NumberOfAdditionalMonths);


			//Query NumberOfAdditionalDays (int)
			if (QueryStrings.Any(d => d.Key == "NumberOfAdditionalDaysFrom") && QueryStrings.Any(d => d.Key == "NumberOfAdditionalDaysTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "NumberOfAdditionalDaysFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "NumberOfAdditionalDaysTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.NumberOfAdditionalDays && d.NumberOfAdditionalDays <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "NumberOfAdditionalDays"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "NumberOfAdditionalDays").Value, out int val))
                    query = query.Where(d => val == d.NumberOfAdditionalDays);


			//Query ToleranceDays (int)
			if (QueryStrings.Any(d => d.Key == "ToleranceDaysFrom") && QueryStrings.Any(d => d.Key == "ToleranceDaysTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ToleranceDaysFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ToleranceDaysTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.ToleranceDays && d.ToleranceDays <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "ToleranceDays"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ToleranceDays").Value, out int val))
                    query = query.Where(d => val == d.ToleranceDays);


			//Query NumberOfInstallments (int)
			if (QueryStrings.Any(d => d.Key == "NumberOfInstallmentsFrom") && QueryStrings.Any(d => d.Key == "NumberOfInstallmentsTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "NumberOfInstallmentsFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "NumberOfInstallmentsTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.NumberOfInstallments && d.NumberOfInstallments <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "NumberOfInstallments"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "NumberOfInstallments").Value, out int val))
                    query = query.Where(d => val == d.NumberOfInstallments);


			//Query FirstPartialPayment (decimal)
			if (QueryStrings.Any(d => d.Key == "FirstPartialPaymentFrom") && QueryStrings.Any(d => d.Key == "FirstPartialPaymentTo"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "FirstPartialPaymentFrom").Value, out decimal fromVal) && decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "FirstPartialPaymentTo").Value, out decimal toVal))
                    query = query.Where(d => fromVal <= d.FirstPartialPayment && d.FirstPartialPayment <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "FirstPartialPayment"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "FirstPartialPayment").Value, out decimal val))
                    query = query.Where(d => val == d.FirstPartialPayment);


			//Query OpenIncomingPayment (string)
			if (QueryStrings.Any(d => d.Key == "OpenIncomingPayment_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "OpenIncomingPayment_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "OpenIncomingPayment_eq").Value;
                query = query.Where(d=>d.OpenIncomingPayment == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "OpenIncomingPayment") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "OpenIncomingPayment").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "OpenIncomingPayment").Value;
                query = query.Where(d=>d.OpenIncomingPayment.Contains(keyword));
            }
...
//                    
        ...
        public static IQueryable<tbl_BANK_PaymentTerm> _sortBuilder(IQueryable<tbl_BANK_PaymentTerm> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_BANK_PaymentTerm>;
                        ...
                        switch (item)
                        {
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "DueDateBasedOn":
								query = isOrdered ? ordered.ThenBy(o => o.DueDateBasedOn) : query.OrderBy(o => o.DueDateBasedOn);
								 break;
							case "DueDateBasedOn_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.DueDateBasedOn) : query.OrderByDescending(o => o.DueDateBasedOn);
                                break;
							case "StartFrom":
								query = isOrdered ? ordered.ThenBy(o => o.StartFrom) : query.OrderBy(o => o.StartFrom);
								 break;
							case "StartFrom_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.StartFrom) : query.OrderByDescending(o => o.StartFrom);
                                break;
							case "NumberOfAdditionalMonths":
								query = isOrdered ? ordered.ThenBy(o => o.NumberOfAdditionalMonths) : query.OrderBy(o => o.NumberOfAdditionalMonths);
								 break;
							case "NumberOfAdditionalMonths_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.NumberOfAdditionalMonths) : query.OrderByDescending(o => o.NumberOfAdditionalMonths);
                                break;
							case "NumberOfAdditionalDays":
								query = isOrdered ? ordered.ThenBy(o => o.NumberOfAdditionalDays) : query.OrderBy(o => o.NumberOfAdditionalDays);
								 break;
							case "NumberOfAdditionalDays_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.NumberOfAdditionalDays) : query.OrderByDescending(o => o.NumberOfAdditionalDays);
                                break;
							case "ToleranceDays":
								query = isOrdered ? ordered.ThenBy(o => o.ToleranceDays) : query.OrderBy(o => o.ToleranceDays);
								 break;
							case "ToleranceDays_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.ToleranceDays) : query.OrderByDescending(o => o.ToleranceDays);
                                break;
							case "NumberOfInstallments":
								query = isOrdered ? ordered.ThenBy(o => o.NumberOfInstallments) : query.OrderBy(o => o.NumberOfInstallments);
								 break;
							case "NumberOfInstallments_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.NumberOfInstallments) : query.OrderByDescending(o => o.NumberOfInstallments);
                                break;
							case "FirstPartialPayment":
								query = isOrdered ? ordered.ThenBy(o => o.FirstPartialPayment) : query.OrderBy(o => o.FirstPartialPayment);
								 break;
							case "FirstPartialPayment_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.FirstPartialPayment) : query.OrderByDescending(o => o.FirstPartialPayment);
                                break;
							case "OpenIncomingPayment":
								query = isOrdered ? ordered.ThenBy(o => o.OpenIncomingPayment) : query.OrderBy(o => o.OpenIncomingPayment);
								 break;
							case "OpenIncomingPayment_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.OpenIncomingPayment) : query.OrderByDescending(o => o.OpenIncomingPayment);
                                break;
							case "IDPriceList":
        ...
        public static IQueryable<tbl_BANK_PaymentTerm> _pagingBuilder(IQueryable<tbl_BANK_PaymentTerm> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
