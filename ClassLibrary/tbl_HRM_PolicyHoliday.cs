...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_PolicyHoliday
    {
...
{
	using System;
	public partial class DTO_HRM_PolicyHoliday
	{
		public int IDBranch { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_PolicyHoliday 
    {
		...
		public static IQueryable<DTO_HRM_PolicyHoliday> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_PolicyHoliday";
		...
		public static DTO_HRM_PolicyHoliday getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_PolicyHoliday.Find(id);
		...
		public static DTO_HRM_PolicyHoliday getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_PolicyHoliday
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_PolicyHoliday.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_PolicyHoliday",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_PolicyHoliday dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_PolicyHoliday);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_PolicyHoliday item, tbl_HRM_PolicyHoliday dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_HRM_PolicyHoliday post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_PolicyHoliday dbitem = new tbl_HRM_PolicyHoliday();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_PolicyHoliday.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_PolicyHoliday",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_PolicyHoliday();
                    ...
                    {
                        dbitem = new tbl_HRM_PolicyHoliday();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_PolicyHoliday.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_PolicyHoliday.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_PolicyHoliday.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_HRM_PolicyHoliday", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_PolicyHoliday",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_PolicyHoliday",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_PolicyHoliday.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_PolicyHoliday.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_PolicyHoliday> toDTO(IQueryable<tbl_HRM_PolicyHoliday> query)
        {
			return query
			.Select(s => new DTO_HRM_PolicyHoliday(){							
				IDBranch = s.IDBranch,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
		...
		public static DTO_HRM_PolicyHoliday toDTO(tbl_HRM_PolicyHoliday dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_PolicyHoliday()
				{							
		...
		public static IQueryable<tbl_HRM_PolicyHoliday> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_PolicyHoliday> query = db.tbl_HRM_PolicyHoliday.AsNoTracking();//.Where(d => d.IsDeleted == false);
			...
			//Query FromDate (System.DateTime)
        ...
        public static IQueryable<tbl_HRM_PolicyHoliday> _sortBuilder(IQueryable<tbl_HRM_PolicyHoliday> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_PolicyHoliday>;
        ...
        public static IQueryable<tbl_HRM_PolicyHoliday> _pagingBuilder(IQueryable<tbl_HRM_PolicyHoliday> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
