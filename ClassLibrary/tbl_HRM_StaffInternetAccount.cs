...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffInternetAccount
    {
        ...
        public Nullable<bool> IsBelongToCompany { get; set; }
        //List 0:1
...
{
	using System;
	public partial class DTO_HRM_StaffInternetAccount
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffInternetAccount 
    {
		...
		public static IQueryable<DTO_HRM_StaffInternetAccount> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffInternetAccount";
		...
		public static DTO_HRM_StaffInternetAccount getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffInternetAccount.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffInternetAccount getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffInternetAccount
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffInternetAccount.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffInternetAccount",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffInternetAccount dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffInternetAccount);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffInternetAccount item, tbl_HRM_StaffInternetAccount dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.Classification = item.Classification;							
			dbitem.Account = item.Account;							
			dbitem.URL = item.URL;							
			dbitem.IsBelongToCompany = item.IsBelongToCompany;        }

		public static DTO_HRM_StaffInternetAccount post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffInternetAccount dbitem = new tbl_HRM_StaffInternetAccount();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffInternetAccount.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffInternetAccount",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffInternetAccount();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffInternetAccount();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffInternetAccount.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffInternetAccount.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffInternetAccount.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffInternetAccount", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffInternetAccount",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_StaffInternetAccount",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffInternetAccount.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffInternetAccount.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffInternetAccount> toDTO(IQueryable<tbl_HRM_StaffInternetAccount> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffInternetAccount(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				Account = s.Account,							
				URL = s.URL,							
				IsBelongToCompany = s.IsBelongToCompany,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_HRM_StaffInternetAccount toDTO(tbl_HRM_StaffInternetAccount dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffInternetAccount()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					URL = dbResult.URL,							
					IsBelongToCompany = dbResult.IsBelongToCompany,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_HRM_StaffInternetAccount> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffInternetAccount> query = db.tbl_HRM_StaffInternetAccount.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			if (QueryStrings.Any(d => d.Key == "Classification_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Classification_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Classification_eq").Value;
                query = query.Where(d=>d.Classification == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "Classification") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Classification").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Classification").Value;
                query = query.Where(d=>d.Classification.Contains(keyword));
            }
			...
			//Query IsBelongToCompany (Nullable<bool>)
            return query;
        }
		
        public static IQueryable<tbl_HRM_StaffInternetAccount> _sortBuilder(IQueryable<tbl_HRM_StaffInternetAccount> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffInternetAccount>;
                        ...
                        switch (item)
                        {
							...
							case "IDStaff_desc":
                                ...
                                break;
							case "Classification":
								query = isOrdered ? ordered.ThenBy(o => o.Classification) : query.OrderBy(o => o.Classification);
								 break;
							case "Classification_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Classification) : query.OrderByDescending(o => o.Classification);
                                break;
							case "Account":
							...
							case "URL_desc":
                                ...
                                break;
							case "IsBelongToCompany":
								query = isOrdered ? ordered.ThenBy(o => o.IsBelongToCompany) : query.OrderBy(o => o.IsBelongToCompany);
								 break;
							case "IsBelongToCompany_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IsBelongToCompany) : query.OrderByDescending(o => o.IsBelongToCompany);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_StaffInternetAccount> _pagingBuilder(IQueryable<tbl_HRM_StaffInternetAccount> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
