...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffTrainingProcess
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_HRM_StaffTrainingProcess()
        {
...
{
	using System;
	public partial class DTO_HRM_StaffTrainingProcess
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffTrainingProcess 
    {
		...
		public static IQueryable<DTO_HRM_StaffTrainingProcess> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffTrainingProcess";
		...
		public static DTO_HRM_StaffTrainingProcess getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffTrainingProcess.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffTrainingProcess getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffTrainingProcess
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffTrainingProcess.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffTrainingProcess",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffTrainingProcess dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffTrainingProcess);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffTrainingProcess item, tbl_HRM_StaffTrainingProcess dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.FromYear = item.FromYear;							
			dbitem.ToYear = item.ToYear;							
			dbitem.Place = item.Place;							
			dbitem.Result = item.Result;							
			dbitem.TrainingProgram = item.TrainingProgram;							
			dbitem.TrainingContent = item.TrainingContent;        }

		public static DTO_HRM_StaffTrainingProcess post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffTrainingProcess dbitem = new tbl_HRM_StaffTrainingProcess();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffTrainingProcess.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffTrainingProcess",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffTrainingProcess();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffTrainingProcess();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffTrainingProcess.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffTrainingProcess.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffTrainingProcess.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffTrainingProcess", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffTrainingProcess",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_StaffTrainingProcess",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffTrainingProcess.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffTrainingProcess.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffTrainingProcess> toDTO(IQueryable<tbl_HRM_StaffTrainingProcess> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffTrainingProcess(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
		...
		public static DTO_HRM_StaffTrainingProcess toDTO(tbl_HRM_StaffTrainingProcess dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffTrainingProcess()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					FromYear = dbResult.FromYear,							
					ToYear = dbResult.ToYear,							
					Place = dbResult.Place,							
					Result = dbResult.Result,							
					TrainingProgram = dbResult.TrainingProgram,							
					TrainingContent = dbResult.TrainingContent,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_HRM_StaffTrainingProcess> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffTrainingProcess> query = db.tbl_HRM_StaffTrainingProcess.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query FromYear (Nullable<System.DateTime>)
			if (QueryStrings.Any(d => d.Key == "FromYearFrom") && QueryStrings.Any(d => d.Key == "FromYearTo"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "FromYearFrom").Value, out DateTime fromDate) && DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "FromYearTo").Value, out DateTime toDate))
                    query = query.Where(d => fromDate <= d.FromYear && d.FromYear <= toDate);

            if (QueryStrings.Any(d => d.Key == "FromYear"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "FromYear").Value, out DateTime val))
                    query = query.Where(d => d.FromYear != null && DbFunctions.TruncateTime(val) == DbFunctions.TruncateTime(d.FromYear));


			//Query ToYear (Nullable<System.DateTime>)
			if (QueryStrings.Any(d => d.Key == "ToYearFrom") && QueryStrings.Any(d => d.Key == "ToYearTo"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ToYearFrom").Value, out DateTime fromDate) && DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ToYearTo").Value, out DateTime toDate))
                    query = query.Where(d => fromDate <= d.ToYear && d.ToYear <= toDate);

            if (QueryStrings.Any(d => d.Key == "ToYear"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ToYear").Value, out DateTime val))
                    query = query.Where(d => d.ToYear != null && DbFunctions.TruncateTime(val) == DbFunctions.TruncateTime(d.ToYear));


			//Query Place (string)
			if (QueryStrings.Any(d => d.Key == "Place_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Place_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Place_eq").Value;
                query = query.Where(d=>d.Place == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "Place") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Place").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Place").Value;
                query = query.Where(d=>d.Place.Contains(keyword));
            }
            

			//Query Result (string)
			if (QueryStrings.Any(d => d.Key == "Result_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Result_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Result_eq").Value;
                query = query.Where(d=>d.Result == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "Result") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Result").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Result").Value;
                query = query.Where(d=>d.Result.Contains(keyword));
            }
            

			//Query TrainingProgram (string)
			if (QueryStrings.Any(d => d.Key == "TrainingProgram_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "TrainingProgram_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "TrainingProgram_eq").Value;
                query = query.Where(d=>d.TrainingProgram == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "TrainingProgram") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "TrainingProgram").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "TrainingProgram").Value;
                query = query.Where(d=>d.TrainingProgram.Contains(keyword));
            }
            

			//Query TrainingContent (string)
			if (QueryStrings.Any(d => d.Key == "TrainingContent_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "TrainingContent_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "TrainingContent_eq").Value;
                query = query.Where(d=>d.TrainingContent == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "TrainingContent") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "TrainingContent").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "TrainingContent").Value;
                query = query.Where(d=>d.TrainingContent.Contains(keyword));
            }
            
            return query;
        }
		
        public static IQueryable<tbl_HRM_StaffTrainingProcess> _sortBuilder(IQueryable<tbl_HRM_StaffTrainingProcess> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffTrainingProcess>;
                        ...
                        switch (item)
                        {
							...
							case "IDStaff_desc":
                                ...
                                break;
							case "FromYear":
								query = isOrdered ? ordered.ThenBy(o => o.FromYear) : query.OrderBy(o => o.FromYear);
								 break;
							case "FromYear_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.FromYear) : query.OrderByDescending(o => o.FromYear);
                                break;
							case "ToYear":
								query = isOrdered ? ordered.ThenBy(o => o.ToYear) : query.OrderBy(o => o.ToYear);
								 break;
							case "ToYear_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.ToYear) : query.OrderByDescending(o => o.ToYear);
                                break;
							case "Place":
								query = isOrdered ? ordered.ThenBy(o => o.Place) : query.OrderBy(o => o.Place);
								 break;
							case "Place_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Place) : query.OrderByDescending(o => o.Place);
                                break;
							case "Result":
								query = isOrdered ? ordered.ThenBy(o => o.Result) : query.OrderBy(o => o.Result);
								 break;
							case "Result_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Result) : query.OrderByDescending(o => o.Result);
                                break;
							case "TrainingProgram":
								query = isOrdered ? ordered.ThenBy(o => o.TrainingProgram) : query.OrderBy(o => o.TrainingProgram);
								 break;
							case "TrainingProgram_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.TrainingProgram) : query.OrderByDescending(o => o.TrainingProgram);
                                break;
							case "TrainingContent":
								query = isOrdered ? ordered.ThenBy(o => o.TrainingContent) : query.OrderBy(o => o.TrainingContent);
								 break;
							case "TrainingContent_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.TrainingContent) : query.OrderByDescending(o => o.TrainingContent);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_StaffTrainingProcess> _pagingBuilder(IQueryable<tbl_HRM_StaffTrainingProcess> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
