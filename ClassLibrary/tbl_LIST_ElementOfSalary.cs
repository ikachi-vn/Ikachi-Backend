...
namespace ClassLibrary
{
    ...
    public partial class tbl_LIST_ElementOfSalary
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_LIST_ElementOfSalary()
        {
...
{
	using System;
	public partial class DTO_LIST_ElementOfSalary
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_LIST_ElementOfSalary 
    {
		...
		public static IQueryable<DTO_LIST_ElementOfSalary> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS LIST_ElementOfSalary";
		...
		public static DTO_LIST_ElementOfSalary getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_LIST_ElementOfSalary.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_LIST_ElementOfSalary getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_LIST_ElementOfSalary
        ...
        {
            bool result = false;
            var dbitem = db.tbl_LIST_ElementOfSalary.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_LIST_ElementOfSalary",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_LIST_ElementOfSalary dbitem, string Username)
        {
            Type type = typeof(tbl_LIST_ElementOfSalary);
        ...
        public static void patchDTOtoDBValue( DTO_LIST_ElementOfSalary item, tbl_LIST_ElementOfSalary dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.Recipe = item.Recipe;        }

		public static DTO_LIST_ElementOfSalary post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_LIST_ElementOfSalary dbitem = new tbl_LIST_ElementOfSalary();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_LIST_ElementOfSalary.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_LIST_ElementOfSalary",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_LIST_ElementOfSalary();
                    ...
                    {
                        dbitem = new tbl_LIST_ElementOfSalary();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_LIST_ElementOfSalary.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_LIST_ElementOfSalary.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_LIST_ElementOfSalary.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_LIST_ElementOfSalary", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_LIST_ElementOfSalary",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_LIST_ElementOfSalary",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_LIST_ElementOfSalary.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_LIST_ElementOfSalary.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_LIST_ElementOfSalary> toDTO(IQueryable<tbl_LIST_ElementOfSalary> query)
        {
			return query
			.Select(s => new DTO_LIST_ElementOfSalary(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
		...
		public static DTO_LIST_ElementOfSalary toDTO(tbl_LIST_ElementOfSalary dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_LIST_ElementOfSalary()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
		...
		public static IQueryable<tbl_LIST_ElementOfSalary> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_LIST_ElementOfSalary> query = db.tbl_LIST_ElementOfSalary.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query Recipe (string)
			if (QueryStrings.Any(d => d.Key == "Recipe_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Recipe_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Recipe_eq").Value;
                query = query.Where(d=>d.Recipe == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "Recipe") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Recipe").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Recipe").Value;
                query = query.Where(d=>d.Recipe.Contains(keyword));
            }
            
            return query;
        }
		
        public static IQueryable<tbl_LIST_ElementOfSalary> _sortBuilder(IQueryable<tbl_LIST_ElementOfSalary> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_LIST_ElementOfSalary>;
                        ...
                        switch (item)
                        {
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "Recipe":
								query = isOrdered ? ordered.ThenBy(o => o.Recipe) : query.OrderBy(o => o.Recipe);
								 break;
							case "Recipe_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Recipe) : query.OrderByDescending(o => o.Recipe);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_LIST_ElementOfSalary> _pagingBuilder(IQueryable<tbl_LIST_ElementOfSalary> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
