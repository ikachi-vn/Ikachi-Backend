...
namespace ClassLibrary
{
    ...
    public partial class tbl_WMS_Vendor
    {
...
{
	using System;
	public partial class DTO_WMS_Vendor
	{
		public int Id { get; set; }
		public string Remark { get; set; }
		public Nullable<int> Sort { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_WMS_Vendor 
    {
		...
		public static IQueryable<DTO_WMS_Vendor> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS WMS_Vendor";
		...
		public static DTO_WMS_Vendor getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_WMS_Vendor.Find(id);
        ...
        {
            bool result = false;
            var dbitem = db.tbl_WMS_Vendor.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_WMS_Vendor",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_WMS_Vendor dbitem, string Username)
        {
            Type type = typeof(tbl_WMS_Vendor);
        ...
        public static void patchDTOtoDBValue( DTO_WMS_Vendor item, tbl_WMS_Vendor dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_WMS_Vendor post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_WMS_Vendor dbitem = new tbl_WMS_Vendor();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_WMS_Vendor.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_WMS_Vendor",e);
                    return null;
                }
            }
            return toDTO(dbitem);
        ...
        public static dynamic import(ExcelPackage package, AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings, string Username)
        {
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    {
                        dbitem = new tbl_WMS_Vendor();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_WMS_Vendor.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_WMS_Vendor.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_WMS_Vendor.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_WMS_Vendor", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_WMS_Vendor",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_WMS_Vendor",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_WMS_Vendor.Any(e => e.Id == id && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_WMS_Vendor> toDTO(IQueryable<tbl_WMS_Vendor> query)
        {
			return query
			.Select(s => new DTO_WMS_Vendor(){							
				Id = s.Id,							
				Remark = s.Remark,							
				Sort = s.Sort,							
		...
		public static DTO_WMS_Vendor toDTO(tbl_WMS_Vendor dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_WMS_Vendor()
				{							
					Id = dbResult.Id,							
		...
		public static IQueryable<tbl_WMS_Vendor> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_WMS_Vendor> query = db.tbl_WMS_Vendor.AsNoTracking();//.Where(d => d.IsDeleted == false );
        ...
        public static IQueryable<tbl_WMS_Vendor> _sortBuilder(IQueryable<tbl_WMS_Vendor> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_WMS_Vendor>;
        ...
        public static IQueryable<tbl_WMS_Vendor> _pagingBuilder(IQueryable<tbl_WMS_Vendor> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
