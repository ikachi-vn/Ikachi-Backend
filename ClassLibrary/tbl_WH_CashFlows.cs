...
{
	using System;
	public partial class DTO_WH_CashFlows
	{
		public int Id { get; set; }
		public Nullable<int> IDBranch { get; set; }
		public Nullable<short> IDParent { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_WH_CashFlows 
    {
		...
		public static IQueryable<DTO_WH_CashFlows> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            ...
            if (QueryStrings.Any(d => d.Key == "AllChildren")  || QueryStrings.Any(d => d.Key == "AllParent"))
            {
                ...
                List<DTO_WH_CashFlows> allItems = db.tbl_WH_CashFlows.Where(d => d.IsDeleted == false).Select(s => new DTO_WH_CashFlows()
                {
                    Id = s.Id,
                    IDParent = s.IDParent
                }).ToList();
                ...
                foreach (var item in query)
                {
                    ...
                    query = db.tbl_WH_CashFlows.Where(d => Ids.Contains(d.Id));
                }
        ...
        public static List<int> findParent(List<DTO_WH_CashFlows> allItems, int Id)
        {
        ...
        public static List<int> findChildrent(List<DTO_WH_CashFlows> allItems, int Id)
        {
            var Ids = new List<int>();
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS WH_CashFlows";
		...
		public static DTO_WH_CashFlows getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_WH_CashFlows.Find(id);
		...
		public static DTO_WH_CashFlows getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_WH_CashFlows
        ...
        {
            bool result = false;
            var dbitem = db.tbl_WH_CashFlows.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_WH_CashFlows",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_WH_CashFlows dbitem, string Username)
        {
            Type type = typeof(tbl_WH_CashFlows);
        ...
        public static void patchDTOtoDBValue( DTO_WH_CashFlows item, tbl_WH_CashFlows dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_WH_CashFlows post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_WH_CashFlows dbitem = new tbl_WH_CashFlows();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_WH_CashFlows.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_WH_CashFlows",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_WH_CashFlows();
                    ...
                    {
                        dbitem = new tbl_WH_CashFlows();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_WH_CashFlows.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_WH_CashFlows.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_WH_CashFlows.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_WH_CashFlows", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_WH_CashFlows",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_WH_CashFlows",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_WH_CashFlows.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_WH_CashFlows.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_WH_CashFlows> toDTO(IQueryable<tbl_WH_CashFlows> query)
        {
			return query
			.Select(s => new DTO_WH_CashFlows(){							
				Id = s.Id,							
				IDBranch = s.IDBranch,							
				IDParent = s.IDParent,							
				Level = s.Level,							
		...
		public static DTO_WH_CashFlows toDTO(tbl_WH_CashFlows dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_WH_CashFlows()
				{							
					Id = dbResult.Id,							
					IDBranch = dbResult.IDBranch,							
					IDParent = dbResult.IDParent,							
					Level = dbResult.Level,							
					Drawer = dbResult.Drawer,							
					Name = dbResult.Name,							
					Code = dbResult.Code,							
		...
		public static IQueryable<tbl_WH_CashFlows> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_WH_CashFlows> query = db.tbl_WH_CashFlows.AsNoTracking();//.Where(d => d.IsDeleted == false);
...
//                    
			...
			//Query IDParent (Nullable<short>)

			//Query Level (Nullable<byte>)

			//Query Drawer (Nullable<byte>)
        ...
        public static IQueryable<tbl_WH_CashFlows> _sortBuilder(IQueryable<tbl_WH_CashFlows> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_WH_CashFlows>;
                        ...
                        switch (item)
                        {
							...
							case "Level_desc":
                                ...
                                break;
							case "Drawer":
								query = isOrdered ? ordered.ThenBy(o => o.Drawer) : query.OrderBy(o => o.Drawer);
								 break;
							case "Drawer_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Drawer) : query.OrderByDescending(o => o.Drawer);
                                break;
							case "Name":
        ...
        public static IQueryable<tbl_WH_CashFlows> _pagingBuilder(IQueryable<tbl_WH_CashFlows> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
