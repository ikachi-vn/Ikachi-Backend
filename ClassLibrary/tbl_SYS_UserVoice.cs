...
namespace ClassLibrary
{
    ...
    public partial class tbl_SYS_UserVoice
    {
...
{
	using System;
	public partial class DTO_SYS_UserVoice
	{
		public int IDBranch { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_SYS_UserVoice 
    {
		...
		public static IQueryable<DTO_SYS_UserVoice> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS SYS_UserVoice";
		...
		public static DTO_SYS_UserVoice getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_SYS_UserVoice.Find(id);
		...
		public static DTO_SYS_UserVoice getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_SYS_UserVoice
        ...
        {
            bool result = false;
            var dbitem = db.tbl_SYS_UserVoice.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_SYS_UserVoice",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_SYS_UserVoice dbitem, string Username)
        {
            Type type = typeof(tbl_SYS_UserVoice);
        ...
        public static void patchDTOtoDBValue( DTO_SYS_UserVoice item, tbl_SYS_UserVoice dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.Stars = item.Stars;        }

		public static DTO_SYS_UserVoice post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_SYS_UserVoice dbitem = new tbl_SYS_UserVoice();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_SYS_UserVoice.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_SYS_UserVoice",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_SYS_UserVoice();
                    ...
                    {
                        dbitem = new tbl_SYS_UserVoice();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_SYS_UserVoice.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_SYS_UserVoice.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_SYS_UserVoice.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_SYS_UserVoice", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_SYS_UserVoice",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_SYS_UserVoice",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_SYS_UserVoice.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_SYS_UserVoice.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_SYS_UserVoice> toDTO(IQueryable<tbl_SYS_UserVoice> query)
        {
			return query
			.Select(s => new DTO_SYS_UserVoice(){							
				IDBranch = s.IDBranch,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
		...
		public static DTO_SYS_UserVoice toDTO(tbl_SYS_UserVoice dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_SYS_UserVoice()
				{							
		...
		public static IQueryable<tbl_SYS_UserVoice> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_SYS_UserVoice> query = db.tbl_SYS_UserVoice.AsNoTracking();//.Where(d => d.IsDeleted == false);
			...
			//Query Stars (int)
			if (QueryStrings.Any(d => d.Key == "StarsFrom") && QueryStrings.Any(d => d.Key == "StarsTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "StarsFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "StarsTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.Stars && d.Stars <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "Stars"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "Stars").Value, out int val))
                    query = query.Where(d => val == d.Stars);

            return query;
        }
		
        public static IQueryable<tbl_SYS_UserVoice> _sortBuilder(IQueryable<tbl_SYS_UserVoice> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_SYS_UserVoice>;
                        ...
                        switch (item)
                        {
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "Stars":
								query = isOrdered ? ordered.ThenBy(o => o.Stars) : query.OrderBy(o => o.Stars);
								 break;
							case "Stars_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Stars) : query.OrderByDescending(o => o.Stars);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_SYS_UserVoice> _pagingBuilder(IQueryable<tbl_SYS_UserVoice> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
