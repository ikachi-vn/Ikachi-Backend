...
namespace ClassLibrary
{
    ...
    public partial class tbl_PROD_BillOfMaterialsDetail
    {
        ...
        //List 0:1
        public virtual tbl_PROD_BillOfMaterials tbl_PROD_BillOfMaterials { get; set; }
        //List 0:1
...
{
	using System;
	public partial class DTO_PROD_BillOfMaterialsDetail
	{
		public int IDBOM { get; set; }
		public int Id { get; set; }
		public string Type { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_PROD_BillOfMaterialsDetail 
    {
		...
		public static IQueryable<DTO_PROD_BillOfMaterialsDetail> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS PROD_BillOfMaterialsDetail";
            ...
            if (workBook != null)
            {
                ...
                //var PROD_BillOfMaterialsList = BS_PROD_BillOfMaterials.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_PROD_BillOfMaterialsDetail getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_PROD_BillOfMaterialsDetail.Find(id);
        ...
        {
            bool result = false;
            var dbitem = db.tbl_PROD_BillOfMaterialsDetail.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_PROD_BillOfMaterialsDetail",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_PROD_BillOfMaterialsDetail dbitem, string Username)
        {
            Type type = typeof(tbl_PROD_BillOfMaterialsDetail);
        ...
        public static void patchDTOtoDBValue( DTO_PROD_BillOfMaterialsDetail item, tbl_PROD_BillOfMaterialsDetail dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDBOM = item.IDBOM;							
			dbitem.Type = item.Type;							
			dbitem.IDItem = item.IDItem;							
			dbitem.IDUoM = item.IDUoM;							
			dbitem.Quantity = item.Quantity;							
			dbitem.AdditionalQuantity = item.AdditionalQuantity;							
			dbitem.UoMPrice = item.UoMPrice;							
			dbitem.IssueMethod = item.IssueMethod;							
		...
		public static DTO_PROD_BillOfMaterialsDetail post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_PROD_BillOfMaterialsDetail dbitem = new tbl_PROD_BillOfMaterialsDetail();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_PROD_BillOfMaterialsDetail.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_PROD_BillOfMaterialsDetail",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_PROD_BillOfMaterialsDetail();
                    ...
                    {
                        dbitem = new tbl_PROD_BillOfMaterialsDetail();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_PROD_BillOfMaterialsDetail.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_PROD_BillOfMaterialsDetail.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_PROD_BillOfMaterialsDetail.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_PROD_BillOfMaterialsDetail", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_PROD_BillOfMaterialsDetail",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_PROD_BillOfMaterialsDetail",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_PROD_BillOfMaterialsDetail.Any(e => e.Id == id && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_PROD_BillOfMaterialsDetail> toDTO(IQueryable<tbl_PROD_BillOfMaterialsDetail> query)
        {
			return query
			.Select(s => new DTO_PROD_BillOfMaterialsDetail(){							
				IDBOM = s.IDBOM,							
				Id = s.Id,							
				Type = s.Type,							
				IDItem = s.IDItem,							
				IDUoM = s.IDUoM,							
				Quantity = s.Quantity,							
				AdditionalQuantity = s.AdditionalQuantity,							
				UoMPrice = s.UoMPrice,							
				IssueMethod = s.IssueMethod,							
		...
		public static DTO_PROD_BillOfMaterialsDetail toDTO(tbl_PROD_BillOfMaterialsDetail dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_PROD_BillOfMaterialsDetail()
				{							
					IDBOM = dbResult.IDBOM,							
					Id = dbResult.Id,							
					Type = dbResult.Type,							
					IDItem = dbResult.IDItem,							
					IDUoM = dbResult.IDUoM,							
					Quantity = dbResult.Quantity,							
					AdditionalQuantity = dbResult.AdditionalQuantity,							
					UoMPrice = dbResult.UoMPrice,							
					IssueMethod = dbResult.IssueMethod,							
		...
		public static IQueryable<tbl_PROD_BillOfMaterialsDetail> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_PROD_BillOfMaterialsDetail> query = db.tbl_PROD_BillOfMaterialsDetail.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query IDBOM (int)
			if (QueryStrings.Any(d => d.Key == "IDBOM"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDBOM").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDBOM));
            }
...
//                    
			...
			//Query AdditionalQuantity (decimal)
			if (QueryStrings.Any(d => d.Key == "AdditionalQuantityFrom") && QueryStrings.Any(d => d.Key == "AdditionalQuantityTo"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "AdditionalQuantityFrom").Value, out decimal fromVal) && decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "AdditionalQuantityTo").Value, out decimal toVal))
                    query = query.Where(d => fromVal <= d.AdditionalQuantity && d.AdditionalQuantity <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "AdditionalQuantity"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "AdditionalQuantity").Value, out decimal val))
                    query = query.Where(d => val == d.AdditionalQuantity);
			...
			//Query IssueMethod (string)
			if (QueryStrings.Any(d => d.Key == "IssueMethod_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "IssueMethod_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "IssueMethod_eq").Value;
                query = query.Where(d=>d.IssueMethod == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "IssueMethod") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "IssueMethod").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "IssueMethod").Value;
                query = query.Where(d=>d.IssueMethod.Contains(keyword));
            }
...
//                    
        ...
        public static IQueryable<tbl_PROD_BillOfMaterialsDetail> _sortBuilder(IQueryable<tbl_PROD_BillOfMaterialsDetail> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_PROD_BillOfMaterialsDetail>;
                        bool isOrdered = ordered.ToString().IndexOf("ORDER BY") !=-1;

                        switch (item)
                        {
							case "IDBOM":
								query = isOrdered ? ordered.ThenBy(o => o.IDBOM) : query.OrderBy(o => o.IDBOM);
								 break;
							case "IDBOM_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDBOM) : query.OrderByDescending(o => o.IDBOM);
                                break;
							case "Id":
							...
							case "AdditionalQuantity":
								query = isOrdered ? ordered.ThenBy(o => o.AdditionalQuantity) : query.OrderBy(o => o.AdditionalQuantity);
								 break;
							case "AdditionalQuantity_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.AdditionalQuantity) : query.OrderByDescending(o => o.AdditionalQuantity);
                                break;
							case "UoMPrice":
							...
							case "UoMPrice_desc":
                                ...
                                break;
							case "IssueMethod":
								query = isOrdered ? ordered.ThenBy(o => o.IssueMethod) : query.OrderBy(o => o.IssueMethod);
								 break;
							case "IssueMethod_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IssueMethod) : query.OrderByDescending(o => o.IssueMethod);
                                break;
							case "IDWarehouse":
        ...
        public static IQueryable<tbl_PROD_BillOfMaterialsDetail> _pagingBuilder(IQueryable<tbl_PROD_BillOfMaterialsDetail> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
