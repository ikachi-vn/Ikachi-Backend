...
namespace ClassLibrary
{
    ...
    public partial class tbl_BANK_IncomingPayment
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_BANK_IncomingPayment()
        {
            this.tbl_BANK_IncomingPaymentDetail = new HashSet<tbl_BANK_IncomingPaymentDetail>();
        }
    
        public Nullable<int> IDJournalEntry { get; set; }
...
{
	using System;
	public partial class DTO_BANK_IncomingPayment
	{
		public Nullable<int> IDJournalEntry { get; set; }
		public Nullable<int> IDBranch { get; set; }
		public Nullable<int> IDStaff { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_BANK_IncomingPayment 
    {
		...
		public static IQueryable<DTO_BANK_IncomingPayment> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS BANK_IncomingPayment";
		...
		public static DTO_BANK_IncomingPayment getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_BANK_IncomingPayment.Find(id);
		...
		public static DTO_BANK_IncomingPayment getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_BANK_IncomingPayment
        ...
        {
            bool result = false;
            var dbitem = db.tbl_BANK_IncomingPayment.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_BANK_IncomingPayment",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_BANK_IncomingPayment dbitem, string Username)
        {
            Type type = typeof(tbl_BANK_IncomingPayment);
        ...
        public static void patchDTOtoDBValue( DTO_BANK_IncomingPayment item, tbl_BANK_IncomingPayment dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDJournalEntry = item.IDJournalEntry;							
			dbitem.IDBranch = item.IDBranch;							
			dbitem.IDStaff = item.IDStaff;							
			dbitem.IDCustomer = item.IDCustomer;							
			...
			dbitem.IsPrinted = item.IsPrinted;							
			dbitem.PostingDate = item.PostingDate;							
			dbitem.DueDate = item.DueDate;							
			dbitem.DocumentDate = item.DocumentDate;        }

		public static DTO_BANK_IncomingPayment post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_BANK_IncomingPayment dbitem = new tbl_BANK_IncomingPayment();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_BANK_IncomingPayment.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_BANK_IncomingPayment",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_BANK_IncomingPayment();
                    ...
                    {
                        dbitem = new tbl_BANK_IncomingPayment();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_BANK_IncomingPayment.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_BANK_IncomingPayment.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_BANK_IncomingPayment.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_BANK_IncomingPayment", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_BANK_IncomingPayment",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_BANK_IncomingPayment",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_BANK_IncomingPayment.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_BANK_IncomingPayment.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_BANK_IncomingPayment> toDTO(IQueryable<tbl_BANK_IncomingPayment> query)
        {
			return query
			.Select(s => new DTO_BANK_IncomingPayment(){							
				IDJournalEntry = s.IDJournalEntry,							
				IDBranch = s.IDBranch,							
				IDStaff = s.IDStaff,							
		...
		public static DTO_BANK_IncomingPayment toDTO(tbl_BANK_IncomingPayment dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_BANK_IncomingPayment()
				{							
					IDJournalEntry = dbResult.IDJournalEntry,							
					IDBranch = dbResult.IDBranch,							
					IDStaff = dbResult.IDStaff,							
					IDCustomer = dbResult.IDCustomer,							
					...
					IsPrinted = dbResult.IsPrinted,							
					PostingDate = dbResult.PostingDate,							
		...
		public static IQueryable<tbl_BANK_IncomingPayment> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_BANK_IncomingPayment> query = db.tbl_BANK_IncomingPayment.AsNoTracking();//.Where(d => d.IsDeleted == false);
			...
			//Query IDJournalEntry (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDJournalEntry"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDJournalEntry").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDJournalEntry));
////                    query = query.Where(d => IDs.Contains(d.IDJournalEntry));
//                    
            }
...
//                    
			...
			//Query IDCustomer (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDCustomer"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDCustomer").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDCustomer));
////                    query = query.Where(d => IDs.Contains(d.IDCustomer));
//                    
            }
			...
			//Query IsCanceled (bool)
			if (QueryStrings.Any(d => d.Key == "IsCanceled"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "IsCanceled").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.IsCanceled);
            }

			//Query IsPrinted (bool)
			if (QueryStrings.Any(d => d.Key == "IsPrinted"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "IsPrinted").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.IsPrinted);
            }

			//Query PostingDate (System.DateTime)
			if (QueryStrings.Any(d => d.Key == "PostingDateFrom") && QueryStrings.Any(d => d.Key == "PostingDateTo"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "PostingDateFrom").Value, out DateTime fromDate) && DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "PostingDateTo").Value, out DateTime toDate))
                    query = query.Where(d => fromDate <= d.PostingDate && d.PostingDate <= toDate);

            if (QueryStrings.Any(d => d.Key == "PostingDate"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "PostingDate").Value, out DateTime val))
                    query = query.Where(d => d.PostingDate != null && DbFunctions.TruncateTime(val) == DbFunctions.TruncateTime(d.PostingDate));
			...
			//Query DocumentDate (System.DateTime)
        ...
        public static IQueryable<tbl_BANK_IncomingPayment> _sortBuilder(IQueryable<tbl_BANK_IncomingPayment> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_BANK_IncomingPayment>;
                        bool isOrdered = ordered.ToString().IndexOf("ORDER BY") !=-1;

                        switch (item)
                        {
							case "IDJournalEntry":
								query = isOrdered ? ordered.ThenBy(o => o.IDJournalEntry) : query.OrderBy(o => o.IDJournalEntry);
								 break;
							case "IDJournalEntry_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDJournalEntry) : query.OrderByDescending(o => o.IDJournalEntry);
                                break;
							case "IDBranch":
							...
							case "IDStaff_desc":
                                ...
                                break;
							case "IDCustomer":
								query = isOrdered ? ordered.ThenBy(o => o.IDCustomer) : query.OrderBy(o => o.IDCustomer);
								 break;
							case "IDCustomer_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDCustomer) : query.OrderByDescending(o => o.IDCustomer);
                                break;
							case "IDType":
							...
							case "IsCanceled_desc":
                                ...
                                break;
							case "IsPrinted":
								query = isOrdered ? ordered.ThenBy(o => o.IsPrinted) : query.OrderBy(o => o.IsPrinted);
								 break;
							case "IsPrinted_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IsPrinted) : query.OrderByDescending(o => o.IsPrinted);
                                break;
							case "PostingDate":
								query = isOrdered ? ordered.ThenBy(o => o.PostingDate) : query.OrderBy(o => o.PostingDate);
								 break;
							case "PostingDate_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.PostingDate) : query.OrderByDescending(o => o.PostingDate);
                                break;
							case "DueDate":
        ...
        public static IQueryable<tbl_BANK_IncomingPayment> _pagingBuilder(IQueryable<tbl_BANK_IncomingPayment> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
