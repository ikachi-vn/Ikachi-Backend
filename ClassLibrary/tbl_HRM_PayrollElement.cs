...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_PayrollElement
    {
        ...
        public Nullable<int> IDElementOfSalary { get; set; }
        public Nullable<decimal> ValueOfElement { get; set; }
        //List 0:1
        public virtual tbl_LIST_ElementOfSalary tbl_LIST_ElementOfSalary { get; set; }
        //List 0:1
...
{
	using System;
	public partial class DTO_HRM_PayrollElement
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_PayrollElement 
    {
		...
		public static IQueryable<DTO_HRM_PayrollElement> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_PayrollElement";
            ...
            if (workBook != null)
            {
                ...
                //var LIST_ElementOfSalaryList = BS_LIST_ElementOfSalary.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_HRM_PayrollElement getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_PayrollElement.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_PayrollElement getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_PayrollElement
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_PayrollElement.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_PayrollElement",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_PayrollElement dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_PayrollElement);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_PayrollElement item, tbl_HRM_PayrollElement dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDElementOfSalary = item.IDElementOfSalary;							
			dbitem.ValueOfElement = item.ValueOfElement;        }

		public static DTO_HRM_PayrollElement post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_PayrollElement dbitem = new tbl_HRM_PayrollElement();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_PayrollElement.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_PayrollElement",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_PayrollElement();
                    ...
                    {
                        dbitem = new tbl_HRM_PayrollElement();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_PayrollElement.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_PayrollElement.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_PayrollElement.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_PayrollElement", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_PayrollElement",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_PayrollElement",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_PayrollElement.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_PayrollElement.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_PayrollElement> toDTO(IQueryable<tbl_HRM_PayrollElement> query)
        {
			return query
			.Select(s => new DTO_HRM_PayrollElement(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				IDElementOfSalary = s.IDElementOfSalary,							
				ValueOfElement = s.ValueOfElement,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_HRM_PayrollElement toDTO(tbl_HRM_PayrollElement dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_PayrollElement()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IDElementOfSalary = dbResult.IDElementOfSalary,							
					ValueOfElement = dbResult.ValueOfElement,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_HRM_PayrollElement> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_PayrollElement> query = db.tbl_HRM_PayrollElement.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query IDElementOfSalary (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDElementOfSalary"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDElementOfSalary").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDElementOfSalary));
////                    query = query.Where(d => IDs.Contains(d.IDElementOfSalary));
//                    
            }

			//Query ValueOfElement (Nullable<decimal>)
			if (QueryStrings.Any(d => d.Key == "ValueOfElementFrom") && QueryStrings.Any(d => d.Key == "ValueOfElementTo"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ValueOfElementFrom").Value, out decimal fromVal) && decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ValueOfElementTo").Value, out decimal toVal))
                    query = query.Where(d => fromVal <= d.ValueOfElement && d.ValueOfElement <= toVal);
            return query;
        }
		
        public static IQueryable<tbl_HRM_PayrollElement> _sortBuilder(IQueryable<tbl_HRM_PayrollElement> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_PayrollElement>;
                        ...
                        switch (item)
                        {
							...
							case "IDPaySheetMaster_desc":
                                ...
                                break;
							case "IDElementOfSalary":
								query = isOrdered ? ordered.ThenBy(o => o.IDElementOfSalary) : query.OrderBy(o => o.IDElementOfSalary);
								 break;
							case "IDElementOfSalary_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDElementOfSalary) : query.OrderByDescending(o => o.IDElementOfSalary);
                                break;
							case "ValueOfElement":
								query = isOrdered ? ordered.ThenBy(o => o.ValueOfElement) : query.OrderBy(o => o.ValueOfElement);
								 break;
							case "ValueOfElement_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.ValueOfElement) : query.OrderByDescending(o => o.ValueOfElement);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_PayrollElement> _pagingBuilder(IQueryable<tbl_HRM_PayrollElement> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
