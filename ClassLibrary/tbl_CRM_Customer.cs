...
namespace ClassLibrary
{
    ...
    public partial class tbl_CRM_Customer
    {
...
{
	using System;
	public partial class DTO_CRM_Customer
	{
		public int IDContact { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_CRM_Customer 
    {
		...
		public static IQueryable<DTO_CRM_Customer> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS CRM_Customer";
		...
		public static DTO_CRM_Customer getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_CRM_Customer.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_CRM_Customer getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_CRM_Customer
        ...
        {
            bool result = false;
            var dbitem = db.tbl_CRM_Customer.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_CRM_Customer",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_CRM_Customer dbitem, string Username)
        {
            Type type = typeof(tbl_CRM_Customer);
        ...
        public static void patchDTOtoDBValue( DTO_CRM_Customer item, tbl_CRM_Customer dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_CRM_Customer post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_CRM_Customer dbitem = new tbl_CRM_Customer();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_CRM_Customer.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_CRM_Customer",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_CRM_Customer();
                    ...
                    {
                        dbitem = new tbl_CRM_Customer();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_CRM_Customer.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_CRM_Customer.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_CRM_Customer.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_CRM_Customer", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_CRM_Customer",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_CRM_Customer",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_CRM_Customer.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_CRM_Customer.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_CRM_Customer> toDTO(IQueryable<tbl_CRM_Customer> query)
        {
			return query
			.Select(s => new DTO_CRM_Customer(){							
		...
		public static DTO_CRM_Customer toDTO(tbl_CRM_Customer dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_CRM_Customer()
				{							
		...
		public static IQueryable<tbl_CRM_Customer> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_CRM_Customer> query = db.tbl_CRM_Customer.AsNoTracking();//.Where(d => d.IsDeleted == false );
        ...
        public static IQueryable<tbl_CRM_Customer> _sortBuilder(IQueryable<tbl_CRM_Customer> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_CRM_Customer>;
        ...
        public static IQueryable<tbl_CRM_Customer> _pagingBuilder(IQueryable<tbl_CRM_Customer> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
