...
namespace ClassLibrary
{
    ...
    public partial class tbl_WMS_UoMGroup
    {
...
{
	using System;
	public partial class DTO_WMS_UoMGroup
	{
		public int IDBranch { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_WMS_UoMGroup 
    {
		...
		public static IQueryable<DTO_WMS_UoMGroup> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS WMS_UoMGroup";
		...
		public static DTO_WMS_UoMGroup getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_WMS_UoMGroup.Find(id);
		...
		public static DTO_WMS_UoMGroup getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_WMS_UoMGroup
        ...
        {
            bool result = false;
            var dbitem = db.tbl_WMS_UoMGroup.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_WMS_UoMGroup",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_WMS_UoMGroup dbitem, string Username)
        {
            Type type = typeof(tbl_WMS_UoMGroup);
        ...
        public static void patchDTOtoDBValue( DTO_WMS_UoMGroup item, tbl_WMS_UoMGroup dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_WMS_UoMGroup post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_WMS_UoMGroup dbitem = new tbl_WMS_UoMGroup();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_WMS_UoMGroup.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_WMS_UoMGroup",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_WMS_UoMGroup();
                    ...
                    {
                        dbitem = new tbl_WMS_UoMGroup();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_WMS_UoMGroup.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_WMS_UoMGroup.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_WMS_UoMGroup.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_WMS_UoMGroup", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_WMS_UoMGroup",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_WMS_UoMGroup",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_WMS_UoMGroup.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_WMS_UoMGroup.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_WMS_UoMGroup> toDTO(IQueryable<tbl_WMS_UoMGroup> query)
        {
			return query
			.Select(s => new DTO_WMS_UoMGroup(){							
				IDBranch = s.IDBranch,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
		...
		public static DTO_WMS_UoMGroup toDTO(tbl_WMS_UoMGroup dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_WMS_UoMGroup()
				{							
		...
		public static IQueryable<tbl_WMS_UoMGroup> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_WMS_UoMGroup> query = db.tbl_WMS_UoMGroup.AsNoTracking();//.Where(d => d.IsDeleted == false);
        ...
        public static IQueryable<tbl_WMS_UoMGroup> _sortBuilder(IQueryable<tbl_WMS_UoMGroup> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_WMS_UoMGroup>;
        ...
        public static IQueryable<tbl_WMS_UoMGroup> _pagingBuilder(IQueryable<tbl_WMS_UoMGroup> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
