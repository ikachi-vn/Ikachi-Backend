...
namespace ClassLibrary
{
    ...
    public partial class tbl_FINANCE_GeneralLedger
    {
        ...
        public bool IsCapitalAccount { get; set; }
    }
}
...
{
	using System;
	public partial class DTO_FINANCE_GeneralLedger
	{
		public Nullable<int> IDBranch { get; set; }
		public Nullable<int> IDParent { get; set; }
		public string RefParentCode { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_FINANCE_GeneralLedger 
    {
		...
		public static IQueryable<DTO_FINANCE_GeneralLedger> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            ...
            if (QueryStrings.Any(d => d.Key == "AllChildren")  || QueryStrings.Any(d => d.Key == "AllParent"))
            {
                ...
                List<DTO_FINANCE_GeneralLedger> allItems = db.tbl_FINANCE_GeneralLedger.Where(d => d.IsDeleted == false).Select(s => new DTO_FINANCE_GeneralLedger()
                {
                    Id = s.Id,
                    IDParent = s.IDParent
                }).ToList();
                ...
                foreach (var item in query)
                {
                    ...
                    query = db.tbl_FINANCE_GeneralLedger.Where(d => Ids.Contains(d.Id));
                }
        ...
        public static List<int> findParent(List<DTO_FINANCE_GeneralLedger> allItems, int Id)
        {
        ...
        public static List<int> findChildrent(List<DTO_FINANCE_GeneralLedger> allItems, int Id)
        {
            var Ids = new List<int>();
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS FINANCE_GeneralLedger";
		...
		public static DTO_FINANCE_GeneralLedger getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_FINANCE_GeneralLedger.Find(id);
		...
		public static DTO_FINANCE_GeneralLedger getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_FINANCE_GeneralLedger
        ...
        {
            bool result = false;
            var dbitem = db.tbl_FINANCE_GeneralLedger.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_FINANCE_GeneralLedger",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_FINANCE_GeneralLedger dbitem, string Username)
        {
            Type type = typeof(tbl_FINANCE_GeneralLedger);
        ...
        public static void patchDTOtoDBValue( DTO_FINANCE_GeneralLedger item, tbl_FINANCE_GeneralLedger dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.ExternalCode = item.ExternalCode;							
			dbitem.IsActiveAccount = item.IsActiveAccount;							
			dbitem.CurrentBalance = item.CurrentBalance;							
			dbitem.OpeningBalance = item.OpeningBalance;							
			dbitem.IsTaxIncome = item.IsTaxIncome;							
			dbitem.IsControlAccount = item.IsControlAccount;							
			...
			dbitem.IsCapitalAccount = item.IsCapitalAccount;        }

		public static DTO_FINANCE_GeneralLedger post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_FINANCE_GeneralLedger dbitem = new tbl_FINANCE_GeneralLedger();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_FINANCE_GeneralLedger.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_FINANCE_GeneralLedger",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_FINANCE_GeneralLedger();
                    ...
                    {
                        dbitem = new tbl_FINANCE_GeneralLedger();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_FINANCE_GeneralLedger.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_FINANCE_GeneralLedger.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_FINANCE_GeneralLedger.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_FINANCE_GeneralLedger", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_FINANCE_GeneralLedger",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_FINANCE_GeneralLedger",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_FINANCE_GeneralLedger.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_FINANCE_GeneralLedger.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_FINANCE_GeneralLedger> toDTO(IQueryable<tbl_FINANCE_GeneralLedger> query)
        {
			return query
			.Select(s => new DTO_FINANCE_GeneralLedger(){							
				IDBranch = s.IDBranch,							
				IDParent = s.IDParent,							
				...
				IsActiveAccount = s.IsActiveAccount,							
				CurrentBalance = s.CurrentBalance,							
				OpeningBalance = s.OpeningBalance,							
				IsTaxIncome = s.IsTaxIncome,							
				IsControlAccount = s.IsControlAccount,							
				...
				IsCapitalAccount = s.IsCapitalAccount,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_FINANCE_GeneralLedger toDTO(tbl_FINANCE_GeneralLedger dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_FINANCE_GeneralLedger()
				{							
					...
					ExternalCode = dbResult.ExternalCode,							
					IsActiveAccount = dbResult.IsActiveAccount,							
					CurrentBalance = dbResult.CurrentBalance,							
					OpeningBalance = dbResult.OpeningBalance,							
					IsTaxIncome = dbResult.IsTaxIncome,							
					IsControlAccount = dbResult.IsControlAccount,							
					...
					IsCapitalAccount = dbResult.IsCapitalAccount,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_FINANCE_GeneralLedger> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_FINANCE_GeneralLedger> query = db.tbl_FINANCE_GeneralLedger.AsNoTracking();//.Where(d => d.IsDeleted == false);
...
//                    
			...
			if (QueryStrings.Any(d => d.Key == "ExternalCode_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "ExternalCode_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "ExternalCode_eq").Value;
                query = query.Where(d=>d.ExternalCode == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "ExternalCode") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "ExternalCode").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "ExternalCode").Value;
                query = query.Where(d=>d.ExternalCode.Contains(keyword));
            }
            

			//Query IsActiveAccount (bool)
			if (QueryStrings.Any(d => d.Key == "IsActiveAccount"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "IsActiveAccount").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.IsActiveAccount);
            }
			...
			//Query IsControlAccount (bool)
			if (QueryStrings.Any(d => d.Key == "IsControlAccount"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "IsControlAccount").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.IsControlAccount);
            }
			...
			//Query IsCapitalAccount (bool)
			if (QueryStrings.Any(d => d.Key == "IsCapitalAccount"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "IsCapitalAccount").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.IsCapitalAccount);
            }
            return query;
        }
		
        public static IQueryable<tbl_FINANCE_GeneralLedger> _sortBuilder(IQueryable<tbl_FINANCE_GeneralLedger> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_FINANCE_GeneralLedger>;
                        ...
                        switch (item)
                        {
							...
							case "ForeignRemark_desc":
                                ...
                                break;
							case "ExternalCode":
								query = isOrdered ? ordered.ThenBy(o => o.ExternalCode) : query.OrderBy(o => o.ExternalCode);
								 break;
							case "ExternalCode_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.ExternalCode) : query.OrderByDescending(o => o.ExternalCode);
                                break;
							case "IsActiveAccount":
								query = isOrdered ? ordered.ThenBy(o => o.IsActiveAccount) : query.OrderBy(o => o.IsActiveAccount);
								 break;
							case "IsActiveAccount_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IsActiveAccount) : query.OrderByDescending(o => o.IsActiveAccount);
                                break;
							case "CurrentBalance":
							...
							case "IsControlAccount":
								query = isOrdered ? ordered.ThenBy(o => o.IsControlAccount) : query.OrderBy(o => o.IsControlAccount);
								 break;
							case "IsControlAccount_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IsControlAccount) : query.OrderByDescending(o => o.IsControlAccount);
                                break;
							case "IsCashAccount":
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "IsCapitalAccount":
								query = isOrdered ? ordered.ThenBy(o => o.IsCapitalAccount) : query.OrderBy(o => o.IsCapitalAccount);
								 break;
							case "IsCapitalAccount_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IsCapitalAccount) : query.OrderByDescending(o => o.IsCapitalAccount);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_FINANCE_GeneralLedger> _pagingBuilder(IQueryable<tbl_FINANCE_GeneralLedger> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
