...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffWorkingDiary
    {
        ...
        public Nullable<int> IDRoleInEvent { get; set; }
...
{
	using System;
	public partial class DTO_HRM_StaffWorkingDiary
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffWorkingDiary 
    {
		...
		public static IQueryable<DTO_HRM_StaffWorkingDiary> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffWorkingDiary";
		...
		public static DTO_HRM_StaffWorkingDiary getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffWorkingDiary.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffWorkingDiary getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffWorkingDiary
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffWorkingDiary.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffWorkingDiary",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffWorkingDiary dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffWorkingDiary);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffWorkingDiary item, tbl_HRM_StaffWorkingDiary dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDEventType = item.IDEventType;							
			dbitem.EventName = item.EventName;							
			dbitem.IDRoleInEvent = item.IDRoleInEvent;							
			dbitem.RoleDescription = item.RoleDescription;							
			dbitem.CustomerName = item.CustomerName;							
			dbitem.StartTime = item.StartTime;							
			dbitem.EndTime = item.EndTime;        }

		public static DTO_HRM_StaffWorkingDiary post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffWorkingDiary dbitem = new tbl_HRM_StaffWorkingDiary();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffWorkingDiary.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffWorkingDiary",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffWorkingDiary();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffWorkingDiary();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffWorkingDiary.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffWorkingDiary.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffWorkingDiary.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffWorkingDiary", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffWorkingDiary",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_StaffWorkingDiary",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffWorkingDiary.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffWorkingDiary.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffWorkingDiary> toDTO(IQueryable<tbl_HRM_StaffWorkingDiary> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffWorkingDiary(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				IDStaff = s.IDStaff,							
				IDEventType = s.IDEventType,							
				EventName = s.EventName,							
				IDRoleInEvent = s.IDRoleInEvent,							
		...
		public static DTO_HRM_StaffWorkingDiary toDTO(tbl_HRM_StaffWorkingDiary dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffWorkingDiary()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IDEventType = dbResult.IDEventType,							
					EventName = dbResult.EventName,							
					IDRoleInEvent = dbResult.IDRoleInEvent,							
					RoleDescription = dbResult.RoleDescription,							
		...
		public static IQueryable<tbl_HRM_StaffWorkingDiary> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffWorkingDiary> query = db.tbl_HRM_StaffWorkingDiary.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query IDEventType (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDEventType"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDEventType").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDEventType));
////                    query = query.Where(d => IDs.Contains(d.IDEventType));
//                    
            }

			//Query EventName (string)
			if (QueryStrings.Any(d => d.Key == "EventName_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "EventName_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "EventName_eq").Value;
                query = query.Where(d=>d.EventName == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "EventName") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "EventName").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "EventName").Value;
                query = query.Where(d=>d.EventName.Contains(keyword));
            }
            

			//Query IDRoleInEvent (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDRoleInEvent"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDRoleInEvent").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDRoleInEvent));
////                    query = query.Where(d => IDs.Contains(d.IDRoleInEvent));
//                    
            }

			//Query RoleDescription (string)
			if (QueryStrings.Any(d => d.Key == "RoleDescription_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "RoleDescription_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "RoleDescription_eq").Value;
                query = query.Where(d=>d.RoleDescription == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "RoleDescription") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "RoleDescription").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "RoleDescription").Value;
                query = query.Where(d=>d.RoleDescription.Contains(keyword));
            }
			...
			//Query StartTime (Nullable<System.DateTime>)
			if (QueryStrings.Any(d => d.Key == "StartTimeFrom") && QueryStrings.Any(d => d.Key == "StartTimeTo"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "StartTimeFrom").Value, out DateTime fromDate) && DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "StartTimeTo").Value, out DateTime toDate))
                    query = query.Where(d => fromDate <= d.StartTime && d.StartTime <= toDate);

            if (QueryStrings.Any(d => d.Key == "StartTime"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "StartTime").Value, out DateTime val))
                    query = query.Where(d => d.StartTime != null && DbFunctions.TruncateTime(val) == DbFunctions.TruncateTime(d.StartTime));


			//Query EndTime (Nullable<System.DateTime>)
			if (QueryStrings.Any(d => d.Key == "EndTimeFrom") && QueryStrings.Any(d => d.Key == "EndTimeTo"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "EndTimeFrom").Value, out DateTime fromDate) && DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "EndTimeTo").Value, out DateTime toDate))
                    query = query.Where(d => fromDate <= d.EndTime && d.EndTime <= toDate);

            if (QueryStrings.Any(d => d.Key == "EndTime"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "EndTime").Value, out DateTime val))
                    query = query.Where(d => d.EndTime != null && DbFunctions.TruncateTime(val) == DbFunctions.TruncateTime(d.EndTime));

            return query;
        }
		
        public static IQueryable<tbl_HRM_StaffWorkingDiary> _sortBuilder(IQueryable<tbl_HRM_StaffWorkingDiary> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffWorkingDiary>;
                        ...
                        switch (item)
                        {
							...
							case "IDStaff_desc":
                                ...
                                break;
							case "IDEventType":
								query = isOrdered ? ordered.ThenBy(o => o.IDEventType) : query.OrderBy(o => o.IDEventType);
								 break;
							case "IDEventType_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDEventType) : query.OrderByDescending(o => o.IDEventType);
                                break;
							case "EventName":
								query = isOrdered ? ordered.ThenBy(o => o.EventName) : query.OrderBy(o => o.EventName);
								 break;
							case "EventName_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.EventName) : query.OrderByDescending(o => o.EventName);
                                break;
							case "IDRoleInEvent":
								query = isOrdered ? ordered.ThenBy(o => o.IDRoleInEvent) : query.OrderBy(o => o.IDRoleInEvent);
								 break;
							case "IDRoleInEvent_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDRoleInEvent) : query.OrderByDescending(o => o.IDRoleInEvent);
                                break;
							case "RoleDescription":
								query = isOrdered ? ordered.ThenBy(o => o.RoleDescription) : query.OrderBy(o => o.RoleDescription);
								 break;
							case "RoleDescription_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.RoleDescription) : query.OrderByDescending(o => o.RoleDescription);
                                break;
							case "CustomerName":
							...
							case "CustomerName_desc":
                                ...
                                break;
							case "StartTime":
								query = isOrdered ? ordered.ThenBy(o => o.StartTime) : query.OrderBy(o => o.StartTime);
								 break;
							case "StartTime_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.StartTime) : query.OrderByDescending(o => o.StartTime);
                                break;
							case "EndTime":
								query = isOrdered ? ordered.ThenBy(o => o.EndTime) : query.OrderBy(o => o.EndTime);
								 break;
							case "EndTime_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.EndTime) : query.OrderByDescending(o => o.EndTime);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_StaffWorkingDiary> _pagingBuilder(IQueryable<tbl_HRM_StaffWorkingDiary> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
