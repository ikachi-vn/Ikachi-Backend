...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffBank
    {
        ...
        public Nullable<int> IDBankBranch { get; set; }
...
{
	using System;
	public partial class DTO_HRM_StaffBank
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffBank 
    {
		...
		public static IQueryable<DTO_HRM_StaffBank> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffBank";
		...
		public static DTO_HRM_StaffBank getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffBank.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffBank getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffBank
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffBank.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffBank",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffBank dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffBank);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffBank item, tbl_HRM_StaffBank dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDBank = item.IDBank;							
			dbitem.IDBankBranch = item.IDBankBranch;							
			dbitem.AccountNumber = item.AccountNumber;							
			dbitem.IsPayrollAccount = item.IsPayrollAccount;        }

		public static DTO_HRM_StaffBank post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffBank dbitem = new tbl_HRM_StaffBank();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffBank.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffBank",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffBank();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffBank();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffBank.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffBank.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffBank.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffBank", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffBank",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_StaffBank",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffBank.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffBank.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffBank> toDTO(IQueryable<tbl_HRM_StaffBank> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffBank(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				IDStaff = s.IDStaff,							
				IDBank = s.IDBank,							
				IDBankBranch = s.IDBankBranch,							
				AccountNumber = s.AccountNumber,							
				IsPayrollAccount = s.IsPayrollAccount,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_HRM_StaffBank toDTO(tbl_HRM_StaffBank dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffBank()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IDBank = dbResult.IDBank,							
					IDBankBranch = dbResult.IDBankBranch,							
					AccountNumber = dbResult.AccountNumber,							
					IsPayrollAccount = dbResult.IsPayrollAccount,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_HRM_StaffBank> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffBank> query = db.tbl_HRM_StaffBank.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query IDBank (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDBank"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDBank").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDBank));
////                    query = query.Where(d => IDs.Contains(d.IDBank));
//                    
            }

			//Query IDBankBranch (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDBankBranch"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDBankBranch").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDBankBranch));
////                    query = query.Where(d => IDs.Contains(d.IDBankBranch));
//                    
            }

			//Query AccountNumber (string)
			if (QueryStrings.Any(d => d.Key == "AccountNumber_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "AccountNumber_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "AccountNumber_eq").Value;
                query = query.Where(d=>d.AccountNumber == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "AccountNumber") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "AccountNumber").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "AccountNumber").Value;
                query = query.Where(d=>d.AccountNumber.Contains(keyword));
            }
            

			//Query IsPayrollAccount (bool)
			if (QueryStrings.Any(d => d.Key == "IsPayrollAccount"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "IsPayrollAccount").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.IsPayrollAccount);
            }
            return query;
        }
		
        public static IQueryable<tbl_HRM_StaffBank> _sortBuilder(IQueryable<tbl_HRM_StaffBank> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffBank>;
                        ...
                        switch (item)
                        {
							...
							case "IDStaff_desc":
                                ...
                                break;
							case "IDBank":
								query = isOrdered ? ordered.ThenBy(o => o.IDBank) : query.OrderBy(o => o.IDBank);
								 break;
							case "IDBank_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDBank) : query.OrderByDescending(o => o.IDBank);
                                break;
							case "IDBankBranch":
								query = isOrdered ? ordered.ThenBy(o => o.IDBankBranch) : query.OrderBy(o => o.IDBankBranch);
								 break;
							case "IDBankBranch_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDBankBranch) : query.OrderByDescending(o => o.IDBankBranch);
                                break;
							case "AccountNumber":
								query = isOrdered ? ordered.ThenBy(o => o.AccountNumber) : query.OrderBy(o => o.AccountNumber);
								 break;
							case "AccountNumber_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.AccountNumber) : query.OrderByDescending(o => o.AccountNumber);
                                break;
							case "IsPayrollAccount":
								query = isOrdered ? ordered.ThenBy(o => o.IsPayrollAccount) : query.OrderBy(o => o.IsPayrollAccount);
								 break;
							case "IsPayrollAccount_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IsPayrollAccount) : query.OrderByDescending(o => o.IsPayrollAccount);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_StaffBank> _pagingBuilder(IQueryable<tbl_HRM_StaffBank> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
