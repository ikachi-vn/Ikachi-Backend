...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffConcurrentProbationryPosition
    {
        ...
        public Nullable<int> IDConcurrentProbationryJobTitle { get; set; }
        public Nullable<int> IDConcurrentProbationryDepartment { get; set; }
        //List 0:1
...
{
	using System;
	public partial class DTO_HRM_StaffConcurrentProbationryPosition
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffConcurrentProbationryPosition 
    {
		...
		public static IQueryable<DTO_HRM_StaffConcurrentProbationryPosition> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffConcurrentProbationryPosition";
		...
		public static DTO_HRM_StaffConcurrentProbationryPosition getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffConcurrentProbationryPosition.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffConcurrentProbationryPosition getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffConcurrentProbationryPosition
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffConcurrentProbationryPosition.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffConcurrentProbationryPosition",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffConcurrentProbationryPosition dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffConcurrentProbationryPosition);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffConcurrentProbationryPosition item, tbl_HRM_StaffConcurrentProbationryPosition dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDConcurrentProbationryJobTitle = item.IDConcurrentProbationryJobTitle;							
			dbitem.IDConcurrentProbationryDepartment = item.IDConcurrentProbationryDepartment;        }

		public static DTO_HRM_StaffConcurrentProbationryPosition post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffConcurrentProbationryPosition dbitem = new tbl_HRM_StaffConcurrentProbationryPosition();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffConcurrentProbationryPosition.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffConcurrentProbationryPosition",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffConcurrentProbationryPosition();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffConcurrentProbationryPosition();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffConcurrentProbationryPosition.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffConcurrentProbationryPosition.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffConcurrentProbationryPosition.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffConcurrentProbationryPosition", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffConcurrentProbationryPosition",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_StaffConcurrentProbationryPosition",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffConcurrentProbationryPosition.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffConcurrentProbationryPosition.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffConcurrentProbationryPosition> toDTO(IQueryable<tbl_HRM_StaffConcurrentProbationryPosition> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffConcurrentProbationryPosition(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				IDStaff = s.IDStaff,							
				IDConcurrentProbationryJobTitle = s.IDConcurrentProbationryJobTitle,							
				IDConcurrentProbationryDepartment = s.IDConcurrentProbationryDepartment,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_HRM_StaffConcurrentProbationryPosition toDTO(tbl_HRM_StaffConcurrentProbationryPosition dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffConcurrentProbationryPosition()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IDConcurrentProbationryJobTitle = dbResult.IDConcurrentProbationryJobTitle,							
					IDConcurrentProbationryDepartment = dbResult.IDConcurrentProbationryDepartment,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_HRM_StaffConcurrentProbationryPosition> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffConcurrentProbationryPosition> query = db.tbl_HRM_StaffConcurrentProbationryPosition.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query IDConcurrentProbationryJobTitle (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDConcurrentProbationryJobTitle"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDConcurrentProbationryJobTitle").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDConcurrentProbationryJobTitle));
////                    query = query.Where(d => IDs.Contains(d.IDConcurrentProbationryJobTitle));
//                    
            }

			//Query IDConcurrentProbationryDepartment (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDConcurrentProbationryDepartment"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDConcurrentProbationryDepartment").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDConcurrentProbationryDepartment));
////                    query = query.Where(d => IDs.Contains(d.IDConcurrentProbationryDepartment));
//                    
            }
            return query;
        ...
        public static IQueryable<tbl_HRM_StaffConcurrentProbationryPosition> _sortBuilder(IQueryable<tbl_HRM_StaffConcurrentProbationryPosition> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffConcurrentProbationryPosition>;
                        ...
                        switch (item)
                        {
							...
							case "IDStaff_desc":
                                ...
                                break;
							case "IDConcurrentProbationryJobTitle":
								query = isOrdered ? ordered.ThenBy(o => o.IDConcurrentProbationryJobTitle) : query.OrderBy(o => o.IDConcurrentProbationryJobTitle);
								 break;
							case "IDConcurrentProbationryJobTitle_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDConcurrentProbationryJobTitle) : query.OrderByDescending(o => o.IDConcurrentProbationryJobTitle);
                                break;
							case "IDConcurrentProbationryDepartment":
								query = isOrdered ? ordered.ThenBy(o => o.IDConcurrentProbationryDepartment) : query.OrderBy(o => o.IDConcurrentProbationryDepartment);
								 break;
							case "IDConcurrentProbationryDepartment_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDConcurrentProbationryDepartment) : query.OrderByDescending(o => o.IDConcurrentProbationryDepartment);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_StaffConcurrentProbationryPosition> _pagingBuilder(IQueryable<tbl_HRM_StaffConcurrentProbationryPosition> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
