...
namespace ClassLibrary
{
    ...
    public partial class tbl_BI_Finance_CashFlow
    {
        public int IDBranch { get; set; }
        public int IDCashFlowComponent { get; set; }
...
{
	using System;
	public partial class DTO_BI_Finance_CashFlow
	{
		public int IDBranch { get; set; }
		public int IDCashFlowComponent { get; set; }
		public int Id { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_BI_Finance_CashFlow 
    {
		...
		public static IQueryable<DTO_BI_Finance_CashFlow> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS BI_Finance_CashFlow";
		...
		public static DTO_BI_Finance_CashFlow getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_BI_Finance_CashFlow.Find(id);
        ...
        {
            bool result = false;
            var dbitem = db.tbl_BI_Finance_CashFlow.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_BI_Finance_CashFlow",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_BI_Finance_CashFlow dbitem, string Username)
        {
            Type type = typeof(tbl_BI_Finance_CashFlow);
        ...
        public static void patchDTOtoDBValue( DTO_BI_Finance_CashFlow item, tbl_BI_Finance_CashFlow dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDCashFlowComponent = item.IDCashFlowComponent;							
		...
		public static DTO_BI_Finance_CashFlow post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_BI_Finance_CashFlow dbitem = new tbl_BI_Finance_CashFlow();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_BI_Finance_CashFlow.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_BI_Finance_CashFlow",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_BI_Finance_CashFlow();
                    ...
                    {
                        dbitem = new tbl_BI_Finance_CashFlow();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_BI_Finance_CashFlow.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_BI_Finance_CashFlow.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_BI_Finance_CashFlow.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_BI_Finance_CashFlow", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_BI_Finance_CashFlow",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_BI_Finance_CashFlow.Any(e => e.Id == id && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_BI_Finance_CashFlow> toDTO(IQueryable<tbl_BI_Finance_CashFlow> query)
        {
			return query
			.Select(s => new DTO_BI_Finance_CashFlow(){							
				IDBranch = s.IDBranch,							
				IDCashFlowComponent = s.IDCashFlowComponent,							
				Id = s.Id,							
		...
		public static DTO_BI_Finance_CashFlow toDTO(tbl_BI_Finance_CashFlow dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_BI_Finance_CashFlow()
				{							
					IDBranch = dbResult.IDBranch,							
					IDCashFlowComponent = dbResult.IDCashFlowComponent,							
					Id = dbResult.Id,							
		...
		public static IQueryable<tbl_BI_Finance_CashFlow> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_BI_Finance_CashFlow> query = db.tbl_BI_Finance_CashFlow.AsNoTracking();//.Where(d => d.IsDeleted == false);
			...
			//Query IDCashFlowComponent (int)
			if (QueryStrings.Any(d => d.Key == "IDCashFlowComponent"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDCashFlowComponent").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDCashFlowComponent));
            }
        ...
        public static IQueryable<tbl_BI_Finance_CashFlow> _sortBuilder(IQueryable<tbl_BI_Finance_CashFlow> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_BI_Finance_CashFlow>;
                        ...
                        switch (item)
                        {
							...
							case "IDBranch_desc":
                                ...
                                break;
							case "IDCashFlowComponent":
								query = isOrdered ? ordered.ThenBy(o => o.IDCashFlowComponent) : query.OrderBy(o => o.IDCashFlowComponent);
								 break;
							case "IDCashFlowComponent_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDCashFlowComponent) : query.OrderByDescending(o => o.IDCashFlowComponent);
                                break;
							case "Id":
        ...
        public static IQueryable<tbl_BI_Finance_CashFlow> _pagingBuilder(IQueryable<tbl_BI_Finance_CashFlow> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
