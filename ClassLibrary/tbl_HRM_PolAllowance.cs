...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_PolAllowance
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_HRM_PolAllowance()
        {
        ...
        public Nullable<int> IDAllowanceType { get; set; }
...
{
	using System;
	public partial class DTO_HRM_PolAllowance
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_PolAllowance 
    {
		...
		public static IQueryable<DTO_HRM_PolAllowance> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_PolAllowance";
		...
		public static DTO_HRM_PolAllowance getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_PolAllowance.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_PolAllowance getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_PolAllowance
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_PolAllowance.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_PolAllowance",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_PolAllowance dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_PolAllowance);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_PolAllowance item, tbl_HRM_PolAllowance dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDAllowanceType = item.IDAllowanceType;							
		...
		public static DTO_HRM_PolAllowance post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_PolAllowance dbitem = new tbl_HRM_PolAllowance();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_PolAllowance.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_PolAllowance",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_PolAllowance();
                    ...
                    {
                        dbitem = new tbl_HRM_PolAllowance();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_PolAllowance.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_PolAllowance.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_PolAllowance.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_PolAllowance", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_PolAllowance",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_PolAllowance",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_PolAllowance.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_PolAllowance.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_PolAllowance> toDTO(IQueryable<tbl_HRM_PolAllowance> query)
        {
			return query
			.Select(s => new DTO_HRM_PolAllowance(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				IDAllowanceType = s.IDAllowanceType,							
		...
		public static DTO_HRM_PolAllowance toDTO(tbl_HRM_PolAllowance dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_PolAllowance()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IDAllowanceType = dbResult.IDAllowanceType,							
		...
		public static IQueryable<tbl_HRM_PolAllowance> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_PolAllowance> query = db.tbl_HRM_PolAllowance.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query IDAllowanceType (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDAllowanceType"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDAllowanceType").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDAllowanceType));
////                    query = query.Where(d => IDs.Contains(d.IDAllowanceType));
//                    
            }
...
//                    
        ...
        public static IQueryable<tbl_HRM_PolAllowance> _sortBuilder(IQueryable<tbl_HRM_PolAllowance> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_PolAllowance>;
                        ...
                        switch (item)
                        {
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "IDAllowanceType":
								query = isOrdered ? ordered.ThenBy(o => o.IDAllowanceType) : query.OrderBy(o => o.IDAllowanceType);
								 break;
							case "IDAllowanceType_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDAllowanceType) : query.OrderByDescending(o => o.IDAllowanceType);
                                break;
							case "DecisionNumber":
        ...
        public static IQueryable<tbl_HRM_PolAllowance> _pagingBuilder(IQueryable<tbl_HRM_PolAllowance> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
