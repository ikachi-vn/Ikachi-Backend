...
namespace ClassLibrary
{
    ...
    public partial class tbl_SHIP_ShipmentDetail
    {
        ...
        //List 0:1
        public virtual tbl_SHIP_Shipment tbl_SHIP_Shipment { get; set; }
        //List 0:1
...
namespace BaseBusiness
{
    ...
    public static partial class BS_SHIP_ShipmentDetail 
    {
		...
		public static IQueryable<DTO_SHIP_ShipmentDetail> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS SHIP_ShipmentDetail";
            ...
            if (workBook != null)
            {
                ...
                //var SHIP_ShipmentList = BS_SHIP_Shipment.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_SHIP_ShipmentDetail getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_SHIP_ShipmentDetail.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_SHIP_ShipmentDetail getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_SHIP_ShipmentDetail
        ...
        {
            bool result = false;
            var dbitem = db.tbl_SHIP_ShipmentDetail.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_SHIP_ShipmentDetail",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_SHIP_ShipmentDetail dbitem, string Username)
        {
            Type type = typeof(tbl_SHIP_ShipmentDetail);
        ...
        public static void patchDTOtoDBValue( DTO_SHIP_ShipmentDetail item, tbl_SHIP_ShipmentDetail dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDShipment = item.IDShipment;							
		...
		public static DTO_SHIP_ShipmentDetail post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_SHIP_ShipmentDetail dbitem = new tbl_SHIP_ShipmentDetail();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_SHIP_ShipmentDetail.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_SHIP_ShipmentDetail",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_SHIP_ShipmentDetail();
                    ...
                    {
                        dbitem = new tbl_SHIP_ShipmentDetail();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_SHIP_ShipmentDetail.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_SHIP_ShipmentDetail.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_SHIP_ShipmentDetail.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_SHIP_ShipmentDetail", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_SHIP_ShipmentDetail",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_SHIP_ShipmentDetail",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_SHIP_ShipmentDetail.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_SHIP_ShipmentDetail.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_SHIP_ShipmentDetail> toDTO(IQueryable<tbl_SHIP_ShipmentDetail> query)
        {
			return query
			.Select(s => new DTO_SHIP_ShipmentDetail(){							
		...
		public static DTO_SHIP_ShipmentDetail toDTO(tbl_SHIP_ShipmentDetail dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_SHIP_ShipmentDetail()
				{							
					IDShipment = dbResult.IDShipment,							
		...
		public static IQueryable<tbl_SHIP_ShipmentDetail> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_SHIP_ShipmentDetail> query = db.tbl_SHIP_ShipmentDetail.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			if (QueryStrings.Any(d => d.Key == "IDShipment"))
            {
                ...
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDShipment));
            }
...
//                    
        ...
        public static IQueryable<tbl_SHIP_ShipmentDetail> _sortBuilder(IQueryable<tbl_SHIP_ShipmentDetail> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_SHIP_ShipmentDetail>;
                        bool isOrdered = ordered.ToString().IndexOf("ORDER BY") !=-1;

                        switch (item)
                        {
							case "IDShipment":
								query = isOrdered ? ordered.ThenBy(o => o.IDShipment) : query.OrderBy(o => o.IDShipment);
								 break;
							case "IDShipment_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDShipment) : query.OrderByDescending(o => o.IDShipment);
                                break;
							case "IDSaleOrder":
        ...
        public static IQueryable<tbl_SHIP_ShipmentDetail> _pagingBuilder(IQueryable<tbl_SHIP_ShipmentDetail> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
