...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffStaffAndFamilyJob
    {
        ...
        //List 0:1
        public virtual tbl_HRM_StaffFamily tbl_HRM_StaffFamily { get; set; }
        //List 0:1
...
{
	using System;
	public partial class DTO_HRM_StaffStaffAndFamilyJob
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffStaffAndFamilyJob 
    {
		...
		public static IQueryable<DTO_HRM_StaffStaffAndFamilyJob> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffStaffAndFamilyJob";
            ...
            if (workBook != null)
            {
                ...
                //var HRM_StaffFamilyList = BS_HRM_StaffFamily.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_HRM_StaffStaffAndFamilyJob getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffStaffAndFamilyJob.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffStaffAndFamilyJob getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffStaffAndFamilyJob
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffStaffAndFamilyJob.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffStaffAndFamilyJob",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffStaffAndFamilyJob dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffStaffAndFamilyJob);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffStaffAndFamilyJob item, tbl_HRM_StaffStaffAndFamilyJob dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IsStaffJob = item.IsStaffJob;							
			dbitem.IDStaff = item.IDStaff;							
			dbitem.IDFamily = item.IDFamily;							
			dbitem.IDJob = item.IDJob;        }

		public static DTO_HRM_StaffStaffAndFamilyJob post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffStaffAndFamilyJob dbitem = new tbl_HRM_StaffStaffAndFamilyJob();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffStaffAndFamilyJob.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffStaffAndFamilyJob",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffStaffAndFamilyJob();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffStaffAndFamilyJob();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffStaffAndFamilyJob.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffStaffAndFamilyJob.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffStaffAndFamilyJob.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffStaffAndFamilyJob", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffStaffAndFamilyJob",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_StaffStaffAndFamilyJob",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffStaffAndFamilyJob.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffStaffAndFamilyJob.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffStaffAndFamilyJob> toDTO(IQueryable<tbl_HRM_StaffStaffAndFamilyJob> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffStaffAndFamilyJob(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
		...
		public static DTO_HRM_StaffStaffAndFamilyJob toDTO(tbl_HRM_StaffStaffAndFamilyJob dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffStaffAndFamilyJob()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IsStaffJob = dbResult.IsStaffJob,							
					IDStaff = dbResult.IDStaff,							
					IDFamily = dbResult.IDFamily,							
					IDJob = dbResult.IDJob,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_HRM_StaffStaffAndFamilyJob> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffStaffAndFamilyJob> query = db.tbl_HRM_StaffStaffAndFamilyJob.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query IsStaffJob (bool)
			if (QueryStrings.Any(d => d.Key == "IsStaffJob"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "IsStaffJob").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.IsStaffJob);
            }
...
//                    
			...
			//Query IDFamily (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDFamily"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDFamily").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDFamily));
////                    query = query.Where(d => IDs.Contains(d.IDFamily));
//                    
            }

			//Query IDJob (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDJob"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDJob").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDJob));
////                    query = query.Where(d => IDs.Contains(d.IDJob));
//                    
            }
            return query;
        ...
        public static IQueryable<tbl_HRM_StaffStaffAndFamilyJob> _sortBuilder(IQueryable<tbl_HRM_StaffStaffAndFamilyJob> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffStaffAndFamilyJob>;
                        ...
                        switch (item)
                        {
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "IsStaffJob":
								query = isOrdered ? ordered.ThenBy(o => o.IsStaffJob) : query.OrderBy(o => o.IsStaffJob);
								 break;
							case "IsStaffJob_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IsStaffJob) : query.OrderByDescending(o => o.IsStaffJob);
                                break;
							case "IDStaff":
							...
							case "IDStaff_desc":
                                ...
                                break;
							case "IDFamily":
								query = isOrdered ? ordered.ThenBy(o => o.IDFamily) : query.OrderBy(o => o.IDFamily);
								 break;
							case "IDFamily_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDFamily) : query.OrderByDescending(o => o.IDFamily);
                                break;
							case "IDJob":
								query = isOrdered ? ordered.ThenBy(o => o.IDJob) : query.OrderBy(o => o.IDJob);
								 break;
							case "IDJob_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDJob) : query.OrderByDescending(o => o.IDJob);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_StaffStaffAndFamilyJob> _pagingBuilder(IQueryable<tbl_HRM_StaffStaffAndFamilyJob> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
