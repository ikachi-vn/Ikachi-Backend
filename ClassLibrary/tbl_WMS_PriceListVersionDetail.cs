...
namespace ClassLibrary
{
    ...
    public partial class tbl_WMS_PriceListVersionDetail
    {
        public int IDPriceListVersion { get; set; }
        public int Id { get; set; }
        public int IDItem { get; set; }
        public int IDItemUoM { get; set; }
        ...
        //List 0:1
        public virtual tbl_WMS_PriceListVersion tbl_WMS_PriceListVersion { get; set; }
    }
}
...
{
	using System;
	public partial class DTO_WMS_PriceListVersionDetail
	{
		public int IDPriceListVersion { get; set; }
		public int Id { get; set; }
		public int IDItem { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_WMS_PriceListVersionDetail 
    {
		...
		public static IQueryable<DTO_WMS_PriceListVersionDetail> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS WMS_PriceListVersionDetail";
            ...
            if (workBook != null)
            {
                ...
                //var WMS_ItemUoMList = BS_WMS_ItemUoM.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
                //var WMS_PriceListVersionList = BS_WMS_PriceListVersion.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_WMS_PriceListVersionDetail getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_WMS_PriceListVersionDetail.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_WMS_PriceListVersionDetail getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_WMS_PriceListVersionDetail
        ...
        {
            bool result = false;
            var dbitem = db.tbl_WMS_PriceListVersionDetail.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_WMS_PriceListVersionDetail",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_WMS_PriceListVersionDetail dbitem, string Username)
        {
            Type type = typeof(tbl_WMS_PriceListVersionDetail);
        ...
        public static void patchDTOtoDBValue( DTO_WMS_PriceListVersionDetail item, tbl_WMS_PriceListVersionDetail dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDPriceListVersion = item.IDPriceListVersion;							
		...
		public static DTO_WMS_PriceListVersionDetail post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_WMS_PriceListVersionDetail dbitem = new tbl_WMS_PriceListVersionDetail();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_WMS_PriceListVersionDetail.Add(dbitem);
                    db.SaveChanges();
        ...
        public static dynamic import(ExcelPackage package, AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings, string Username)
        {
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    {
                        dbitem = new tbl_WMS_PriceListVersionDetail();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_WMS_PriceListVersionDetail.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_WMS_PriceListVersionDetail.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_WMS_PriceListVersionDetail.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_WMS_PriceListVersionDetail", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_WMS_PriceListVersionDetail",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_WMS_PriceListVersionDetail",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_WMS_PriceListVersionDetail.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_WMS_PriceListVersionDetail.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_WMS_PriceListVersionDetail> toDTO(IQueryable<tbl_WMS_PriceListVersionDetail> query)
        {
			return query
			.Select(s => new DTO_WMS_PriceListVersionDetail(){							
				IDPriceListVersion = s.IDPriceListVersion,							
				Id = s.Id,							
				IDItem = s.IDItem,							
				IDItemUoM = s.IDItemUoM,							
				Price = s.Price,							
				Price1 = s.Price1,							
				Price2 = s.Price2,							
				IsManual = s.IsManual,							
		...
		public static DTO_WMS_PriceListVersionDetail toDTO(tbl_WMS_PriceListVersionDetail dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_WMS_PriceListVersionDetail()
				{							
					IDPriceListVersion = dbResult.IDPriceListVersion,							
					Id = dbResult.Id,							
					IDItem = dbResult.IDItem,							
					IDItemUoM = dbResult.IDItemUoM,							
		...
		public static IQueryable<tbl_WMS_PriceListVersionDetail> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_WMS_PriceListVersionDetail> query = db.tbl_WMS_PriceListVersionDetail.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			if (QueryStrings.Any(d => d.Key == "IDPriceListVersion"))
            {
                ...
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDPriceListVersion));
            }
			...
			//Query IDItemUoM (int)
			if (QueryStrings.Any(d => d.Key == "IDItemUoM"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDItemUoM").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDItemUoM));
            }

			//Query Price (decimal)
			if (QueryStrings.Any(d => d.Key == "PriceFrom") && QueryStrings.Any(d => d.Key == "PriceTo"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "PriceFrom").Value, out decimal fromVal) && decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "PriceTo").Value, out decimal toVal))
                    query = query.Where(d => fromVal <= d.Price && d.Price <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "Price"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "Price").Value, out decimal val))
                    query = query.Where(d => val == d.Price);


			//Query Price1 (decimal)
			if (QueryStrings.Any(d => d.Key == "Price1From") && QueryStrings.Any(d => d.Key == "Price1To"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "Price1From").Value, out decimal fromVal) && decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "Price1To").Value, out decimal toVal))
                    query = query.Where(d => fromVal <= d.Price1 && d.Price1 <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "Price1"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "Price1").Value, out decimal val))
                    query = query.Where(d => val == d.Price1);


			//Query Price2 (decimal)
			if (QueryStrings.Any(d => d.Key == "Price2From") && QueryStrings.Any(d => d.Key == "Price2To"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "Price2From").Value, out decimal fromVal) && decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "Price2To").Value, out decimal toVal))
                    query = query.Where(d => fromVal <= d.Price2 && d.Price2 <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "Price2"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "Price2").Value, out decimal val))
                    query = query.Where(d => val == d.Price2);


			//Query IsManual (bool)
			if (QueryStrings.Any(d => d.Key == "IsManual"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "IsManual").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.IsManual);
            }
        ...
        public static IQueryable<tbl_WMS_PriceListVersionDetail> _sortBuilder(IQueryable<tbl_WMS_PriceListVersionDetail> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_WMS_PriceListVersionDetail>;
                        bool isOrdered = ordered.ToString().IndexOf("ORDER BY") !=-1;

                        switch (item)
                        {
							case "IDPriceListVersion":
								query = isOrdered ? ordered.ThenBy(o => o.IDPriceListVersion) : query.OrderBy(o => o.IDPriceListVersion);
								 break;
							case "IDPriceListVersion_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDPriceListVersion) : query.OrderByDescending(o => o.IDPriceListVersion);
                                break;
							case "Id":
							...
							case "IDItem_desc":
                                ...
                                break;
							case "IDItemUoM":
								query = isOrdered ? ordered.ThenBy(o => o.IDItemUoM) : query.OrderBy(o => o.IDItemUoM);
								 break;
							case "IDItemUoM_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDItemUoM) : query.OrderByDescending(o => o.IDItemUoM);
                                break;
							case "Price":
								query = isOrdered ? ordered.ThenBy(o => o.Price) : query.OrderBy(o => o.Price);
								 break;
							case "Price_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Price) : query.OrderByDescending(o => o.Price);
                                break;
							case "Price1":
								query = isOrdered ? ordered.ThenBy(o => o.Price1) : query.OrderBy(o => o.Price1);
								 break;
							case "Price1_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Price1) : query.OrderByDescending(o => o.Price1);
                                break;
							case "Price2":
								query = isOrdered ? ordered.ThenBy(o => o.Price2) : query.OrderBy(o => o.Price2);
								 break;
							case "Price2_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Price2) : query.OrderByDescending(o => o.Price2);
                                break;
							case "IsManual":
        ...
        public static IQueryable<tbl_WMS_PriceListVersionDetail> _pagingBuilder(IQueryable<tbl_WMS_PriceListVersionDetail> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
