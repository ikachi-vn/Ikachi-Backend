...
namespace ClassLibrary
{
    ...
    public partial class tbl_WH_ContactGroups
    {
...
{
	using System;
	public partial class DTO_WH_ContactGroups
	{
		public int Id { get; set; }
		public Nullable<short> IDBranch { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_WH_ContactGroups 
    {
		...
		public static IQueryable<DTO_WH_ContactGroups> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS WH_ContactGroups";
		...
		public static DTO_WH_ContactGroups getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_WH_ContactGroups.Find(id);
		...
		public static DTO_WH_ContactGroups getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_WH_ContactGroups
        ...
        {
            bool result = false;
            var dbitem = db.tbl_WH_ContactGroups.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_WH_ContactGroups",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_WH_ContactGroups dbitem, string Username)
        {
            Type type = typeof(tbl_WH_ContactGroups);
        ...
        public static void patchDTOtoDBValue( DTO_WH_ContactGroups item, tbl_WH_ContactGroups dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_WH_ContactGroups post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_WH_ContactGroups dbitem = new tbl_WH_ContactGroups();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_WH_ContactGroups.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_WH_ContactGroups",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_WH_ContactGroups();
                    ...
                    {
                        dbitem = new tbl_WH_ContactGroups();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_WH_ContactGroups.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_WH_ContactGroups.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_WH_ContactGroups.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_WH_ContactGroups", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_WH_ContactGroups",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_WH_ContactGroups",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_WH_ContactGroups.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_WH_ContactGroups.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_WH_ContactGroups> toDTO(IQueryable<tbl_WH_ContactGroups> query)
        {
			return query
			.Select(s => new DTO_WH_ContactGroups(){							
				Id = s.Id,							
				IDBranch = s.IDBranch,							
				Code = s.Code,							
				Name = s.Name,							
		...
		public static DTO_WH_ContactGroups toDTO(tbl_WH_ContactGroups dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_WH_ContactGroups()
				{							
					Id = dbResult.Id,							
		...
		public static IQueryable<tbl_WH_ContactGroups> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_WH_ContactGroups> query = db.tbl_WH_ContactGroups.AsNoTracking();//.Where(d => d.IsDeleted == false);
        ...
        public static IQueryable<tbl_WH_ContactGroups> _sortBuilder(IQueryable<tbl_WH_ContactGroups> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_WH_ContactGroups>;
        ...
        public static IQueryable<tbl_WH_ContactGroups> _pagingBuilder(IQueryable<tbl_WH_ContactGroups> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
