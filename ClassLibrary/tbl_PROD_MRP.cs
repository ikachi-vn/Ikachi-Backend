...
namespace ClassLibrary
{
    ...
    public partial class tbl_PROD_MRP
    {
        public int Id { get; set; }
        public Nullable<int> IDMRP { get; set; }
        public Nullable<int> IDMRPLineID { get; set; }
        public Nullable<int> IDItem { get; set; }
        public Nullable<int> IDUoM { get; set; }
        public Nullable<int> IDPreferVendor { get; set; }
        ...
        public Nullable<short> MRPLeadtime { get; set; }
        public Nullable<short> MRPToleranDay { get; set; }
...
{
	using System;
	public partial class DTO_PROD_MRP
	{
		public int Id { get; set; }
		public Nullable<int> IDMRP { get; set; }
		public Nullable<int> IDMRPLineID { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_PROD_MRP 
    {
		...
		public static IQueryable<DTO_PROD_MRP> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS PROD_MRP";
		...
		public static DTO_PROD_MRP getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_PROD_MRP.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_PROD_MRP getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_PROD_MRP
        ...
        {
            bool result = false;
            var dbitem = db.tbl_PROD_MRP.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_PROD_MRP",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_PROD_MRP dbitem, string Username)
        {
            Type type = typeof(tbl_PROD_MRP);
        ...
        public static void patchDTOtoDBValue( DTO_PROD_MRP item, tbl_PROD_MRP dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDMRP = item.IDMRP;							
			dbitem.IDMRPLineID = item.IDMRPLineID;							
			dbitem.IDItem = item.IDItem;							
			dbitem.IDUoM = item.IDUoM;							
			dbitem.IDPreferVendor = item.IDPreferVendor;							
			dbitem.MRPType = item.MRPType;							
			dbitem.Name = item.Name;							
			dbitem.Code = item.Code;							
			dbitem.Remark = item.Remark;							
			dbitem.Startdate = item.Startdate;							
			dbitem.EndDate = item.EndDate;							
			dbitem.MRPLeadtime = item.MRPLeadtime;							
			dbitem.MRPToleranDay = item.MRPToleranDay;							
			dbitem.DueDate = item.DueDate;							
			dbitem.UnitPrice = item.UnitPrice;							
			dbitem.Discount = item.Discount;							
			dbitem.PriceAfterDiscount = item.PriceAfterDiscount;							
			dbitem.QuantityOrdered = item.QuantityOrdered;							
			dbitem.QuantityInStock = item.QuantityInStock;							
			dbitem.Sort = item.Sort;							
			dbitem.IsDisabled = item.IsDisabled;							
			dbitem.IsDeleted = item.IsDeleted;        }

		public static DTO_PROD_MRP post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_PROD_MRP dbitem = new tbl_PROD_MRP();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_PROD_MRP.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_PROD_MRP",e);
                    return null;
                }
            }
            return toDTO(dbitem);
        ...
        public static dynamic import(ExcelPackage package, AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings, string Username)
        {
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    {
                        dbitem = new tbl_PROD_MRP();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_PROD_MRP.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_PROD_MRP.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_PROD_MRP.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_PROD_MRP", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_PROD_MRP",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_PROD_MRP",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_PROD_MRP.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_PROD_MRP.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_PROD_MRP> toDTO(IQueryable<tbl_PROD_MRP> query)
        {
			return query
			.Select(s => new DTO_PROD_MRP(){							
				Id = s.Id,							
				IDMRP = s.IDMRP,							
				IDMRPLineID = s.IDMRPLineID,							
				IDItem = s.IDItem,							
				IDUoM = s.IDUoM,							
				IDPreferVendor = s.IDPreferVendor,							
				MRPType = s.MRPType,							
				Name = s.Name,							
				Code = s.Code,							
				Remark = s.Remark,							
				...
				EndDate = s.EndDate,							
				MRPLeadtime = s.MRPLeadtime,							
				MRPToleranDay = s.MRPToleranDay,							
				DueDate = s.DueDate,							
		...
		public static DTO_PROD_MRP toDTO(tbl_PROD_MRP dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_PROD_MRP()
				{							
					Id = dbResult.Id,							
					IDMRP = dbResult.IDMRP,							
					IDMRPLineID = dbResult.IDMRPLineID,							
					IDItem = dbResult.IDItem,							
					IDUoM = dbResult.IDUoM,							
					IDPreferVendor = dbResult.IDPreferVendor,							
					MRPType = dbResult.MRPType,							
					Name = dbResult.Name,							
					Code = dbResult.Code,							
					Remark = dbResult.Remark,							
					Startdate = dbResult.Startdate,							
					EndDate = dbResult.EndDate,							
					MRPLeadtime = dbResult.MRPLeadtime,							
					MRPToleranDay = dbResult.MRPToleranDay,							
					DueDate = dbResult.DueDate,							
					UnitPrice = dbResult.UnitPrice,							
					Discount = dbResult.Discount,							
					PriceAfterDiscount = dbResult.PriceAfterDiscount,							
					QuantityOrdered = dbResult.QuantityOrdered,							
					QuantityInStock = dbResult.QuantityInStock,							
					Sort = dbResult.Sort,							
		...
		public static IQueryable<tbl_PROD_MRP> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_PROD_MRP> query = db.tbl_PROD_MRP.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query IDMRP (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDMRP"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDMRP").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDMRP));
////                    query = query.Where(d => IDs.Contains(d.IDMRP));
//                    
            }

			//Query IDMRPLineID (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDMRPLineID"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDMRPLineID").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDMRPLineID));
////                    query = query.Where(d => IDs.Contains(d.IDMRPLineID));
//                    
            }

			//Query IDItem (Nullable<int>)
...
//                    
			...
			//Query IDPreferVendor (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDPreferVendor"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDPreferVendor").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDPreferVendor));
////                    query = query.Where(d => IDs.Contains(d.IDPreferVendor));
//                    
            }

			//Query MRPType (string)
			if (QueryStrings.Any(d => d.Key == "MRPType_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "MRPType_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "MRPType_eq").Value;
                query = query.Where(d=>d.MRPType == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "MRPType") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "MRPType").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "MRPType").Value;
                query = query.Where(d=>d.MRPType.Contains(keyword));
            }
			...
			//Query Startdate (Nullable<System.DateTime>)
			...
			//Query EndDate (Nullable<System.DateTime>)
			...
			//Query MRPLeadtime (Nullable<short>)

			//Query MRPToleranDay (Nullable<short>)
			...
			//Query UnitPrice (Nullable<double>)

			//Query Discount (Nullable<double>)

			//Query PriceAfterDiscount (Nullable<double>)

			//Query QuantityOrdered (Nullable<double>)

			//Query QuantityInStock (Nullable<double>)
        ...
        public static IQueryable<tbl_PROD_MRP> _sortBuilder(IQueryable<tbl_PROD_MRP> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_PROD_MRP>;
                        ...
                        switch (item)
                        {
							...
							case "Id_desc":
                                ...
                                break;
							case "IDMRP":
								query = isOrdered ? ordered.ThenBy(o => o.IDMRP) : query.OrderBy(o => o.IDMRP);
								 break;
							case "IDMRP_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDMRP) : query.OrderByDescending(o => o.IDMRP);
                                break;
							case "IDMRPLineID":
								query = isOrdered ? ordered.ThenBy(o => o.IDMRPLineID) : query.OrderBy(o => o.IDMRPLineID);
								 break;
							case "IDMRPLineID_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDMRPLineID) : query.OrderByDescending(o => o.IDMRPLineID);
                                break;
							case "IDItem":
							...
							case "IDUoM_desc":
                                ...
                                break;
							case "IDPreferVendor":
								query = isOrdered ? ordered.ThenBy(o => o.IDPreferVendor) : query.OrderBy(o => o.IDPreferVendor);
								 break;
							case "IDPreferVendor_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDPreferVendor) : query.OrderByDescending(o => o.IDPreferVendor);
                                break;
							case "MRPType":
								query = isOrdered ? ordered.ThenBy(o => o.MRPType) : query.OrderBy(o => o.MRPType);
								 break;
							case "MRPType_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.MRPType) : query.OrderByDescending(o => o.MRPType);
                                break;
							case "Name":
							...
							case "EndDate_desc":
                                ...
                                break;
							case "MRPLeadtime":
								query = isOrdered ? ordered.ThenBy(o => o.MRPLeadtime) : query.OrderBy(o => o.MRPLeadtime);
								 break;
							case "MRPLeadtime_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.MRPLeadtime) : query.OrderByDescending(o => o.MRPLeadtime);
                                break;
							case "MRPToleranDay":
								query = isOrdered ? ordered.ThenBy(o => o.MRPToleranDay) : query.OrderBy(o => o.MRPToleranDay);
								 break;
							case "MRPToleranDay_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.MRPToleranDay) : query.OrderByDescending(o => o.MRPToleranDay);
                                break;
							case "DueDate":
							...
							case "DueDate_desc":
                                ...
                                break;
							case "UnitPrice":
								query = isOrdered ? ordered.ThenBy(o => o.UnitPrice) : query.OrderBy(o => o.UnitPrice);
								 break;
							case "UnitPrice_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.UnitPrice) : query.OrderByDescending(o => o.UnitPrice);
                                break;
							case "Discount":
								query = isOrdered ? ordered.ThenBy(o => o.Discount) : query.OrderBy(o => o.Discount);
								 break;
							case "Discount_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Discount) : query.OrderByDescending(o => o.Discount);
                                break;
							case "PriceAfterDiscount":
								query = isOrdered ? ordered.ThenBy(o => o.PriceAfterDiscount) : query.OrderBy(o => o.PriceAfterDiscount);
								 break;
							case "PriceAfterDiscount_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.PriceAfterDiscount) : query.OrderByDescending(o => o.PriceAfterDiscount);
                                break;
							case "QuantityOrdered":
								query = isOrdered ? ordered.ThenBy(o => o.QuantityOrdered) : query.OrderBy(o => o.QuantityOrdered);
								 break;
							case "QuantityOrdered_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.QuantityOrdered) : query.OrderByDescending(o => o.QuantityOrdered);
                                break;
							case "QuantityInStock":
								query = isOrdered ? ordered.ThenBy(o => o.QuantityInStock) : query.OrderBy(o => o.QuantityInStock);
								 break;
							case "QuantityInStock_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.QuantityInStock) : query.OrderByDescending(o => o.QuantityInStock);
                                break;
							case "Sort":
        ...
        public static IQueryable<tbl_PROD_MRP> _pagingBuilder(IQueryable<tbl_PROD_MRP> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
