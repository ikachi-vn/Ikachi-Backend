...
namespace ClassLibrary
{
    ...
    public partial class tbl_LIST_Province
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_LIST_Province()
        {
            this.tbl_LIST_District = new HashSet<tbl_LIST_District>();
        }
        ...
        public virtual ICollection<tbl_LIST_District> tbl_LIST_District { get; set; }
    }
}
...
{
	using System;
	public partial class DTO_LIST_Province
	{
		public int IDCountry { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_LIST_Province 
    {
		...
		public static IQueryable<DTO_LIST_Province> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS LIST_Province";
		...
		public static DTO_LIST_Province getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_LIST_Province.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_LIST_Province getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_LIST_Province
        ...
        {
            bool result = false;
            var dbitem = db.tbl_LIST_Province.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_LIST_Province",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_LIST_Province dbitem, string Username)
        {
            Type type = typeof(tbl_LIST_Province);
        ...
        public static void patchDTOtoDBValue( DTO_LIST_Province item, tbl_LIST_Province dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_LIST_Province post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_LIST_Province dbitem = new tbl_LIST_Province();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_LIST_Province.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_LIST_Province",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_LIST_Province();
                    ...
                    {
                        dbitem = new tbl_LIST_Province();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_LIST_Province.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_LIST_Province.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_LIST_Province.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_LIST_Province", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_LIST_Province",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_LIST_Province",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_LIST_Province.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_LIST_Province.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_LIST_Province> toDTO(IQueryable<tbl_LIST_Province> query)
        {
			return query
			.Select(s => new DTO_LIST_Province(){							
		...
		public static DTO_LIST_Province toDTO(tbl_LIST_Province dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_LIST_Province()
				{							
		...
		public static IQueryable<tbl_LIST_Province> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_LIST_Province> query = db.tbl_LIST_Province.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query TelephoneCode (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "TelephoneCodeFrom") && QueryStrings.Any(d => d.Key == "TelephoneCodeTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "TelephoneCodeFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "TelephoneCodeTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.TelephoneCode && d.TelephoneCode <= toVal);
        ...
        public static IQueryable<tbl_LIST_Province> _sortBuilder(IQueryable<tbl_LIST_Province> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_LIST_Province>;
        ...
        public static IQueryable<tbl_LIST_Province> _pagingBuilder(IQueryable<tbl_LIST_Province> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
