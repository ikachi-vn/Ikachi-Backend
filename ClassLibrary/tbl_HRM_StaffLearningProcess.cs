...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffLearningProcess
    {
...
{
	using System;
	public partial class DTO_HRM_StaffLearningProcess
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffLearningProcess 
    {
		...
		public static IQueryable<DTO_HRM_StaffLearningProcess> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffLearningProcess";
		...
		public static DTO_HRM_StaffLearningProcess getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffLearningProcess.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffLearningProcess getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffLearningProcess
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffLearningProcess.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffLearningProcess",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffLearningProcess dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffLearningProcess);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffLearningProcess item, tbl_HRM_StaffLearningProcess dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDSpecialist = item.IDSpecialist;							
			dbitem.Result = item.Result;							
			dbitem.Certificate = item.Certificate;        }

		public static DTO_HRM_StaffLearningProcess post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffLearningProcess dbitem = new tbl_HRM_StaffLearningProcess();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffLearningProcess.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffLearningProcess",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffLearningProcess();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffLearningProcess();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffLearningProcess.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffLearningProcess.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffLearningProcess.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffLearningProcess", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffLearningProcess",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_StaffLearningProcess",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffLearningProcess.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffLearningProcess.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffLearningProcess> toDTO(IQueryable<tbl_HRM_StaffLearningProcess> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffLearningProcess(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
		...
		public static DTO_HRM_StaffLearningProcess toDTO(tbl_HRM_StaffLearningProcess dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffLearningProcess()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IDSpecialist = dbResult.IDSpecialist,							
		...
		public static IQueryable<tbl_HRM_StaffLearningProcess> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffLearningProcess> query = db.tbl_HRM_StaffLearningProcess.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query IDSpecialist (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDSpecialist"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDSpecialist").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDSpecialist));
////                    query = query.Where(d => IDs.Contains(d.IDSpecialist));
//                    
            }
			...
			if (QueryStrings.Any(d => d.Key == "Certificate_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Certificate_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Certificate_eq").Value;
                query = query.Where(d=>d.Certificate == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "Certificate") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Certificate").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Certificate").Value;
                query = query.Where(d=>d.Certificate.Contains(keyword));
            }
            
            return query;
        }
		
        public static IQueryable<tbl_HRM_StaffLearningProcess> _sortBuilder(IQueryable<tbl_HRM_StaffLearningProcess> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffLearningProcess>;
                        ...
                        switch (item)
                        {
							...
							case "Place_desc":
                                ...
                                break;
							case "IDSpecialist":
								query = isOrdered ? ordered.ThenBy(o => o.IDSpecialist) : query.OrderBy(o => o.IDSpecialist);
								 break;
							case "IDSpecialist_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDSpecialist) : query.OrderByDescending(o => o.IDSpecialist);
                                break;
							case "Result":
							...
							case "Result_desc":
                                ...
                                break;
							case "Certificate":
								query = isOrdered ? ordered.ThenBy(o => o.Certificate) : query.OrderBy(o => o.Certificate);
								 break;
							case "Certificate_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Certificate) : query.OrderByDescending(o => o.Certificate);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_StaffLearningProcess> _pagingBuilder(IQueryable<tbl_HRM_StaffLearningProcess> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
