...
namespace ClassLibrary
{
    ...
    public partial class tbl_CRM_Outlets
    {
        ...
        public string CustomersAccess { get; set; }
        public string Display { get; set; }
        public Nullable<int> MainRouteId { get; set; }
...
{
	using System;
	public partial class DTO_CRM_Outlets
	{
		public int Id { get; set; }
		public string Type { get; set; }
		public string Location { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_CRM_Outlets 
    {
		...
		public static IQueryable<DTO_CRM_Outlets> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS CRM_Outlets";
		...
		public static DTO_CRM_Outlets getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_CRM_Outlets.Find(id);
        ...
        {
            bool result = false;
            var dbitem = db.tbl_CRM_Outlets.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_CRM_Outlets",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_CRM_Outlets dbitem, string Username)
        {
            Type type = typeof(tbl_CRM_Outlets);
        ...
        public static void patchDTOtoDBValue( DTO_CRM_Outlets item, tbl_CRM_Outlets dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.BusinessType = item.BusinessType;							
			dbitem.PopulationDistribution = item.PopulationDistribution;							
			dbitem.CustomersAccess = item.CustomersAccess;							
			dbitem.Display = item.Display;							
			dbitem.MainRouteId = item.MainRouteId;							
		...
		public static DTO_CRM_Outlets post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_CRM_Outlets dbitem = new tbl_CRM_Outlets();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_CRM_Outlets.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_CRM_Outlets",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_CRM_Outlets();
                    ...
                    {
                        dbitem = new tbl_CRM_Outlets();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_CRM_Outlets.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_CRM_Outlets.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_CRM_Outlets.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_CRM_Outlets", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_CRM_Outlets",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_CRM_Outlets",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_CRM_Outlets.Any(e => e.Id == id && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_CRM_Outlets> toDTO(IQueryable<tbl_CRM_Outlets> query)
        {
			return query
			.Select(s => new DTO_CRM_Outlets(){							
				Id = s.Id,							
				Type = s.Type,							
				Location = s.Location,							
				BusinessType = s.BusinessType,							
				PopulationDistribution = s.PopulationDistribution,							
				CustomersAccess = s.CustomersAccess,							
				Display = s.Display,							
		...
		public static DTO_CRM_Outlets toDTO(tbl_CRM_Outlets dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_CRM_Outlets()
				{							
					Id = dbResult.Id,							
					Type = dbResult.Type,							
					Location = dbResult.Location,							
					BusinessType = dbResult.BusinessType,							
					PopulationDistribution = dbResult.PopulationDistribution,							
					CustomersAccess = dbResult.CustomersAccess,							
					Display = dbResult.Display,							
					MainRouteId = dbResult.MainRouteId,							
		...
		public static IQueryable<tbl_CRM_Outlets> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_CRM_Outlets> query = db.tbl_CRM_Outlets.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			if (QueryStrings.Any(d => d.Key == "Location_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Location_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Location_eq").Value;
                query = query.Where(d=>d.Location == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "Location") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Location").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Location").Value;
                query = query.Where(d=>d.Location.Contains(keyword));
            }
            

			//Query BusinessType (string)
			if (QueryStrings.Any(d => d.Key == "BusinessType_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "BusinessType_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "BusinessType_eq").Value;
                query = query.Where(d=>d.BusinessType == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "BusinessType") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "BusinessType").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "BusinessType").Value;
                query = query.Where(d=>d.BusinessType.Contains(keyword));
            }
            

			//Query PopulationDistribution (string)
			if (QueryStrings.Any(d => d.Key == "PopulationDistribution_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "PopulationDistribution_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "PopulationDistribution_eq").Value;
                query = query.Where(d=>d.PopulationDistribution == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "PopulationDistribution") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "PopulationDistribution").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "PopulationDistribution").Value;
                query = query.Where(d=>d.PopulationDistribution.Contains(keyword));
            }
            

			//Query CustomersAccess (string)
			if (QueryStrings.Any(d => d.Key == "CustomersAccess_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "CustomersAccess_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "CustomersAccess_eq").Value;
                query = query.Where(d=>d.CustomersAccess == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "CustomersAccess") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "CustomersAccess").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "CustomersAccess").Value;
                query = query.Where(d=>d.CustomersAccess.Contains(keyword));
            }
            

			//Query Display (string)
			if (QueryStrings.Any(d => d.Key == "Display_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Display_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Display_eq").Value;
                query = query.Where(d=>d.Display == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "Display") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Display").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Display").Value;
                query = query.Where(d=>d.Display.Contains(keyword));
            }
            

			//Query MainRouteId (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "MainRouteIdFrom") && QueryStrings.Any(d => d.Key == "MainRouteIdTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "MainRouteIdFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "MainRouteIdTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.MainRouteId && d.MainRouteId <= toVal);
        ...
        public static IQueryable<tbl_CRM_Outlets> _sortBuilder(IQueryable<tbl_CRM_Outlets> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_CRM_Outlets>;
                        ...
                        switch (item)
                        {
							...
							case "Location_desc":
                                ...
                                break;
							case "BusinessType":
								query = isOrdered ? ordered.ThenBy(o => o.BusinessType) : query.OrderBy(o => o.BusinessType);
								 break;
							case "BusinessType_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.BusinessType) : query.OrderByDescending(o => o.BusinessType);
                                break;
							case "PopulationDistribution":
								query = isOrdered ? ordered.ThenBy(o => o.PopulationDistribution) : query.OrderBy(o => o.PopulationDistribution);
								 break;
							case "PopulationDistribution_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.PopulationDistribution) : query.OrderByDescending(o => o.PopulationDistribution);
                                break;
							case "CustomersAccess":
								query = isOrdered ? ordered.ThenBy(o => o.CustomersAccess) : query.OrderBy(o => o.CustomersAccess);
								 break;
							case "CustomersAccess_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.CustomersAccess) : query.OrderByDescending(o => o.CustomersAccess);
                                break;
							case "Display":
								query = isOrdered ? ordered.ThenBy(o => o.Display) : query.OrderBy(o => o.Display);
								 break;
							case "Display_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Display) : query.OrderByDescending(o => o.Display);
                                break;
							case "MainRouteId":
								query = isOrdered ? ordered.ThenBy(o => o.MainRouteId) : query.OrderBy(o => o.MainRouteId);
								 break;
							case "MainRouteId_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.MainRouteId) : query.OrderByDescending(o => o.MainRouteId);
                                break;
							case "Remark":
        ...
        public static IQueryable<tbl_CRM_Outlets> _pagingBuilder(IQueryable<tbl_CRM_Outlets> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
