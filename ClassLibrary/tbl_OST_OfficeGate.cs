...
namespace ClassLibrary
{
    ...
    public partial class tbl_OST_OfficeGate
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_OST_OfficeGate()
        {
        ...
        public bool IsVerifyLocation { get; set; }
        ...
        //List 0:1
        public virtual tbl_OST_Office tbl_OST_Office { get; set; }
    }
}
...
{
	using System;
	public partial class DTO_OST_OfficeGate
	{
		public Nullable<int> IDOffice { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_OST_OfficeGate 
    {
		...
		public static IQueryable<DTO_OST_OfficeGate> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS OST_OfficeGate";
            ...
            if (workBook != null)
            {
                ...
                //var OST_OfficeList = BS_OST_Office.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_OST_OfficeGate getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_OST_OfficeGate.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_OST_OfficeGate getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_OST_OfficeGate
        ...
        {
            bool result = false;
            var dbitem = db.tbl_OST_OfficeGate.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_OST_OfficeGate",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_OST_OfficeGate dbitem, string Username)
        {
            Type type = typeof(tbl_OST_OfficeGate);
        ...
        public static void patchDTOtoDBValue( DTO_OST_OfficeGate item, tbl_OST_OfficeGate dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IsVerifyLocation = item.IsVerifyLocation;							
			dbitem.Lat = item.Lat;							
			dbitem.Long = item.Long;							
			dbitem.MaxDistance = item.MaxDistance;        }

		public static DTO_OST_OfficeGate post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_OST_OfficeGate dbitem = new tbl_OST_OfficeGate();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_OST_OfficeGate.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_OST_OfficeGate",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_OST_OfficeGate();
                    ...
                    {
                        dbitem = new tbl_OST_OfficeGate();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_OST_OfficeGate.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_OST_OfficeGate.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_OST_OfficeGate.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_OST_OfficeGate", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_OST_OfficeGate",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_OST_OfficeGate",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_OST_OfficeGate.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_OST_OfficeGate.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_OST_OfficeGate> toDTO(IQueryable<tbl_OST_OfficeGate> query)
        {
			return query
			.Select(s => new DTO_OST_OfficeGate(){							
				IDOffice = s.IDOffice,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				...
				IsVerifyLocation = s.IsVerifyLocation,							
				Lat = s.Lat,							
				Long = s.Long,							
		...
		public static DTO_OST_OfficeGate toDTO(tbl_OST_OfficeGate dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_OST_OfficeGate()
				{							
					IDOffice = dbResult.IDOffice,							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IsVerifyLocation = dbResult.IsVerifyLocation,							
					Lat = dbResult.Lat,							
					Long = dbResult.Long,							
					MaxDistance = dbResult.MaxDistance,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_OST_OfficeGate> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_OST_OfficeGate> query = db.tbl_OST_OfficeGate.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query IDOffice (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDOffice"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDOffice").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDOffice));
////                    query = query.Where(d => IDs.Contains(d.IDOffice));
//                    
            }
			...
			//Query IsVerifyLocation (bool)
			if (QueryStrings.Any(d => d.Key == "IsVerifyLocation"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "IsVerifyLocation").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.IsVerifyLocation);
            }
			...
			//Query MaxDistance (int)
			if (QueryStrings.Any(d => d.Key == "MaxDistanceFrom") && QueryStrings.Any(d => d.Key == "MaxDistanceTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "MaxDistanceFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "MaxDistanceTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.MaxDistance && d.MaxDistance <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "MaxDistance"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "MaxDistance").Value, out int val))
                    query = query.Where(d => val == d.MaxDistance);

            return query;
        }
		
        public static IQueryable<tbl_OST_OfficeGate> _sortBuilder(IQueryable<tbl_OST_OfficeGate> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_OST_OfficeGate>;
                        bool isOrdered = ordered.ToString().IndexOf("ORDER BY") !=-1;

                        switch (item)
                        {
							case "IDOffice":
								query = isOrdered ? ordered.ThenBy(o => o.IDOffice) : query.OrderBy(o => o.IDOffice);
								 break;
							case "IDOffice_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDOffice) : query.OrderByDescending(o => o.IDOffice);
                                break;
							case "Id":
							...
							case "IPAddress_desc":
                                ...
                                break;
							case "IsVerifyLocation":
								query = isOrdered ? ordered.ThenBy(o => o.IsVerifyLocation) : query.OrderBy(o => o.IsVerifyLocation);
								 break;
							case "IsVerifyLocation_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IsVerifyLocation) : query.OrderByDescending(o => o.IsVerifyLocation);
                                break;
							case "Lat":
							...
							case "Long_desc":
                                ...
                                break;
							case "MaxDistance":
								query = isOrdered ? ordered.ThenBy(o => o.MaxDistance) : query.OrderBy(o => o.MaxDistance);
								 break;
							case "MaxDistance_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.MaxDistance) : query.OrderByDescending(o => o.MaxDistance);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_OST_OfficeGate> _pagingBuilder(IQueryable<tbl_OST_OfficeGate> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
