...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffBounusOnSalary
    {
        ...
        public Nullable<int> IDBounus { get; set; }
        public Nullable<System.DateTime> AriseDay { get; set; }
        public Nullable<int> IDPayPeriod { get; set; }
        public Nullable<decimal> BounusRate { get; set; }
        public Nullable<bool> IsExecuted { get; set; }
        public Nullable<bool> IsAutomatedAddedToSalary { get; set; }
        //List 0:1
...
{
	using System;
	public partial class DTO_HRM_StaffBounusOnSalary
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffBounusOnSalary 
    {
		...
		public static IQueryable<DTO_HRM_StaffBounusOnSalary> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffBounusOnSalary";
		...
		public static DTO_HRM_StaffBounusOnSalary getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffBounusOnSalary.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffBounusOnSalary getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffBounusOnSalary
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffBounusOnSalary.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffBounusOnSalary",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffBounusOnSalary dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffBounusOnSalary);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffBounusOnSalary item, tbl_HRM_StaffBounusOnSalary dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDBounus = item.IDBounus;							
			dbitem.AriseDay = item.AriseDay;							
			dbitem.IDPayPeriod = item.IDPayPeriod;							
			dbitem.BounusRate = item.BounusRate;							
			dbitem.IsExecuted = item.IsExecuted;							
			dbitem.IsAutomatedAddedToSalary = item.IsAutomatedAddedToSalary;        }

		public static DTO_HRM_StaffBounusOnSalary post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffBounusOnSalary dbitem = new tbl_HRM_StaffBounusOnSalary();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffBounusOnSalary.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffBounusOnSalary",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffBounusOnSalary();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffBounusOnSalary();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffBounusOnSalary.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffBounusOnSalary.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffBounusOnSalary.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffBounusOnSalary", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffBounusOnSalary",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_StaffBounusOnSalary",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffBounusOnSalary.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffBounusOnSalary.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffBounusOnSalary> toDTO(IQueryable<tbl_HRM_StaffBounusOnSalary> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffBounusOnSalary(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				BounusRate = s.BounusRate,							
				IsExecuted = s.IsExecuted,							
				IsAutomatedAddedToSalary = s.IsAutomatedAddedToSalary,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_HRM_StaffBounusOnSalary toDTO(tbl_HRM_StaffBounusOnSalary dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffBounusOnSalary()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IDBounus = dbResult.IDBounus,							
					AriseDay = dbResult.AriseDay,							
					IDPayPeriod = dbResult.IDPayPeriod,							
					BounusRate = dbResult.BounusRate,							
					IsExecuted = dbResult.IsExecuted,							
					IsAutomatedAddedToSalary = dbResult.IsAutomatedAddedToSalary,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_HRM_StaffBounusOnSalary> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffBounusOnSalary> query = db.tbl_HRM_StaffBounusOnSalary.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query IDBounus (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDBounus"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDBounus").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDBounus));
////                    query = query.Where(d => IDs.Contains(d.IDBounus));
//                    
            }
...
//                    
			...
			//Query BounusRate (Nullable<decimal>)
			if (QueryStrings.Any(d => d.Key == "BounusRateFrom") && QueryStrings.Any(d => d.Key == "BounusRateTo"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "BounusRateFrom").Value, out decimal fromVal) && decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "BounusRateTo").Value, out decimal toVal))
                    query = query.Where(d => fromVal <= d.BounusRate && d.BounusRate <= toVal);

			//Query IsExecuted (Nullable<bool>)

			//Query IsAutomatedAddedToSalary (Nullable<bool>)
            return query;
        }
		
        public static IQueryable<tbl_HRM_StaffBounusOnSalary> _sortBuilder(IQueryable<tbl_HRM_StaffBounusOnSalary> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffBounusOnSalary>;
                        ...
                        switch (item)
                        {
							...
							case "IDStaff_desc":
                                ...
                                break;
							case "IDBounus":
								query = isOrdered ? ordered.ThenBy(o => o.IDBounus) : query.OrderBy(o => o.IDBounus);
								 break;
							case "IDBounus_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDBounus) : query.OrderByDescending(o => o.IDBounus);
                                break;
							case "AriseDay":
							...
							case "IDPayPeriod_desc":
                                ...
                                break;
							case "BounusRate":
								query = isOrdered ? ordered.ThenBy(o => o.BounusRate) : query.OrderBy(o => o.BounusRate);
								 break;
							case "BounusRate_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.BounusRate) : query.OrderByDescending(o => o.BounusRate);
                                break;
							case "IsExecuted":
							...
							case "IsExecuted_desc":
                                ...
                                break;
							case "IsAutomatedAddedToSalary":
								query = isOrdered ? ordered.ThenBy(o => o.IsAutomatedAddedToSalary) : query.OrderBy(o => o.IsAutomatedAddedToSalary);
								 break;
							case "IsAutomatedAddedToSalary_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IsAutomatedAddedToSalary) : query.OrderByDescending(o => o.IsAutomatedAddedToSalary);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_StaffBounusOnSalary> _pagingBuilder(IQueryable<tbl_HRM_StaffBounusOnSalary> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
