...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffResignationInfo
    {
        ...
        public Nullable<int> IDReasonForResign { get; set; }
...
{
	using System;
	public partial class DTO_HRM_StaffResignationInfo
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffResignationInfo 
    {
		...
		public static IQueryable<DTO_HRM_StaffResignationInfo> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffResignationInfo";
		...
		public static DTO_HRM_StaffResignationInfo getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffResignationInfo.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffResignationInfo getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffResignationInfo
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffResignationInfo.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffResignationInfo",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffResignationInfo dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffResignationInfo);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffResignationInfo item, tbl_HRM_StaffResignationInfo dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.DateOfApplication = item.DateOfApplication;							
			dbitem.IDReasonForResign = item.IDReasonForResign;							
		...
		public static DTO_HRM_StaffResignationInfo post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffResignationInfo dbitem = new tbl_HRM_StaffResignationInfo();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffResignationInfo.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffResignationInfo",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffResignationInfo();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffResignationInfo();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffResignationInfo.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffResignationInfo.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffResignationInfo.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffResignationInfo", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffResignationInfo",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_StaffResignationInfo",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffResignationInfo.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffResignationInfo.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffResignationInfo> toDTO(IQueryable<tbl_HRM_StaffResignationInfo> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffResignationInfo(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				IDStaff = s.IDStaff,							
				DateOfApplication = s.DateOfApplication,							
				IDReasonForResign = s.IDReasonForResign,							
		...
		public static DTO_HRM_StaffResignationInfo toDTO(tbl_HRM_StaffResignationInfo dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffResignationInfo()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					DateOfApplication = dbResult.DateOfApplication,							
					IDReasonForResign = dbResult.IDReasonForResign,							
		...
		public static IQueryable<tbl_HRM_StaffResignationInfo> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffResignationInfo> query = db.tbl_HRM_StaffResignationInfo.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query DateOfApplication (Nullable<System.DateTime>)
			if (QueryStrings.Any(d => d.Key == "DateOfApplicationFrom") && QueryStrings.Any(d => d.Key == "DateOfApplicationTo"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "DateOfApplicationFrom").Value, out DateTime fromDate) && DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "DateOfApplicationTo").Value, out DateTime toDate))
                    query = query.Where(d => fromDate <= d.DateOfApplication && d.DateOfApplication <= toDate);

            if (QueryStrings.Any(d => d.Key == "DateOfApplication"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "DateOfApplication").Value, out DateTime val))
                    query = query.Where(d => d.DateOfApplication != null && DbFunctions.TruncateTime(val) == DbFunctions.TruncateTime(d.DateOfApplication));


			//Query IDReasonForResign (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDReasonForResign"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDReasonForResign").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDReasonForResign));
////                    query = query.Where(d => IDs.Contains(d.IDReasonForResign));
//                    
            }
...
//                    
        ...
        public static IQueryable<tbl_HRM_StaffResignationInfo> _sortBuilder(IQueryable<tbl_HRM_StaffResignationInfo> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffResignationInfo>;
                        ...
                        switch (item)
                        {
							...
							case "DateOfApplication":
								query = isOrdered ? ordered.ThenBy(o => o.DateOfApplication) : query.OrderBy(o => o.DateOfApplication);
								 break;
							case "DateOfApplication_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.DateOfApplication) : query.OrderByDescending(o => o.DateOfApplication);
                                break;
							case "IDReasonForResign":
								query = isOrdered ? ordered.ThenBy(o => o.IDReasonForResign) : query.OrderBy(o => o.IDReasonForResign);
								 break;
							case "IDReasonForResign_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDReasonForResign) : query.OrderByDescending(o => o.IDReasonForResign);
                                break;
							case "DecisionNumber":
        ...
        public static IQueryable<tbl_HRM_StaffResignationInfo> _pagingBuilder(IQueryable<tbl_HRM_StaffResignationInfo> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
