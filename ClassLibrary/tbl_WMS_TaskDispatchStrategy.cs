...
namespace ClassLibrary
{
    ...
    public partial class tbl_WMS_TaskDispatchStrategy
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_WMS_TaskDispatchStrategy()
        {
            this.tbl_WMS_TaskDispatchStrategyDetail = new HashSet<tbl_WMS_TaskDispatchStrategyDetail>();
        }
        ...
        public bool InterleaveTasks { get; set; }
        //List 0:1
        public virtual tbl_BRA_Branch tbl_BRA_Branch { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<tbl_WMS_TaskDispatchStrategyDetail> tbl_WMS_TaskDispatchStrategyDetail { get; set; }
    }
}
...
{
	using System;
	public partial class DTO_WMS_TaskDispatchStrategy
	{
		public int IDBranch { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_WMS_TaskDispatchStrategy 
    {
		...
		public static IQueryable<DTO_WMS_TaskDispatchStrategy> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS WMS_TaskDispatchStrategy";
		...
		public static DTO_WMS_TaskDispatchStrategy getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_WMS_TaskDispatchStrategy.Find(id);
		...
		public static DTO_WMS_TaskDispatchStrategy getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_WMS_TaskDispatchStrategy
        ...
        {
            bool result = false;
            var dbitem = db.tbl_WMS_TaskDispatchStrategy.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_WMS_TaskDispatchStrategy",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_WMS_TaskDispatchStrategy dbitem, string Username)
        {
            Type type = typeof(tbl_WMS_TaskDispatchStrategy);
        ...
        public static void patchDTOtoDBValue( DTO_WMS_TaskDispatchStrategy item, tbl_WMS_TaskDispatchStrategy dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.InterleaveTasks = item.InterleaveTasks;        }

		public static DTO_WMS_TaskDispatchStrategy post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_WMS_TaskDispatchStrategy dbitem = new tbl_WMS_TaskDispatchStrategy();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_WMS_TaskDispatchStrategy.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_WMS_TaskDispatchStrategy",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_WMS_TaskDispatchStrategy();
                    ...
                    {
                        dbitem = new tbl_WMS_TaskDispatchStrategy();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_WMS_TaskDispatchStrategy.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_WMS_TaskDispatchStrategy.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_WMS_TaskDispatchStrategy.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_WMS_TaskDispatchStrategy", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_WMS_TaskDispatchStrategy",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_WMS_TaskDispatchStrategy",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_WMS_TaskDispatchStrategy.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_WMS_TaskDispatchStrategy.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_WMS_TaskDispatchStrategy> toDTO(IQueryable<tbl_WMS_TaskDispatchStrategy> query)
        {
			return query
			.Select(s => new DTO_WMS_TaskDispatchStrategy(){							
				IDBranch = s.IDBranch,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				...
				InterleaveTasks = s.InterleaveTasks,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_WMS_TaskDispatchStrategy toDTO(tbl_WMS_TaskDispatchStrategy dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_WMS_TaskDispatchStrategy()
				{							
					...
					InterleaveTasks = dbResult.InterleaveTasks,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_WMS_TaskDispatchStrategy> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_WMS_TaskDispatchStrategy> query = db.tbl_WMS_TaskDispatchStrategy.AsNoTracking();//.Where(d => d.IsDeleted == false);
			...
			//Query InterleaveTasks (bool)
			if (QueryStrings.Any(d => d.Key == "InterleaveTasks"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "InterleaveTasks").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.InterleaveTasks);
            }
            return query;
        }
		
        public static IQueryable<tbl_WMS_TaskDispatchStrategy> _sortBuilder(IQueryable<tbl_WMS_TaskDispatchStrategy> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_WMS_TaskDispatchStrategy>;
                        ...
                        switch (item)
                        {
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "InterleaveTasks":
								query = isOrdered ? ordered.ThenBy(o => o.InterleaveTasks) : query.OrderBy(o => o.InterleaveTasks);
								 break;
							case "InterleaveTasks_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.InterleaveTasks) : query.OrderByDescending(o => o.InterleaveTasks);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_WMS_TaskDispatchStrategy> _pagingBuilder(IQueryable<tbl_WMS_TaskDispatchStrategy> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
