...
namespace ClassLibrary
{
    ...
    public partial class tbl_WMS_Adjustment
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_WMS_Adjustment()
        {
...
{
	using System;
	public partial class DTO_WMS_Adjustment
	{
		public int IDBranch { get; set; }
		public int IDStorer { get; set; }
		public int Id { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_WMS_Adjustment 
    {
		...
		public static IQueryable<DTO_WMS_Adjustment> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS WMS_Adjustment";
		...
		public static DTO_WMS_Adjustment getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_WMS_Adjustment.Find(id);
        ...
        {
            bool result = false;
            var dbitem = db.tbl_WMS_Adjustment.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_WMS_Adjustment",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_WMS_Adjustment dbitem, string Username)
        {
            Type type = typeof(tbl_WMS_Adjustment);
        ...
        public static void patchDTOtoDBValue( DTO_WMS_Adjustment item, tbl_WMS_Adjustment dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_WMS_Adjustment post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_WMS_Adjustment dbitem = new tbl_WMS_Adjustment();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_WMS_Adjustment.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_WMS_Adjustment",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_WMS_Adjustment();
                    ...
                    {
                        dbitem = new tbl_WMS_Adjustment();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_WMS_Adjustment.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_WMS_Adjustment.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_WMS_Adjustment.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_WMS_Adjustment", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_WMS_Adjustment",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_WMS_Adjustment",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_WMS_Adjustment.Any(e => e.Id == id && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_WMS_Adjustment> toDTO(IQueryable<tbl_WMS_Adjustment> query)
        {
			return query
			.Select(s => new DTO_WMS_Adjustment(){							
				IDBranch = s.IDBranch,							
				IDStorer = s.IDStorer,							
				Id = s.Id,							
				Status = s.Status,							
		...
		public static DTO_WMS_Adjustment toDTO(tbl_WMS_Adjustment dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_WMS_Adjustment()
				{							
		...
		public static IQueryable<tbl_WMS_Adjustment> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_WMS_Adjustment> query = db.tbl_WMS_Adjustment.AsNoTracking();//.Where(d => d.IsDeleted == false);
			...
			//Query Reason (string)
			if (QueryStrings.Any(d => d.Key == "Reason_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Reason_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Reason_eq").Value;
                query = query.Where(d=>d.Reason == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "Reason") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Reason").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Reason").Value;
                query = query.Where(d=>d.Reason.Contains(keyword));
            }
        ...
        public static IQueryable<tbl_WMS_Adjustment> _sortBuilder(IQueryable<tbl_WMS_Adjustment> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_WMS_Adjustment>;
                        ...
                        switch (item)
                        {
							...
							case "Status_desc":
                                ...
                                break;
							case "Reason":
								query = isOrdered ? ordered.ThenBy(o => o.Reason) : query.OrderBy(o => o.Reason);
								 break;
							case "Reason_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Reason) : query.OrderByDescending(o => o.Reason);
                                break;
							case "Remark":
        ...
        public static IQueryable<tbl_WMS_Adjustment> _pagingBuilder(IQueryable<tbl_WMS_Adjustment> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
