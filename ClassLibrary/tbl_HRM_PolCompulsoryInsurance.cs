...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_PolCompulsoryInsurance
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_HRM_PolCompulsoryInsurance()
        {
...
{
	using System;
	public partial class DTO_HRM_PolCompulsoryInsurance
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_PolCompulsoryInsurance 
    {
		...
		public static IQueryable<DTO_HRM_PolCompulsoryInsurance> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_PolCompulsoryInsurance";
		...
		public static DTO_HRM_PolCompulsoryInsurance getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_PolCompulsoryInsurance.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_PolCompulsoryInsurance getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_PolCompulsoryInsurance
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_PolCompulsoryInsurance.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_PolCompulsoryInsurance",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_PolCompulsoryInsurance dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_PolCompulsoryInsurance);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_PolCompulsoryInsurance item, tbl_HRM_PolCompulsoryInsurance dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_HRM_PolCompulsoryInsurance post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_PolCompulsoryInsurance dbitem = new tbl_HRM_PolCompulsoryInsurance();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_PolCompulsoryInsurance.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_PolCompulsoryInsurance",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_PolCompulsoryInsurance();
                    ...
                    {
                        dbitem = new tbl_HRM_PolCompulsoryInsurance();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_PolCompulsoryInsurance.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_PolCompulsoryInsurance.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_PolCompulsoryInsurance.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_PolCompulsoryInsurance", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_PolCompulsoryInsurance",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_PolCompulsoryInsurance",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_PolCompulsoryInsurance.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_PolCompulsoryInsurance.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_PolCompulsoryInsurance> toDTO(IQueryable<tbl_HRM_PolCompulsoryInsurance> query)
        {
			return query
			.Select(s => new DTO_HRM_PolCompulsoryInsurance(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
		...
		public static DTO_HRM_PolCompulsoryInsurance toDTO(tbl_HRM_PolCompulsoryInsurance dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_PolCompulsoryInsurance()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
		...
		public static IQueryable<tbl_HRM_PolCompulsoryInsurance> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_PolCompulsoryInsurance> query = db.tbl_HRM_PolCompulsoryInsurance.AsNoTracking();//.Where(d => d.IsDeleted == false );
        ...
        public static IQueryable<tbl_HRM_PolCompulsoryInsurance> _sortBuilder(IQueryable<tbl_HRM_PolCompulsoryInsurance> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_PolCompulsoryInsurance>;
        ...
        public static IQueryable<tbl_HRM_PolCompulsoryInsurance> _pagingBuilder(IQueryable<tbl_HRM_PolCompulsoryInsurance> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
