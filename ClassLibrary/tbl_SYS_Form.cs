...
namespace ClassLibrary
{
    ...
    public partial class tbl_SYS_Form
    {
        ...
        public tbl_SYS_Form()
        {
            this.tbl_SYS_Form1 = new HashSet<tbl_SYS_Form>();
            this.tbl_SYS_PermissionList = new HashSet<tbl_SYS_PermissionList>();
        }
    
        public Nullable<int> IDFormGroup { get; set; }
        ...
        public virtual ICollection<tbl_SYS_Form> tbl_SYS_Form1 { get; set; }
        //List 0:1
        public virtual tbl_SYS_Form tbl_SYS_Form2 { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
...
namespace BaseBusiness
{
    ...
    public static partial class BS_SYS_Form 
    {
		...
		public static IQueryable<DTO_SYS_Form> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            ...
            if (QueryStrings.Any(d => d.Key == "AllChildren")  || QueryStrings.Any(d => d.Key == "AllParent"))
            {
                ...
                List<DTO_SYS_Form> allItems = db.tbl_SYS_Form.Where(d => d.IsDeleted == false).Select(s => new DTO_SYS_Form()
                {
                    Id = s.Id,
                    IDParent = s.IDParent
                }).ToList();
                ...
                foreach (var item in query)
                {
                    ...
                    query = db.tbl_SYS_Form.Where(d => Ids.Contains(d.Id));
                }
        ...
        public static List<int> findParent(List<DTO_SYS_Form> allItems, int Id)
        {
        ...
        public static List<int> findChildrent(List<DTO_SYS_Form> allItems, int Id)
        {
            var Ids = new List<int>();
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS SYS_Form";
            ...
            if (workBook != null)
            {
                ...
                //var SYS_Form2List = BS_SYS_Form.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_SYS_Form getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
		...
		public static DTO_SYS_Form getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
        ...
        public static void patchDynamicToDB(dynamic item, tbl_SYS_Form dbitem, string Username)
        {
            Type type = typeof(tbl_SYS_Form);
        ...
        public static void patchDTOtoDBValue( DTO_SYS_Form item, tbl_SYS_Form dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_SYS_Form post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
        ...
        public static dynamic import(ExcelPackage package, AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings, string Username)
        {
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    {
                        dbitem = new tbl_SYS_Form();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_SYS_Form.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_SYS_Form.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_SYS_Form.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_SYS_Form", updateDate, Username);
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_SYS_Form",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_SYS_Form.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_SYS_Form.Any(e => e.Code == Code && e.IsDeleted == false);
		...
		public static IQueryable<tbl_SYS_Form> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_SYS_Form> query = db.tbl_SYS_Form.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query IDFormGroup (Nullable<int>)
...
//                    
			...
			//Query APIs (string)
			if (QueryStrings.Any(d => d.Key == "APIs_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "APIs_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "APIs_eq").Value;
                query = query.Where(d=>d.APIs == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "APIs") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "APIs").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "APIs").Value;
                query = query.Where(d=>d.APIs.Contains(keyword));
            }
			...
			//Query Icon (string)
			if (QueryStrings.Any(d => d.Key == "Icon_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Icon_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Icon_eq").Value;
            ...
            if (QueryStrings.Any(d => d.Key == "Icon") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Icon").Value))
            {
                ...
                query = query.Where(d=>d.Icon.Contains(keyword));
            }
            

			//Query BadgeColor (string)
			if (QueryStrings.Any(d => d.Key == "BadgeColor_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "BadgeColor_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "BadgeColor_eq").Value;
            ...
            if (QueryStrings.Any(d => d.Key == "BadgeColor") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "BadgeColor").Value))
            {
                ...
                query = query.Where(d=>d.BadgeColor.Contains(keyword));
            }
            
            return query;
        }
		
        public static IQueryable<tbl_SYS_Form> _sortBuilder(IQueryable<tbl_SYS_Form> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_SYS_Form>;
                        bool isOrdered = ordered.ToString().IndexOf("ORDER BY") !=-1;

                        switch (item)
                        {
							case "IDFormGroup":
								query = isOrdered ? ordered.ThenBy(o => o.IDFormGroup) : query.OrderBy(o => o.IDFormGroup);
								 break;
							case "IDFormGroup_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDFormGroup) : query.OrderByDescending(o => o.IDFormGroup);
                                break;
							case "IDParent":
							...
							case "Remark_desc":
                                ...
                                break;
							case "APIs":
								query = isOrdered ? ordered.ThenBy(o => o.APIs) : query.OrderBy(o => o.APIs);
								 break;
							case "APIs_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.APIs) : query.OrderByDescending(o => o.APIs);
                                break;
							case "Sort":
							...
							case "Type_desc":
                                ...
                                break;
							case "Icon":
								query = isOrdered ? ordered.ThenBy(o => o.Icon) : query.OrderBy(o => o.Icon);
								 break;
							case "Icon_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Icon) : query.OrderByDescending(o => o.Icon);
                                break;
							case "BadgeColor":
								query = isOrdered ? ordered.ThenBy(o => o.BadgeColor) : query.OrderBy(o => o.BadgeColor);
								 break;
							case "BadgeColor_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.BadgeColor) : query.OrderByDescending(o => o.BadgeColor);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_SYS_Form> _pagingBuilder(IQueryable<tbl_SYS_Form> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
