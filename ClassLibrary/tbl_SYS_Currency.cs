...
namespace ClassLibrary
{
    ...
    public partial class tbl_SYS_Currency
    {
...
{
	using System;
	public partial class DTO_SYS_Currency
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_SYS_Currency 
    {
		...
		public static IQueryable<DTO_SYS_Currency> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS SYS_Currency";
		...
		public static DTO_SYS_Currency getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_SYS_Currency.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_SYS_Currency getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_SYS_Currency
        ...
        {
            bool result = false;
            var dbitem = db.tbl_SYS_Currency.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_SYS_Currency",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_SYS_Currency dbitem, string Username)
        {
            Type type = typeof(tbl_SYS_Currency);
        ...
        public static void patchDTOtoDBValue( DTO_SYS_Currency item, tbl_SYS_Currency dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.Decimals = item.Decimals;        }

		public static DTO_SYS_Currency post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_SYS_Currency dbitem = new tbl_SYS_Currency();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_SYS_Currency.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_SYS_Currency",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_SYS_Currency();
                    ...
                    {
                        dbitem = new tbl_SYS_Currency();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_SYS_Currency.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_SYS_Currency.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_SYS_Currency.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_SYS_Currency", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_SYS_Currency",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_SYS_Currency",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_SYS_Currency.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_SYS_Currency.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_SYS_Currency> toDTO(IQueryable<tbl_SYS_Currency> query)
        {
			return query
			.Select(s => new DTO_SYS_Currency(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
		...
		public static DTO_SYS_Currency toDTO(tbl_SYS_Currency dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_SYS_Currency()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
		...
		public static IQueryable<tbl_SYS_Currency> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_SYS_Currency> query = db.tbl_SYS_Currency.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query Decimals (int)
			if (QueryStrings.Any(d => d.Key == "DecimalsFrom") && QueryStrings.Any(d => d.Key == "DecimalsTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "DecimalsFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "DecimalsTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.Decimals && d.Decimals <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "Decimals"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "Decimals").Value, out int val))
                    query = query.Where(d => val == d.Decimals);

            return query;
        }
		
        public static IQueryable<tbl_SYS_Currency> _sortBuilder(IQueryable<tbl_SYS_Currency> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_SYS_Currency>;
                        ...
                        switch (item)
                        {
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "Decimals":
								query = isOrdered ? ordered.ThenBy(o => o.Decimals) : query.OrderBy(o => o.Decimals);
								 break;
							case "Decimals_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Decimals) : query.OrderByDescending(o => o.Decimals);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_SYS_Currency> _pagingBuilder(IQueryable<tbl_SYS_Currency> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
