...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffPTOEnrollment
    {
        public int IDPolicyPTO { get; set; }
...
{
	using System;
	public partial class DTO_HRM_StaffPTOEnrollment
	{
		public int IDPolicyPTO { get; set; }
		public int IDStaff { get; set; }
		public Nullable<System.DateTime> StartDate { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffPTOEnrollment 
    {
		...
		public static IQueryable<DTO_HRM_StaffPTOEnrollment> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffPTOEnrollment";
		...
		public static DTO_HRM_StaffPTOEnrollment getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffPTOEnrollment.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffPTOEnrollment getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffPTOEnrollment
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffPTOEnrollment.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffPTOEnrollment",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffPTOEnrollment dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffPTOEnrollment);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffPTOEnrollment item, tbl_HRM_StaffPTOEnrollment dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDPolicyPTO = item.IDPolicyPTO;							
		...
		public static DTO_HRM_StaffPTOEnrollment post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffPTOEnrollment dbitem = new tbl_HRM_StaffPTOEnrollment();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffPTOEnrollment.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffPTOEnrollment",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffPTOEnrollment();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffPTOEnrollment();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffPTOEnrollment.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffPTOEnrollment.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffPTOEnrollment.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffPTOEnrollment", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffPTOEnrollment",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_StaffPTOEnrollment",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffPTOEnrollment.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffPTOEnrollment.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffPTOEnrollment> toDTO(IQueryable<tbl_HRM_StaffPTOEnrollment> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffPTOEnrollment(){							
				IDPolicyPTO = s.IDPolicyPTO,							
				IDStaff = s.IDStaff,							
		...
		public static DTO_HRM_StaffPTOEnrollment toDTO(tbl_HRM_StaffPTOEnrollment dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffPTOEnrollment()
				{							
					IDPolicyPTO = dbResult.IDPolicyPTO,							
		...
		public static IQueryable<tbl_HRM_StaffPTOEnrollment> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffPTOEnrollment> query = db.tbl_HRM_StaffPTOEnrollment.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query IDPolicyPTO (int)
			if (QueryStrings.Any(d => d.Key == "IDPolicyPTO"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDPolicyPTO").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDPolicyPTO));
            }
        ...
        public static IQueryable<tbl_HRM_StaffPTOEnrollment> _sortBuilder(IQueryable<tbl_HRM_StaffPTOEnrollment> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffPTOEnrollment>;
                        bool isOrdered = ordered.ToString().IndexOf("ORDER BY") !=-1;

                        switch (item)
                        {
							case "IDPolicyPTO":
								query = isOrdered ? ordered.ThenBy(o => o.IDPolicyPTO) : query.OrderBy(o => o.IDPolicyPTO);
								 break;
							case "IDPolicyPTO_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDPolicyPTO) : query.OrderByDescending(o => o.IDPolicyPTO);
                                break;
							case "IDStaff":
        ...
        public static IQueryable<tbl_HRM_StaffPTOEnrollment> _pagingBuilder(IQueryable<tbl_HRM_StaffPTOEnrollment> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
