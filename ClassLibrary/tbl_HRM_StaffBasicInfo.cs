...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffBasicInfo
    {
        ...
        public Nullable<int> IDEthnic { get; set; }
...
{
	using System;
	public partial class DTO_HRM_StaffBasicInfo
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffBasicInfo 
    {
		...
		public static IQueryable<DTO_HRM_StaffBasicInfo> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffBasicInfo";
		...
		public static DTO_HRM_StaffBasicInfo getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffBasicInfo.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffBasicInfo getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffBasicInfo
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffBasicInfo.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffBasicInfo",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffBasicInfo dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffBasicInfo);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffBasicInfo item, tbl_HRM_StaffBasicInfo dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDNationality = item.IDNationality;							
			dbitem.IDEthnic = item.IDEthnic;							
			dbitem.IDReligion = item.IDReligion;							
		...
		public static DTO_HRM_StaffBasicInfo post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffBasicInfo dbitem = new tbl_HRM_StaffBasicInfo();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffBasicInfo.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffBasicInfo",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffBasicInfo();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffBasicInfo();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffBasicInfo.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffBasicInfo.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffBasicInfo.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffBasicInfo", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffBasicInfo",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffBasicInfo.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffBasicInfo.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffBasicInfo> toDTO(IQueryable<tbl_HRM_StaffBasicInfo> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffBasicInfo(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				IDNationality = s.IDNationality,							
				IDEthnic = s.IDEthnic,							
				IDReligion = s.IDReligion,							
		...
		public static DTO_HRM_StaffBasicInfo toDTO(tbl_HRM_StaffBasicInfo dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffBasicInfo()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IDNationality = dbResult.IDNationality,							
					IDEthnic = dbResult.IDEthnic,							
					IDReligion = dbResult.IDReligion,							
		...
		public static IQueryable<tbl_HRM_StaffBasicInfo> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffBasicInfo> query = db.tbl_HRM_StaffBasicInfo.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query IDNationality (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDNationality"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDNationality").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDNationality));
////                    query = query.Where(d => IDs.Contains(d.IDNationality));
//                    
            }

			//Query IDEthnic (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDEthnic"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDEthnic").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDEthnic));
////                    query = query.Where(d => IDs.Contains(d.IDEthnic));
//                    
            }

			//Query IDReligion (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDReligion"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDReligion").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDReligion));
////                    query = query.Where(d => IDs.Contains(d.IDReligion));
//                    
            }

			//Query MaritalStatus (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "MaritalStatusFrom") && QueryStrings.Any(d => d.Key == "MaritalStatusTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "MaritalStatusFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "MaritalStatusTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.MaritalStatus && d.MaritalStatus <= toVal);
			...
			//Query POB (string)
			if (QueryStrings.Any(d => d.Key == "POB_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "POB_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "POB_eq").Value;
                query = query.Where(d=>d.POB == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "POB") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "POB").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "POB").Value;
                query = query.Where(d=>d.POB.Contains(keyword));
            }
        ...
        public static IQueryable<tbl_HRM_StaffBasicInfo> _sortBuilder(IQueryable<tbl_HRM_StaffBasicInfo> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffBasicInfo>;
                        ...
                        switch (item)
                        {
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "IDNationality":
								query = isOrdered ? ordered.ThenBy(o => o.IDNationality) : query.OrderBy(o => o.IDNationality);
								 break;
							case "IDNationality_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDNationality) : query.OrderByDescending(o => o.IDNationality);
                                break;
							case "IDEthnic":
								query = isOrdered ? ordered.ThenBy(o => o.IDEthnic) : query.OrderBy(o => o.IDEthnic);
								 break;
							case "IDEthnic_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDEthnic) : query.OrderByDescending(o => o.IDEthnic);
                                break;
							case "IDReligion":
								query = isOrdered ? ordered.ThenBy(o => o.IDReligion) : query.OrderBy(o => o.IDReligion);
								 break;
							case "IDReligion_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDReligion) : query.OrderByDescending(o => o.IDReligion);
                                break;
							case "MaritalStatus":
							...
							case "Domicile_desc":
                                ...
                                break;
							case "POB":
								query = isOrdered ? ordered.ThenBy(o => o.POB) : query.OrderBy(o => o.POB);
								 break;
							case "POB_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.POB) : query.OrderByDescending(o => o.POB);
                                break;
							case "IsDeleted":
        ...
        public static IQueryable<tbl_HRM_StaffBasicInfo> _pagingBuilder(IQueryable<tbl_HRM_StaffBasicInfo> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
