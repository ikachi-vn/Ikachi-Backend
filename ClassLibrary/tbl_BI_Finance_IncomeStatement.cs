...
namespace ClassLibrary
{
    ...
    public partial class tbl_BI_Finance_IncomeStatement
    {
        public int IDBranch { get; set; }
        public int IDIncomeComponent { get; set; }
...
{
	using System;
	public partial class DTO_BI_Finance_IncomeStatement
	{
		public int IDBranch { get; set; }
		public int IDIncomeComponent { get; set; }
		public int Id { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_BI_Finance_IncomeStatement 
    {
		...
		public static IQueryable<DTO_BI_Finance_IncomeStatement> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS BI_Finance_IncomeStatement";
		...
		public static DTO_BI_Finance_IncomeStatement getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_BI_Finance_IncomeStatement.Find(id);
        ...
        {
            bool result = false;
            var dbitem = db.tbl_BI_Finance_IncomeStatement.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_BI_Finance_IncomeStatement",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_BI_Finance_IncomeStatement dbitem, string Username)
        {
            Type type = typeof(tbl_BI_Finance_IncomeStatement);
        ...
        public static void patchDTOtoDBValue( DTO_BI_Finance_IncomeStatement item, tbl_BI_Finance_IncomeStatement dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDIncomeComponent = item.IDIncomeComponent;							
		...
		public static DTO_BI_Finance_IncomeStatement post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_BI_Finance_IncomeStatement dbitem = new tbl_BI_Finance_IncomeStatement();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_BI_Finance_IncomeStatement.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_BI_Finance_IncomeStatement",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_BI_Finance_IncomeStatement();
                    ...
                    {
                        dbitem = new tbl_BI_Finance_IncomeStatement();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_BI_Finance_IncomeStatement.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_BI_Finance_IncomeStatement.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_BI_Finance_IncomeStatement.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_BI_Finance_IncomeStatement", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_BI_Finance_IncomeStatement",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_BI_Finance_IncomeStatement.Any(e => e.Id == id && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_BI_Finance_IncomeStatement> toDTO(IQueryable<tbl_BI_Finance_IncomeStatement> query)
        {
			return query
			.Select(s => new DTO_BI_Finance_IncomeStatement(){							
				IDBranch = s.IDBranch,							
				IDIncomeComponent = s.IDIncomeComponent,							
				Id = s.Id,							
		...
		public static DTO_BI_Finance_IncomeStatement toDTO(tbl_BI_Finance_IncomeStatement dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_BI_Finance_IncomeStatement()
				{							
					IDBranch = dbResult.IDBranch,							
					IDIncomeComponent = dbResult.IDIncomeComponent,							
					Id = dbResult.Id,							
		...
		public static IQueryable<tbl_BI_Finance_IncomeStatement> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_BI_Finance_IncomeStatement> query = db.tbl_BI_Finance_IncomeStatement.AsNoTracking();//.Where(d => d.IsDeleted == false);
			...
			//Query IDIncomeComponent (int)
			if (QueryStrings.Any(d => d.Key == "IDIncomeComponent"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDIncomeComponent").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDIncomeComponent));
            }
        ...
        public static IQueryable<tbl_BI_Finance_IncomeStatement> _sortBuilder(IQueryable<tbl_BI_Finance_IncomeStatement> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_BI_Finance_IncomeStatement>;
                        ...
                        switch (item)
                        {
							...
							case "IDBranch_desc":
                                ...
                                break;
							case "IDIncomeComponent":
								query = isOrdered ? ordered.ThenBy(o => o.IDIncomeComponent) : query.OrderBy(o => o.IDIncomeComponent);
								 break;
							case "IDIncomeComponent_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDIncomeComponent) : query.OrderByDescending(o => o.IDIncomeComponent);
                                break;
							case "Id":
        ...
        public static IQueryable<tbl_BI_Finance_IncomeStatement> _pagingBuilder(IQueryable<tbl_BI_Finance_IncomeStatement> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
