...
namespace ClassLibrary
{
    ...
    public partial class tbl_WH_Branches
    {
...
{
	using System;
	public partial class DTO_WH_Branches
	{
		public int Id { get; set; }
		public Nullable<int> IDParent { get; set; }
		public Nullable<int> IDType { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_WH_Branches 
    {
		...
		public static IQueryable<DTO_WH_Branches> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            ...
            if (QueryStrings.Any(d => d.Key == "AllChildren")  || QueryStrings.Any(d => d.Key == "AllParent"))
            {
                ...
                List<DTO_WH_Branches> allItems = db.tbl_WH_Branches.Where(d => d.IsDeleted == false).Select(s => new DTO_WH_Branches()
                {
                    Id = s.Id,
                    IDParent = s.IDParent
                }).ToList();
                ...
                foreach (var item in query)
                {
                    ...
                    query = db.tbl_WH_Branches.Where(d => Ids.Contains(d.Id));
                }
        ...
        public static List<int> findParent(List<DTO_WH_Branches> allItems, int Id)
        {
        ...
        public static List<int> findChildrent(List<DTO_WH_Branches> allItems, int Id)
        {
            var Ids = new List<int>();
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS WH_Branches";
		...
		public static DTO_WH_Branches getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_WH_Branches.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_WH_Branches getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_WH_Branches
        ...
        {
            bool result = false;
            var dbitem = db.tbl_WH_Branches.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_WH_Branches",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_WH_Branches dbitem, string Username)
        {
            Type type = typeof(tbl_WH_Branches);
        ...
        public static void patchDTOtoDBValue( DTO_WH_Branches item, tbl_WH_Branches dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_WH_Branches post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_WH_Branches dbitem = new tbl_WH_Branches();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_WH_Branches.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_WH_Branches",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_WH_Branches();
                    ...
                    {
                        dbitem = new tbl_WH_Branches();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_WH_Branches.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_WH_Branches.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_WH_Branches.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_WH_Branches", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_WH_Branches",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_WH_Branches",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_WH_Branches.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_WH_Branches.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_WH_Branches> toDTO(IQueryable<tbl_WH_Branches> query)
        {
			return query
			.Select(s => new DTO_WH_Branches(){							
				Id = s.Id,							
				IDParent = s.IDParent,							
				IDType = s.IDType,							
				Code = s.Code,							
		...
		public static DTO_WH_Branches toDTO(tbl_WH_Branches dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_WH_Branches()
				{							
					Id = dbResult.Id,							
		...
		public static IQueryable<tbl_WH_Branches> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_WH_Branches> query = db.tbl_WH_Branches.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
        ...
        public static IQueryable<tbl_WH_Branches> _sortBuilder(IQueryable<tbl_WH_Branches> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_WH_Branches>;
        ...
        public static IQueryable<tbl_WH_Branches> _pagingBuilder(IQueryable<tbl_WH_Branches> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
