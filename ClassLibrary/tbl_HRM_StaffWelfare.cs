...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffWelfare
    {
        ...
        public Nullable<int> IDPOLWelfareApplyFor { get; set; }
        //List 0:1
        public virtual tbl_HRM_PolWelfareApplyFor tbl_HRM_PolWelfareApplyFor { get; set; }
        //List 0:1
...
{
	using System;
	public partial class DTO_HRM_StaffWelfare
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffWelfare 
    {
		...
		public static IQueryable<DTO_HRM_StaffWelfare> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffWelfare";
            ...
            if (workBook != null)
            {
                ...
                //var HRM_PolWelfareApplyForList = BS_HRM_PolWelfareApplyFor.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_HRM_StaffWelfare getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffWelfare.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffWelfare getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffWelfare
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffWelfare.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffWelfare",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffWelfare dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffWelfare);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffWelfare item, tbl_HRM_StaffWelfare dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDPOLWelfareApplyFor = item.IDPOLWelfareApplyFor;        }

		public static DTO_HRM_StaffWelfare post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffWelfare dbitem = new tbl_HRM_StaffWelfare();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffWelfare.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffWelfare",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffWelfare();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffWelfare();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffWelfare.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffWelfare.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffWelfare.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffWelfare", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffWelfare",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_StaffWelfare",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffWelfare.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffWelfare.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffWelfare> toDTO(IQueryable<tbl_HRM_StaffWelfare> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffWelfare(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				IDPOLWelfareApplyFor = s.IDPOLWelfareApplyFor,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_HRM_StaffWelfare toDTO(tbl_HRM_StaffWelfare dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffWelfare()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IDPOLWelfareApplyFor = dbResult.IDPOLWelfareApplyFor,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_HRM_StaffWelfare> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffWelfare> query = db.tbl_HRM_StaffWelfare.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query IDPOLWelfareApplyFor (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDPOLWelfareApplyFor"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDPOLWelfareApplyFor").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDPOLWelfareApplyFor));
////                    query = query.Where(d => IDs.Contains(d.IDPOLWelfareApplyFor));
//                    
            }
            return query;
        ...
        public static IQueryable<tbl_HRM_StaffWelfare> _sortBuilder(IQueryable<tbl_HRM_StaffWelfare> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffWelfare>;
                        ...
                        switch (item)
                        {
							...
							case "IsHasRunSalary_desc":
                                ...
                                break;
							case "IDPOLWelfareApplyFor":
								query = isOrdered ? ordered.ThenBy(o => o.IDPOLWelfareApplyFor) : query.OrderBy(o => o.IDPOLWelfareApplyFor);
								 break;
							case "IDPOLWelfareApplyFor_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDPOLWelfareApplyFor) : query.OrderByDescending(o => o.IDPOLWelfareApplyFor);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_StaffWelfare> _pagingBuilder(IQueryable<tbl_HRM_StaffWelfare> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
