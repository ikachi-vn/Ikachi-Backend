...
namespace ClassLibrary
{
    ...
    public partial class tbl_SHIP_Shipment
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_SHIP_Shipment()
        {
        ...
        public string ConfirmFinishedBy { get; set; }
        public Nullable<System.DateTime> ConfirmFinishedDate { get; set; }
        public bool IsLostMoney { get; set; }
        ...
        //List 0:1
        public virtual tbl_SHIP_Vehicle tbl_SHIP_Vehicle { get; set; }
        //List 0:1
...
namespace BaseBusiness
{
    ...
    public static partial class BS_SHIP_Shipment 
    {
		...
		public static IQueryable<DTO_SHIP_Shipment> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS SHIP_Shipment";
            ...
            if (workBook != null)
            {
                ...
                //var SHIP_VehicleList = BS_SHIP_Vehicle.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_SHIP_Shipment getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_SHIP_Shipment.Find(id);
		...
		public static DTO_SHIP_Shipment getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_SHIP_Shipment
		...
		public static bool put(AppEntities db, int IDBranch, int StaffID, int Id, dynamic item, string Username, Dictionary<string, string> QueryStrings)
        {
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_SHIP_Shipment",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_SHIP_Shipment dbitem, string Username)
        {
            Type type = typeof(tbl_SHIP_Shipment);
        ...
        public static void patchDTOtoDBValue( DTO_SHIP_Shipment item, tbl_SHIP_Shipment dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDVehicle = item.IDVehicle;							
			dbitem.IDShipper = item.IDShipper;							
			dbitem.IDWarehouse = item.IDWarehouse;							
			dbitem.Code = item.Code;							
			dbitem.Name = item.Name;							
			dbitem.Remark = item.Remark;							
			dbitem.DeliveryDate = item.DeliveryDate;							
			dbitem.ShippedDate = item.ShippedDate;							
			dbitem.Sort = item.Sort;							
			dbitem.IsDisabled = item.IsDisabled;							
			dbitem.IsDeleted = item.IsDeleted;							
			dbitem.ConfirmFinishedBy = item.ConfirmFinishedBy;							
			dbitem.ConfirmFinishedDate = item.ConfirmFinishedDate;							
			dbitem.IsLostMoney = item.IsLostMoney;							
			dbitem.LostAmount = item.LostAmount;							
			dbitem.LostRemark = item.LostRemark;        }

		public static DTO_SHIP_Shipment post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_SHIP_Shipment dbitem = new tbl_SHIP_Shipment();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                {
					errorLog.logMessage("post_SHIP_Shipment",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_SHIP_Shipment();
                    ...
                    {
                        dbitem = db.tbl_SHIP_Shipment.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_SHIP_Shipment.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_SHIP_Shipment", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_SHIP_Shipment",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_SHIP_Shipment",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_SHIP_Shipment.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_SHIP_Shipment.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_SHIP_Shipment> toDTO(IQueryable<tbl_SHIP_Shipment> query)
        {
			return query
			.Select(s => new DTO_SHIP_Shipment(){							
				IDBranch = s.IDBranch,							
				...
				ConfirmFinishedBy = s.ConfirmFinishedBy,							
				ConfirmFinishedDate = s.ConfirmFinishedDate,							
				IsLostMoney = s.IsLostMoney,							
				LostAmount = s.LostAmount,							
				LostRemark = s.LostRemark,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_SHIP_Shipment toDTO(tbl_SHIP_Shipment dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_SHIP_Shipment()
				{							
					IDBranch = dbResult.IDBranch,							
					IDContact = dbResult.IDContact,							
					IDStatus = dbResult.IDStatus,							
					IDType = dbResult.IDType,							
					IDVehicle = dbResult.IDVehicle,							
					...
					DeliveryDate = dbResult.DeliveryDate,							
					...
					ConfirmFinishedBy = dbResult.ConfirmFinishedBy,							
					ConfirmFinishedDate = dbResult.ConfirmFinishedDate,							
					IsLostMoney = dbResult.IsLostMoney,							
					LostAmount = dbResult.LostAmount,							
					LostRemark = dbResult.LostRemark,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_SHIP_Shipment> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_SHIP_Shipment> query = db.tbl_SHIP_Shipment.AsNoTracking();//.Where(d => d.IsDeleted == false);
...
//                    
			...
			//Query IDVehicle (int)
			if (QueryStrings.Any(d => d.Key == "IDVehicle"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDVehicle").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDVehicle));
            }
...
//                    
			...
			//Query IDWarehouse (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDWarehouse"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDWarehouse").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDWarehouse));
////                    query = query.Where(d => IDs.Contains(d.IDWarehouse));
//                    
            }
			...
			//Query DeliveryDate (System.DateTime)
			if (QueryStrings.Any(d => d.Key == "DeliveryDateFrom") && QueryStrings.Any(d => d.Key == "DeliveryDateTo"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "DeliveryDateFrom").Value, out DateTime fromDate) && DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "DeliveryDateTo").Value, out DateTime toDate))
                    query = query.Where(d => fromDate <= d.DeliveryDate && d.DeliveryDate <= toDate);

            if (QueryStrings.Any(d => d.Key == "DeliveryDate"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "DeliveryDate").Value, out DateTime val))
                    query = query.Where(d => d.DeliveryDate != null && DbFunctions.TruncateTime(val) == DbFunctions.TruncateTime(d.DeliveryDate));
			...
			//Query ConfirmFinishedBy (string)
			if (QueryStrings.Any(d => d.Key == "ConfirmFinishedBy_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "ConfirmFinishedBy_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "ConfirmFinishedBy_eq").Value;
                query = query.Where(d=>d.ConfirmFinishedBy == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "ConfirmFinishedBy") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "ConfirmFinishedBy").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "ConfirmFinishedBy").Value;
                query = query.Where(d=>d.ConfirmFinishedBy.Contains(keyword));
            }
            

			//Query ConfirmFinishedDate (Nullable<System.DateTime>)
			if (QueryStrings.Any(d => d.Key == "ConfirmFinishedDateFrom") && QueryStrings.Any(d => d.Key == "ConfirmFinishedDateTo"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ConfirmFinishedDateFrom").Value, out DateTime fromDate) && DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ConfirmFinishedDateTo").Value, out DateTime toDate))
                    query = query.Where(d => fromDate <= d.ConfirmFinishedDate && d.ConfirmFinishedDate <= toDate);

            if (QueryStrings.Any(d => d.Key == "ConfirmFinishedDate"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ConfirmFinishedDate").Value, out DateTime val))
                    query = query.Where(d => d.ConfirmFinishedDate != null && DbFunctions.TruncateTime(val) == DbFunctions.TruncateTime(d.ConfirmFinishedDate));


			//Query IsLostMoney (bool)
			if (QueryStrings.Any(d => d.Key == "IsLostMoney"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "IsLostMoney").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.IsLostMoney);
            }

			//Query LostAmount (decimal)
			if (QueryStrings.Any(d => d.Key == "LostAmountFrom") && QueryStrings.Any(d => d.Key == "LostAmountTo"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "LostAmountFrom").Value, out decimal fromVal) && decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "LostAmountTo").Value, out decimal toVal))
                    query = query.Where(d => fromVal <= d.LostAmount && d.LostAmount <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "LostAmount"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "LostAmount").Value, out decimal val))
                    query = query.Where(d => val == d.LostAmount);


			//Query LostRemark (string)
			if (QueryStrings.Any(d => d.Key == "LostRemark_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "LostRemark_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "LostRemark_eq").Value;
                query = query.Where(d=>d.LostRemark == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "LostRemark") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "LostRemark").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "LostRemark").Value;
                query = query.Where(d=>d.LostRemark.Contains(keyword));
            }
            
            return query;
        }
		
        public static IQueryable<tbl_SHIP_Shipment> _sortBuilder(IQueryable<tbl_SHIP_Shipment> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_SHIP_Shipment>;
                        ...
                        switch (item)
                        {
							...
							case "IDType_desc":
                                ...
                                break;
							case "IDVehicle":
								query = isOrdered ? ordered.ThenBy(o => o.IDVehicle) : query.OrderBy(o => o.IDVehicle);
								 break;
							case "IDVehicle_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDVehicle) : query.OrderByDescending(o => o.IDVehicle);
                                break;
							case "IDShipper":
							...
							case "IDShipper_desc":
                                ...
                                break;
							case "IDWarehouse":
								query = isOrdered ? ordered.ThenBy(o => o.IDWarehouse) : query.OrderBy(o => o.IDWarehouse);
								 break;
							case "IDWarehouse_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDWarehouse) : query.OrderByDescending(o => o.IDWarehouse);
                                break;
							case "Id":
							...
							case "Remark_desc":
                                ...
                                break;
							case "DeliveryDate":
								query = isOrdered ? ordered.ThenBy(o => o.DeliveryDate) : query.OrderBy(o => o.DeliveryDate);
								 break;
							case "DeliveryDate_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.DeliveryDate) : query.OrderByDescending(o => o.DeliveryDate);
                                break;
							case "ShippedDate":
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "ConfirmFinishedBy":
								query = isOrdered ? ordered.ThenBy(o => o.ConfirmFinishedBy) : query.OrderBy(o => o.ConfirmFinishedBy);
								 break;
							case "ConfirmFinishedBy_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.ConfirmFinishedBy) : query.OrderByDescending(o => o.ConfirmFinishedBy);
                                break;
							case "ConfirmFinishedDate":
								query = isOrdered ? ordered.ThenBy(o => o.ConfirmFinishedDate) : query.OrderBy(o => o.ConfirmFinishedDate);
								 break;
							case "ConfirmFinishedDate_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.ConfirmFinishedDate) : query.OrderByDescending(o => o.ConfirmFinishedDate);
                                break;
							case "IsLostMoney":
								query = isOrdered ? ordered.ThenBy(o => o.IsLostMoney) : query.OrderBy(o => o.IsLostMoney);
								 break;
							case "IsLostMoney_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IsLostMoney) : query.OrderByDescending(o => o.IsLostMoney);
                                break;
							case "LostAmount":
								query = isOrdered ? ordered.ThenBy(o => o.LostAmount) : query.OrderBy(o => o.LostAmount);
								 break;
							case "LostAmount_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.LostAmount) : query.OrderByDescending(o => o.LostAmount);
                                break;
							case "LostRemark":
								query = isOrdered ? ordered.ThenBy(o => o.LostRemark) : query.OrderBy(o => o.LostRemark);
								 break;
							case "LostRemark_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.LostRemark) : query.OrderByDescending(o => o.LostRemark);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_SHIP_Shipment> _pagingBuilder(IQueryable<tbl_SHIP_Shipment> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
