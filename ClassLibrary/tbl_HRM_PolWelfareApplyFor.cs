...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_PolWelfareApplyFor
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_HRM_PolWelfareApplyFor()
        {
        ...
        public Nullable<int> IDPOLWelfare { get; set; }
        //List 0:1
        public virtual tbl_BRA_Branch tbl_BRA_Branch { get; set; }
        //List 0:1
        public virtual tbl_HRM_PolWelfare tbl_HRM_PolWelfare { get; set; }
        //List 0:1
...
{
	using System;
	public partial class DTO_HRM_PolWelfareApplyFor
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_PolWelfareApplyFor 
    {
		...
		public static IQueryable<DTO_HRM_PolWelfareApplyFor> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_PolWelfareApplyFor";
            ...
            if (workBook != null)
            {
                ...
                //var HRM_PolWelfareList = BS_HRM_PolWelfare.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_HRM_PolWelfareApplyFor getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_PolWelfareApplyFor.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_PolWelfareApplyFor getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_PolWelfareApplyFor
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_PolWelfareApplyFor.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_PolWelfareApplyFor",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_PolWelfareApplyFor dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_PolWelfareApplyFor);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_PolWelfareApplyFor item, tbl_HRM_PolWelfareApplyFor dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDPOLWelfare = item.IDPOLWelfare;        }

		public static DTO_HRM_PolWelfareApplyFor post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_PolWelfareApplyFor dbitem = new tbl_HRM_PolWelfareApplyFor();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_PolWelfareApplyFor.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_PolWelfareApplyFor",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_PolWelfareApplyFor();
                    ...
                    {
                        dbitem = new tbl_HRM_PolWelfareApplyFor();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_PolWelfareApplyFor.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_PolWelfareApplyFor.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_PolWelfareApplyFor.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_PolWelfareApplyFor", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_PolWelfareApplyFor",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_PolWelfareApplyFor",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_PolWelfareApplyFor.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_PolWelfareApplyFor.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_PolWelfareApplyFor> toDTO(IQueryable<tbl_HRM_PolWelfareApplyFor> query)
        {
			return query
			.Select(s => new DTO_HRM_PolWelfareApplyFor(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				IDPOLWelfare = s.IDPOLWelfare,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_HRM_PolWelfareApplyFor toDTO(tbl_HRM_PolWelfareApplyFor dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_PolWelfareApplyFor()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IDPOLWelfare = dbResult.IDPOLWelfare,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_HRM_PolWelfareApplyFor> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_PolWelfareApplyFor> query = db.tbl_HRM_PolWelfareApplyFor.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query IDPOLWelfare (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDPOLWelfare"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDPOLWelfare").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDPOLWelfare));
////                    query = query.Where(d => IDs.Contains(d.IDPOLWelfare));
//                    
            }
            return query;
        ...
        public static IQueryable<tbl_HRM_PolWelfareApplyFor> _sortBuilder(IQueryable<tbl_HRM_PolWelfareApplyFor> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_PolWelfareApplyFor>;
                        ...
                        switch (item)
                        {
							...
							case "IsShareholder_desc":
                                ...
                                break;
							case "IDPOLWelfare":
								query = isOrdered ? ordered.ThenBy(o => o.IDPOLWelfare) : query.OrderBy(o => o.IDPOLWelfare);
								 break;
							case "IDPOLWelfare_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDPOLWelfare) : query.OrderByDescending(o => o.IDPOLWelfare);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_PolWelfareApplyFor> _pagingBuilder(IQueryable<tbl_HRM_PolWelfareApplyFor> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
