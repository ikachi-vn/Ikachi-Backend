...
namespace ClassLibrary
{
    ...
    public partial class tbl_SHIFT_TimeSheet
    {
        ...
        public string OffTimes { get; set; }
        public Nullable<int> EventGroup { get; set; }
...
{
	using System;
	public partial class DTO_SHIFT_TimeSheet
	{
		public int IDBranch { get; set; }
		public int IDDoctor { get; set; }
		public int Id { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_SHIFT_TimeSheet 
    {
		...
		public static IQueryable<DTO_SHIFT_TimeSheet> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS SHIFT_TimeSheet";
		...
		public static DTO_SHIFT_TimeSheet getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_SHIFT_TimeSheet.Find(id);
		...
		public static DTO_SHIFT_TimeSheet getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_SHIFT_TimeSheet
        ...
        {
            bool result = false;
            var dbitem = db.tbl_SHIFT_TimeSheet.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_SHIFT_TimeSheet",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_SHIFT_TimeSheet dbitem, string Username)
        {
            Type type = typeof(tbl_SHIFT_TimeSheet);
        ...
        public static void patchDTOtoDBValue( DTO_SHIFT_TimeSheet item, tbl_SHIFT_TimeSheet dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.End = item.End;							
			dbitem.OffTimes = item.OffTimes;							
			dbitem.EventGroup = item.EventGroup;							
			dbitem.Type = item.Type;        }

		public static DTO_SHIFT_TimeSheet post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_SHIFT_TimeSheet dbitem = new tbl_SHIFT_TimeSheet();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_SHIFT_TimeSheet.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_SHIFT_TimeSheet",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_SHIFT_TimeSheet();
                    ...
                    {
                        dbitem = new tbl_SHIFT_TimeSheet();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_SHIFT_TimeSheet.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_SHIFT_TimeSheet.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_SHIFT_TimeSheet.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_SHIFT_TimeSheet", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_SHIFT_TimeSheet",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_SHIFT_TimeSheet",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_SHIFT_TimeSheet.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_SHIFT_TimeSheet.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_SHIFT_TimeSheet> toDTO(IQueryable<tbl_SHIFT_TimeSheet> query)
        {
			return query
			.Select(s => new DTO_SHIFT_TimeSheet(){							
				IDBranch = s.IDBranch,							
				IDDoctor = s.IDDoctor,							
				Id = s.Id,							
				Code = s.Code,							
		...
		public static DTO_SHIFT_TimeSheet toDTO(tbl_SHIFT_TimeSheet dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_SHIFT_TimeSheet()
				{							
					IDBranch = dbResult.IDBranch,							
					IDDoctor = dbResult.IDDoctor,							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					Start = dbResult.Start,							
					End = dbResult.End,							
					OffTimes = dbResult.OffTimes,							
					EventGroup = dbResult.EventGroup,							
					Type = dbResult.Type,
				};
			}
			else
		...
		public static IQueryable<tbl_SHIFT_TimeSheet> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_SHIFT_TimeSheet> query = db.tbl_SHIFT_TimeSheet.AsNoTracking();//.Where(d => d.IsDeleted == false);
			...
			//Query IDDoctor (int)
			if (QueryStrings.Any(d => d.Key == "IDDoctor"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDDoctor").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDDoctor));
            }
			...
			//Query Start (string)
			if (QueryStrings.Any(d => d.Key == "Start_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Start_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Start_eq").Value;
                query = query.Where(d=>d.Start == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "Start") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Start").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Start").Value;
                query = query.Where(d=>d.Start.Contains(keyword));
            }
            

			//Query End (string)
			if (QueryStrings.Any(d => d.Key == "End_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "End_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "End_eq").Value;
                query = query.Where(d=>d.End == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "End") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "End").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "End").Value;
                query = query.Where(d=>d.End.Contains(keyword));
            }
            

			//Query OffTimes (string)
			if (QueryStrings.Any(d => d.Key == "OffTimes_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "OffTimes_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "OffTimes_eq").Value;
                query = query.Where(d=>d.OffTimes == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "OffTimes") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "OffTimes").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "OffTimes").Value;
                query = query.Where(d=>d.OffTimes.Contains(keyword));
            }
            

			//Query EventGroup (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "EventGroupFrom") && QueryStrings.Any(d => d.Key == "EventGroupTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "EventGroupFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "EventGroupTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.EventGroup && d.EventGroup <= toVal);
        ...
        public static IQueryable<tbl_SHIFT_TimeSheet> _sortBuilder(IQueryable<tbl_SHIFT_TimeSheet> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_SHIFT_TimeSheet>;
                        ...
                        switch (item)
                        {
							...
							case "IDBranch_desc":
                                ...
                                break;
							case "IDDoctor":
								query = isOrdered ? ordered.ThenBy(o => o.IDDoctor) : query.OrderBy(o => o.IDDoctor);
								 break;
							case "IDDoctor_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDDoctor) : query.OrderByDescending(o => o.IDDoctor);
                                break;
							case "Id":
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "Start":
								query = isOrdered ? ordered.ThenBy(o => o.Start) : query.OrderBy(o => o.Start);
								 break;
							case "Start_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Start) : query.OrderByDescending(o => o.Start);
                                break;
							case "End":
								query = isOrdered ? ordered.ThenBy(o => o.End) : query.OrderBy(o => o.End);
								 break;
							case "End_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.End) : query.OrderByDescending(o => o.End);
                                break;
							case "OffTimes":
								query = isOrdered ? ordered.ThenBy(o => o.OffTimes) : query.OrderBy(o => o.OffTimes);
								 break;
							case "OffTimes_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.OffTimes) : query.OrderByDescending(o => o.OffTimes);
                                break;
							case "EventGroup":
								query = isOrdered ? ordered.ThenBy(o => o.EventGroup) : query.OrderBy(o => o.EventGroup);
								 break;
							case "EventGroup_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.EventGroup) : query.OrderByDescending(o => o.EventGroup);
                                break;
							case "Type":
        ...
        public static IQueryable<tbl_SHIFT_TimeSheet> _pagingBuilder(IQueryable<tbl_SHIFT_TimeSheet> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
