...
namespace ClassLibrary
{
    ...
    public partial class tbl_WMS_CartonGroup
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_WMS_CartonGroup()
        {
            this.tbl_WMS_Carton = new HashSet<tbl_WMS_Carton>();
        }
        ...
        public virtual ICollection<tbl_WMS_Carton> tbl_WMS_Carton { get; set; }
    }
}
...
{
	using System;
	public partial class DTO_WMS_CartonGroup
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_WMS_CartonGroup 
    {
		...
		public static IQueryable<DTO_WMS_CartonGroup> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS WMS_CartonGroup";
		...
		public static DTO_WMS_CartonGroup getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_WMS_CartonGroup.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_WMS_CartonGroup getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_WMS_CartonGroup
        ...
        {
            bool result = false;
            var dbitem = db.tbl_WMS_CartonGroup.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_WMS_CartonGroup",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_WMS_CartonGroup dbitem, string Username)
        {
            Type type = typeof(tbl_WMS_CartonGroup);
        ...
        public static void patchDTOtoDBValue( DTO_WMS_CartonGroup item, tbl_WMS_CartonGroup dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_WMS_CartonGroup post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_WMS_CartonGroup dbitem = new tbl_WMS_CartonGroup();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_WMS_CartonGroup.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_WMS_CartonGroup",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_WMS_CartonGroup();
                    ...
                    {
                        dbitem = new tbl_WMS_CartonGroup();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_WMS_CartonGroup.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_WMS_CartonGroup.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_WMS_CartonGroup.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_WMS_CartonGroup", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_WMS_CartonGroup",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_WMS_CartonGroup",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_WMS_CartonGroup.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_WMS_CartonGroup.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_WMS_CartonGroup> toDTO(IQueryable<tbl_WMS_CartonGroup> query)
        {
			return query
			.Select(s => new DTO_WMS_CartonGroup(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
		...
		public static DTO_WMS_CartonGroup toDTO(tbl_WMS_CartonGroup dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_WMS_CartonGroup()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
		...
		public static IQueryable<tbl_WMS_CartonGroup> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_WMS_CartonGroup> query = db.tbl_WMS_CartonGroup.AsNoTracking();//.Where(d => d.IsDeleted == false );
        ...
        public static IQueryable<tbl_WMS_CartonGroup> _sortBuilder(IQueryable<tbl_WMS_CartonGroup> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_WMS_CartonGroup>;
        ...
        public static IQueryable<tbl_WMS_CartonGroup> _pagingBuilder(IQueryable<tbl_WMS_CartonGroup> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
