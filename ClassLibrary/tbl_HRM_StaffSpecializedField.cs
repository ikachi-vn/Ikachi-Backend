...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffSpecializedField
    {
        ...
        public Nullable<int> IDSpecializedField { get; set; }
        //List 0:1
...
{
	using System;
	public partial class DTO_HRM_StaffSpecializedField
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffSpecializedField 
    {
		...
		public static IQueryable<DTO_HRM_StaffSpecializedField> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffSpecializedField";
		...
		public static DTO_HRM_StaffSpecializedField getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffSpecializedField.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffSpecializedField getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffSpecializedField
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffSpecializedField.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffSpecializedField",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffSpecializedField dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffSpecializedField);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffSpecializedField item, tbl_HRM_StaffSpecializedField dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDSpecializedField = item.IDSpecializedField;        }

		public static DTO_HRM_StaffSpecializedField post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffSpecializedField dbitem = new tbl_HRM_StaffSpecializedField();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffSpecializedField.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffSpecializedField",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffSpecializedField();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffSpecializedField();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffSpecializedField.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffSpecializedField.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffSpecializedField.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffSpecializedField", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffSpecializedField",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_StaffSpecializedField",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffSpecializedField.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffSpecializedField.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffSpecializedField> toDTO(IQueryable<tbl_HRM_StaffSpecializedField> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffSpecializedField(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				IDStaff = s.IDStaff,							
				IDSpecializedField = s.IDSpecializedField,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_HRM_StaffSpecializedField toDTO(tbl_HRM_StaffSpecializedField dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffSpecializedField()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IDSpecializedField = dbResult.IDSpecializedField,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_HRM_StaffSpecializedField> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffSpecializedField> query = db.tbl_HRM_StaffSpecializedField.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query IDSpecializedField (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDSpecializedField"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDSpecializedField").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDSpecializedField));
////                    query = query.Where(d => IDs.Contains(d.IDSpecializedField));
//                    
            }
            return query;
        ...
        public static IQueryable<tbl_HRM_StaffSpecializedField> _sortBuilder(IQueryable<tbl_HRM_StaffSpecializedField> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffSpecializedField>;
                        ...
                        switch (item)
                        {
							...
							case "IDStaff_desc":
                                ...
                                break;
							case "IDSpecializedField":
								query = isOrdered ? ordered.ThenBy(o => o.IDSpecializedField) : query.OrderBy(o => o.IDSpecializedField);
								 break;
							case "IDSpecializedField_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDSpecializedField) : query.OrderByDescending(o => o.IDSpecializedField);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_StaffSpecializedField> _pagingBuilder(IQueryable<tbl_HRM_StaffSpecializedField> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
