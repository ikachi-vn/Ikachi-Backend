...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffAddress
    {
...
{
	using System;
	public partial class DTO_HRM_StaffAddress
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffAddress 
    {
		...
		public static IQueryable<DTO_HRM_StaffAddress> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffAddress";
		...
		public static DTO_HRM_StaffAddress getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffAddress.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffAddress getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffAddress
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffAddress.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffAddress",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffAddress dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffAddress);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffAddress item, tbl_HRM_StaffAddress dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.PermanentAddress = item.PermanentAddress;							
			dbitem.TemporaryAddress = item.TemporaryAddress;							
			dbitem.HomeAddress = item.HomeAddress;							
			dbitem.IsDeleted = item.IsDeleted;        }

		public static DTO_HRM_StaffAddress post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffAddress dbitem = new tbl_HRM_StaffAddress();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffAddress.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffAddress",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffAddress();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffAddress();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffAddress.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffAddress.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffAddress.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffAddress", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffAddress",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffAddress.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffAddress.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffAddress> toDTO(IQueryable<tbl_HRM_StaffAddress> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffAddress(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
		...
		public static DTO_HRM_StaffAddress toDTO(tbl_HRM_StaffAddress dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffAddress()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					PermanentAddress = dbResult.PermanentAddress,							
					TemporaryAddress = dbResult.TemporaryAddress,							
		...
		public static IQueryable<tbl_HRM_StaffAddress> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffAddress> query = db.tbl_HRM_StaffAddress.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			if (QueryStrings.Any(d => d.Key == "PermanentAddress_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "PermanentAddress_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "PermanentAddress_eq").Value;
                query = query.Where(d=>d.PermanentAddress == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "PermanentAddress") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "PermanentAddress").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "PermanentAddress").Value;
                query = query.Where(d=>d.PermanentAddress.Contains(keyword));
            }
            

			//Query TemporaryAddress (string)
			if (QueryStrings.Any(d => d.Key == "TemporaryAddress_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "TemporaryAddress_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "TemporaryAddress_eq").Value;
                query = query.Where(d=>d.TemporaryAddress == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "TemporaryAddress") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "TemporaryAddress").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "TemporaryAddress").Value;
                query = query.Where(d=>d.TemporaryAddress.Contains(keyword));
            }
        ...
        public static IQueryable<tbl_HRM_StaffAddress> _sortBuilder(IQueryable<tbl_HRM_StaffAddress> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffAddress>;
                        ...
                        switch (item)
                        {
							...
							case "PermanentAddress":
								query = isOrdered ? ordered.ThenBy(o => o.PermanentAddress) : query.OrderBy(o => o.PermanentAddress);
								 break;
							case "PermanentAddress_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.PermanentAddress) : query.OrderByDescending(o => o.PermanentAddress);
                                break;
							case "TemporaryAddress":
								query = isOrdered ? ordered.ThenBy(o => o.TemporaryAddress) : query.OrderBy(o => o.TemporaryAddress);
								 break;
							case "TemporaryAddress_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.TemporaryAddress) : query.OrderByDescending(o => o.TemporaryAddress);
                                break;
							case "HomeAddress":
        ...
        public static IQueryable<tbl_HRM_StaffAddress> _pagingBuilder(IQueryable<tbl_HRM_StaffAddress> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
