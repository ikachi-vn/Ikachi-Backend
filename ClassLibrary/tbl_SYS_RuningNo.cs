...
namespace ClassLibrary
{
    ...
    public partial class tbl_SYS_RuningNo
    {
        ...
        public int BaseRadixType { get; set; }
...
{
	using System;
	public partial class DTO_SYS_RuningNo
	{
		public int IDBranch { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_SYS_RuningNo 
    {
		...
		public static IQueryable<DTO_SYS_RuningNo> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS SYS_RuningNo";
		...
		public static DTO_SYS_RuningNo getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_SYS_RuningNo.Find(id);
		...
		public static DTO_SYS_RuningNo getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_SYS_RuningNo
        ...
        {
            bool result = false;
            var dbitem = db.tbl_SYS_RuningNo.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_SYS_RuningNo",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_SYS_RuningNo dbitem, string Username)
        {
            Type type = typeof(tbl_SYS_RuningNo);
        ...
        public static void patchDTOtoDBValue( DTO_SYS_RuningNo item, tbl_SYS_RuningNo dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.Prefix = item.Prefix;							
			dbitem.BaseRadixType = item.BaseRadixType;							
			dbitem.Value = item.Value;        }

		public static DTO_SYS_RuningNo post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_SYS_RuningNo dbitem = new tbl_SYS_RuningNo();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_SYS_RuningNo.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_SYS_RuningNo",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_SYS_RuningNo();
                    ...
                    {
                        dbitem = new tbl_SYS_RuningNo();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_SYS_RuningNo.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_SYS_RuningNo.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_SYS_RuningNo.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_SYS_RuningNo", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_SYS_RuningNo",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_SYS_RuningNo",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_SYS_RuningNo.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_SYS_RuningNo.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_SYS_RuningNo> toDTO(IQueryable<tbl_SYS_RuningNo> query)
        {
			return query
			.Select(s => new DTO_SYS_RuningNo(){							
				IDBranch = s.IDBranch,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				...
				Prefix = s.Prefix,							
				BaseRadixType = s.BaseRadixType,							
				Value = s.Value,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_SYS_RuningNo toDTO(tbl_SYS_RuningNo dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_SYS_RuningNo()
				{							
					...
					BaseRadixType = dbResult.BaseRadixType,							
		...
		public static IQueryable<tbl_SYS_RuningNo> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_SYS_RuningNo> query = db.tbl_SYS_RuningNo.AsNoTracking();//.Where(d => d.IsDeleted == false);
			...
			//Query Prefix (string)
			if (QueryStrings.Any(d => d.Key == "Prefix_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Prefix_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Prefix_eq").Value;
                query = query.Where(d=>d.Prefix == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "Prefix") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Prefix").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Prefix").Value;
                query = query.Where(d=>d.Prefix.Contains(keyword));
            }
            

			//Query BaseRadixType (int)
			if (QueryStrings.Any(d => d.Key == "BaseRadixTypeFrom") && QueryStrings.Any(d => d.Key == "BaseRadixTypeTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "BaseRadixTypeFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "BaseRadixTypeTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.BaseRadixType && d.BaseRadixType <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "BaseRadixType"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "BaseRadixType").Value, out int val))
                    query = query.Where(d => val == d.BaseRadixType);


			//Query Value (string)
			if (QueryStrings.Any(d => d.Key == "Value_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Value_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Value_eq").Value;
            ...
            if (QueryStrings.Any(d => d.Key == "Value") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Value").Value))
            {
                ...
                query = query.Where(d=>d.Value.Contains(keyword));
            }
            
            return query;
        }
		
        public static IQueryable<tbl_SYS_RuningNo> _sortBuilder(IQueryable<tbl_SYS_RuningNo> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_SYS_RuningNo>;
                        ...
                        switch (item)
                        {
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "Prefix":
								query = isOrdered ? ordered.ThenBy(o => o.Prefix) : query.OrderBy(o => o.Prefix);
								 break;
							case "Prefix_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Prefix) : query.OrderByDescending(o => o.Prefix);
                                break;
							case "BaseRadixType":
								query = isOrdered ? ordered.ThenBy(o => o.BaseRadixType) : query.OrderBy(o => o.BaseRadixType);
								 break;
							case "BaseRadixType_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.BaseRadixType) : query.OrderByDescending(o => o.BaseRadixType);
                                break;
							case "Value":
								query = isOrdered ? ordered.ThenBy(o => o.Value) : query.OrderBy(o => o.Value);
								 break;
							case "Value_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Value) : query.OrderByDescending(o => o.Value);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_SYS_RuningNo> _pagingBuilder(IQueryable<tbl_SYS_RuningNo> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
