...
namespace ClassLibrary
{
    ...
    public partial class tbl_PROD_BillOfMaterials
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_PROD_BillOfMaterials()
        {
        ...
        public int IDStdCostPriceList { get; set; }
...
{
	using System;
	public partial class DTO_PROD_BillOfMaterials
	{
		public int Id { get; set; }
		public string Name { get; set; }
		public int IDItem { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_PROD_BillOfMaterials 
    {
		...
		public static IQueryable<DTO_PROD_BillOfMaterials> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
		...
		public static DTO_PROD_BillOfMaterials getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_PROD_BillOfMaterials.Find(id);
        ...
        {
            bool result = false;
            var dbitem = db.tbl_PROD_BillOfMaterials.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_PROD_BillOfMaterials",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_PROD_BillOfMaterials dbitem, string Username)
        {
            Type type = typeof(tbl_PROD_BillOfMaterials);
        ...
        public static void patchDTOtoDBValue( DTO_PROD_BillOfMaterials item, tbl_PROD_BillOfMaterials dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDStdCostPriceList = item.IDStdCostPriceList;							
			dbitem.BatchSize = item.BatchSize;							
			dbitem.Sort = item.Sort;							
			dbitem.IsDisabled = item.IsDisabled;							
			dbitem.IsDeleted = item.IsDeleted;        }

		public static DTO_PROD_BillOfMaterials post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_PROD_BillOfMaterials();
                    ...
                    {
                        dbitem = new tbl_PROD_BillOfMaterials();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_PROD_BillOfMaterials.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_PROD_BillOfMaterials.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_PROD_BillOfMaterials.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_PROD_BillOfMaterials", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_PROD_BillOfMaterials",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_PROD_BillOfMaterials",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_PROD_BillOfMaterials.Any(e => e.Id == id && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_PROD_BillOfMaterials> toDTO(IQueryable<tbl_PROD_BillOfMaterials> query)
        {
			return query
			.Select(s => new DTO_PROD_BillOfMaterials(){							
				Id = s.Id,							
				Name = s.Name,							
				IDItem = s.IDItem,							
				Type = s.Type,							
		...
		public static DTO_PROD_BillOfMaterials toDTO(tbl_PROD_BillOfMaterials dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_PROD_BillOfMaterials()
				{							
					Id = dbResult.Id,							
					Name = dbResult.Name,							
		...
		public static IQueryable<tbl_PROD_BillOfMaterials> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_PROD_BillOfMaterials> query = db.tbl_PROD_BillOfMaterials.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query IDStdCostPriceList (int)
			if (QueryStrings.Any(d => d.Key == "IDStdCostPriceList"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDStdCostPriceList").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDStdCostPriceList));
            }

			//Query BatchSize (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "BatchSizeFrom") && QueryStrings.Any(d => d.Key == "BatchSizeTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "BatchSizeFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "BatchSizeTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.BatchSize && d.BatchSize <= toVal);
        ...
        public static IQueryable<tbl_PROD_BillOfMaterials> _sortBuilder(IQueryable<tbl_PROD_BillOfMaterials> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_PROD_BillOfMaterials>;
                        ...
                        switch (item)
                        {
							...
							case "IDPriceList_desc":
                                ...
                                break;
							case "IDStdCostPriceList":
								query = isOrdered ? ordered.ThenBy(o => o.IDStdCostPriceList) : query.OrderBy(o => o.IDStdCostPriceList);
								 break;
							case "IDStdCostPriceList_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDStdCostPriceList) : query.OrderByDescending(o => o.IDStdCostPriceList);
                                break;
							case "BatchSize":
								query = isOrdered ? ordered.ThenBy(o => o.BatchSize) : query.OrderBy(o => o.BatchSize);
								 break;
							case "BatchSize_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.BatchSize) : query.OrderByDescending(o => o.BatchSize);
                                break;
							case "Sort":
        ...
        public static IQueryable<tbl_PROD_BillOfMaterials> _pagingBuilder(IQueryable<tbl_PROD_BillOfMaterials> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
