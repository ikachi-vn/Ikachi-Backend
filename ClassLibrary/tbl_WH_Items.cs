...
namespace ClassLibrary
{
    ...
    public partial class tbl_WH_Items
    {
        ...
        public Nullable<int> UoMInventory { get; set; }
        public Nullable<int> UoMSales { get; set; }
        public Nullable<int> UoMPurchasing { get; set; }
        public Nullable<int> UoMBase { get; set; }
...
{
	using System;
	public partial class DTO_WH_Items
	{
		public int Id { get; set; }
		public Nullable<int> IDItemGroup { get; set; }
		public Nullable<int> IDBranch { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_WH_Items 
    {
		...
		public static IQueryable<DTO_WH_Items> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS WH_Items";
		...
		public static DTO_WH_Items getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_WH_Items.Find(id);
		...
		public static DTO_WH_Items getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_WH_Items
        ...
        {
            bool result = false;
            var dbitem = db.tbl_WH_Items.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_WH_Items",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_WH_Items dbitem, string Username)
        {
            Type type = typeof(tbl_WH_Items);
        ...
        public static void patchDTOtoDBValue( DTO_WH_Items item, tbl_WH_Items dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.UoMInventory = item.UoMInventory;							
			dbitem.UoMSales = item.UoMSales;							
			dbitem.UoMPurchasing = item.UoMPurchasing;							
			dbitem.UoMBase = item.UoMBase;							
			dbitem.IsExpired = item.IsExpired;							
			dbitem.Sort = item.Sort;							
			dbitem.IsDisabled = item.IsDisabled;							
			dbitem.IsDeleted = item.IsDeleted;        }

		public static DTO_WH_Items post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_WH_Items dbitem = new tbl_WH_Items();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_WH_Items.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_WH_Items",e);
                    return null;
                }
            }
            return toDTO(dbitem);
        ...
        public static dynamic import(ExcelPackage package, AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings, string Username)
        {
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    {
                        dbitem = new tbl_WH_Items();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_WH_Items.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_WH_Items.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_WH_Items.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_WH_Items", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_WH_Items",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_WH_Items",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_WH_Items.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_WH_Items.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_WH_Items> toDTO(IQueryable<tbl_WH_Items> query)
        {
			return query
			.Select(s => new DTO_WH_Items(){							
				Id = s.Id,							
				...
				UoMInventory = s.UoMInventory,							
				UoMSales = s.UoMSales,							
				UoMPurchasing = s.UoMPurchasing,							
				UoMBase = s.UoMBase,							
		...
		public static DTO_WH_Items toDTO(tbl_WH_Items dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_WH_Items()
				{							
					Id = dbResult.Id,							
					...
					UoMInventory = dbResult.UoMInventory,							
					UoMSales = dbResult.UoMSales,							
					UoMPurchasing = dbResult.UoMPurchasing,							
					UoMBase = dbResult.UoMBase,							
					IsExpired = dbResult.IsExpired,							
					Sort = dbResult.Sort,							
		...
		public static IQueryable<tbl_WH_Items> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_WH_Items> query = db.tbl_WH_Items.AsNoTracking();//.Where(d => d.IsDeleted == false);
...
//                    
			...
			//Query IsInventoryItem (Nullable<bool>)

			//Query IsSalesItem (Nullable<bool>)

			//Query IsPurchaseItem (Nullable<bool>)

			//Query IsPhantomItem (Nullable<bool>)

			//Query UoMInventory (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "UoMInventoryFrom") && QueryStrings.Any(d => d.Key == "UoMInventoryTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "UoMInventoryFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "UoMInventoryTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.UoMInventory && d.UoMInventory <= toVal);

			//Query UoMSales (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "UoMSalesFrom") && QueryStrings.Any(d => d.Key == "UoMSalesTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "UoMSalesFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "UoMSalesTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.UoMSales && d.UoMSales <= toVal);

			//Query UoMPurchasing (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "UoMPurchasingFrom") && QueryStrings.Any(d => d.Key == "UoMPurchasingTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "UoMPurchasingFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "UoMPurchasingTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.UoMPurchasing && d.UoMPurchasing <= toVal);

			//Query UoMBase (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "UoMBaseFrom") && QueryStrings.Any(d => d.Key == "UoMBaseTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "UoMBaseFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "UoMBaseTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.UoMBase && d.UoMBase <= toVal);

			//Query IsExpired (Nullable<bool>)
        ...
        public static IQueryable<tbl_WH_Items> _sortBuilder(IQueryable<tbl_WH_Items> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_WH_Items>;
                        ...
                        switch (item)
                        {
							...
							case "IsPhantomItem_desc":
                                ...
                                break;
							case "UoMInventory":
								query = isOrdered ? ordered.ThenBy(o => o.UoMInventory) : query.OrderBy(o => o.UoMInventory);
								 break;
							case "UoMInventory_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.UoMInventory) : query.OrderByDescending(o => o.UoMInventory);
                                break;
							case "UoMSales":
								query = isOrdered ? ordered.ThenBy(o => o.UoMSales) : query.OrderBy(o => o.UoMSales);
								 break;
							case "UoMSales_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.UoMSales) : query.OrderByDescending(o => o.UoMSales);
                                break;
							case "UoMPurchasing":
								query = isOrdered ? ordered.ThenBy(o => o.UoMPurchasing) : query.OrderBy(o => o.UoMPurchasing);
								 break;
							case "UoMPurchasing_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.UoMPurchasing) : query.OrderByDescending(o => o.UoMPurchasing);
                                break;
							case "UoMBase":
								query = isOrdered ? ordered.ThenBy(o => o.UoMBase) : query.OrderBy(o => o.UoMBase);
								 break;
							case "UoMBase_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.UoMBase) : query.OrderByDescending(o => o.UoMBase);
                                break;
							case "IsExpired":
								query = isOrdered ? ordered.ThenBy(o => o.IsExpired) : query.OrderBy(o => o.IsExpired);
								 break;
							case "IsExpired_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IsExpired) : query.OrderByDescending(o => o.IsExpired);
                                break;
							case "Sort":
        ...
        public static IQueryable<tbl_WH_Items> _pagingBuilder(IQueryable<tbl_WH_Items> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
