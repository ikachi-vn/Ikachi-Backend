...
namespace ClassLibrary
{
    ...
    public partial class tbl_BI_Oppotunity
    {
...
{
	using System;
	public partial class DTO_BI_Oppotunity
	{
		public int IDBranch { get; set; }
		public int Id { get; set; }
		public int Frequency { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_BI_Oppotunity 
    {
		...
		public static IQueryable<DTO_BI_Oppotunity> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS BI_Oppotunity";
		...
		public static DTO_BI_Oppotunity getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_BI_Oppotunity.Find(id);
        ...
        {
            bool result = false;
            var dbitem = db.tbl_BI_Oppotunity.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_BI_Oppotunity",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_BI_Oppotunity dbitem, string Username)
        {
            Type type = typeof(tbl_BI_Oppotunity);
        ...
        public static void patchDTOtoDBValue( DTO_BI_Oppotunity item, tbl_BI_Oppotunity dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_BI_Oppotunity post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_BI_Oppotunity dbitem = new tbl_BI_Oppotunity();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_BI_Oppotunity.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_BI_Oppotunity",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_BI_Oppotunity();
                    ...
                    {
                        dbitem = new tbl_BI_Oppotunity();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_BI_Oppotunity.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_BI_Oppotunity.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_BI_Oppotunity.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_BI_Oppotunity", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_BI_Oppotunity",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_BI_Oppotunity.Any(e => e.Id == id && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_BI_Oppotunity> toDTO(IQueryable<tbl_BI_Oppotunity> query)
        {
			return query
			.Select(s => new DTO_BI_Oppotunity(){							
				IDBranch = s.IDBranch,							
				Id = s.Id,							
		...
		public static DTO_BI_Oppotunity toDTO(tbl_BI_Oppotunity dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_BI_Oppotunity()
				{							
		...
		public static IQueryable<tbl_BI_Oppotunity> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_BI_Oppotunity> query = db.tbl_BI_Oppotunity.AsNoTracking();//.Where(d => d.IsDeleted == false);
        ...
        public static IQueryable<tbl_BI_Oppotunity> _sortBuilder(IQueryable<tbl_BI_Oppotunity> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_BI_Oppotunity>;
        ...
        public static IQueryable<tbl_BI_Oppotunity> _pagingBuilder(IQueryable<tbl_BI_Oppotunity> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
