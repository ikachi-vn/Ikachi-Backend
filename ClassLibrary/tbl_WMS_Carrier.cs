...
namespace ClassLibrary
{
    ...
    public partial class tbl_WMS_Carrier
    {
...
{
	using System;
	public partial class DTO_WMS_Carrier
	{
		public int Id { get; set; }
		public string Remark { get; set; }
		public Nullable<int> Sort { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_WMS_Carrier 
    {
		...
		public static IQueryable<DTO_WMS_Carrier> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS WMS_Carrier";
		...
		public static DTO_WMS_Carrier getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_WMS_Carrier.Find(id);
        ...
        {
            bool result = false;
            var dbitem = db.tbl_WMS_Carrier.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_WMS_Carrier",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_WMS_Carrier dbitem, string Username)
        {
            Type type = typeof(tbl_WMS_Carrier);
        ...
        public static void patchDTOtoDBValue( DTO_WMS_Carrier item, tbl_WMS_Carrier dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_WMS_Carrier post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_WMS_Carrier dbitem = new tbl_WMS_Carrier();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_WMS_Carrier.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_WMS_Carrier",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_WMS_Carrier();
                    ...
                    {
                        dbitem = new tbl_WMS_Carrier();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_WMS_Carrier.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_WMS_Carrier.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_WMS_Carrier.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_WMS_Carrier", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_WMS_Carrier",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_WMS_Carrier",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_WMS_Carrier.Any(e => e.Id == id && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_WMS_Carrier> toDTO(IQueryable<tbl_WMS_Carrier> query)
        {
			return query
			.Select(s => new DTO_WMS_Carrier(){							
				Id = s.Id,							
				Remark = s.Remark,							
				Sort = s.Sort,							
		...
		public static DTO_WMS_Carrier toDTO(tbl_WMS_Carrier dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_WMS_Carrier()
				{							
					Id = dbResult.Id,							
		...
		public static IQueryable<tbl_WMS_Carrier> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_WMS_Carrier> query = db.tbl_WMS_Carrier.AsNoTracking();//.Where(d => d.IsDeleted == false );
        ...
        public static IQueryable<tbl_WMS_Carrier> _sortBuilder(IQueryable<tbl_WMS_Carrier> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_WMS_Carrier>;
        ...
        public static IQueryable<tbl_WMS_Carrier> _pagingBuilder(IQueryable<tbl_WMS_Carrier> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
