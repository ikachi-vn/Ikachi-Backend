...
namespace ClassLibrary
{
    ...
    public partial class tbl_LIST_Ward
    {
        ...
        //List 0:1
        public virtual tbl_LIST_District tbl_LIST_District { get; set; }
    }
}
...
{
	using System;
	public partial class DTO_LIST_Ward
	{
		public int IDDistrict { get; set; }
		public int Id { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_LIST_Ward 
    {
		...
		public static IQueryable<DTO_LIST_Ward> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS LIST_Ward";
            ...
            if (workBook != null)
            {
                ...
                //var LIST_DistrictList = BS_LIST_District.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_LIST_Ward getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_LIST_Ward.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_LIST_Ward getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_LIST_Ward
        ...
        {
            bool result = false;
            var dbitem = db.tbl_LIST_Ward.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_LIST_Ward",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_LIST_Ward dbitem, string Username)
        {
            Type type = typeof(tbl_LIST_Ward);
        ...
        public static void patchDTOtoDBValue( DTO_LIST_Ward item, tbl_LIST_Ward dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDDistrict = item.IDDistrict;							
			dbitem.Name = item.Name;							
			dbitem.Type = item.Type;							
			dbitem.LatiLongTude = item.LatiLongTude;							
		...
		public static DTO_LIST_Ward post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_LIST_Ward dbitem = new tbl_LIST_Ward();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_LIST_Ward.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_LIST_Ward",e);
                    return null;
                }
            }
            return toDTO(dbitem);
        ...
        public static dynamic import(ExcelPackage package, AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings, string Username)
        {
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    {
                        dbitem = new tbl_LIST_Ward();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_LIST_Ward.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_LIST_Ward.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_LIST_Ward.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_LIST_Ward", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_LIST_Ward",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_LIST_Ward",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_LIST_Ward.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_LIST_Ward.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_LIST_Ward> toDTO(IQueryable<tbl_LIST_Ward> query)
        {
			return query
			.Select(s => new DTO_LIST_Ward(){							
				IDDistrict = s.IDDistrict,							
				Id = s.Id,							
				Name = s.Name,							
				Type = s.Type,							
				LatiLongTude = s.LatiLongTude,							
				Code = s.Code,							
				Remark = s.Remark,							
				Sort = s.Sort,							
		...
		public static DTO_LIST_Ward toDTO(tbl_LIST_Ward dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_LIST_Ward()
				{							
					IDDistrict = dbResult.IDDistrict,							
					Id = dbResult.Id,							
					Name = dbResult.Name,							
					Type = dbResult.Type,							
					LatiLongTude = dbResult.LatiLongTude,							
					Code = dbResult.Code,							
		...
		public static IQueryable<tbl_LIST_Ward> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_LIST_Ward> query = db.tbl_LIST_Ward.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query IDDistrict (int)
			if (QueryStrings.Any(d => d.Key == "IDDistrict"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDDistrict").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDDistrict));
            }
			...
			//Query LatiLongTude (string)
			if (QueryStrings.Any(d => d.Key == "LatiLongTude_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "LatiLongTude_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "LatiLongTude_eq").Value;
                query = query.Where(d=>d.LatiLongTude == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "LatiLongTude") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "LatiLongTude").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "LatiLongTude").Value;
                query = query.Where(d=>d.LatiLongTude.Contains(keyword));
            }
        ...
        public static IQueryable<tbl_LIST_Ward> _sortBuilder(IQueryable<tbl_LIST_Ward> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_LIST_Ward>;
                        bool isOrdered = ordered.ToString().IndexOf("ORDER BY") !=-1;

                        switch (item)
                        {
							case "IDDistrict":
								query = isOrdered ? ordered.ThenBy(o => o.IDDistrict) : query.OrderBy(o => o.IDDistrict);
								 break;
							case "IDDistrict_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDDistrict) : query.OrderByDescending(o => o.IDDistrict);
                                break;
							case "Id":
							...
							case "Type_desc":
                                ...
                                break;
							case "LatiLongTude":
								query = isOrdered ? ordered.ThenBy(o => o.LatiLongTude) : query.OrderBy(o => o.LatiLongTude);
								 break;
							case "LatiLongTude_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.LatiLongTude) : query.OrderByDescending(o => o.LatiLongTude);
                                break;
							case "Code":
        ...
        public static IQueryable<tbl_LIST_Ward> _pagingBuilder(IQueryable<tbl_LIST_Ward> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
