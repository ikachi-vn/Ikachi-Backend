...
namespace ClassLibrary
{
    ...
    public partial class tbl_BP_Partner
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_BP_Partner()
        {
            this.tbl_BRA_Branch = new HashSet<tbl_BRA_Branch>();
        }
        ...
        public virtual ICollection<tbl_BRA_Branch> tbl_BRA_Branch { get; set; }
    }
}
...
{
	using System;
	public partial class DTO_BP_Partner
	{
		public Nullable<int> IDParent { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_BP_Partner 
    {
		...
		public static IQueryable<DTO_BP_Partner> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            ...
            if (QueryStrings.Any(d => d.Key == "AllChildren")  || QueryStrings.Any(d => d.Key == "AllParent"))
            {
                ...
                List<DTO_BP_Partner> allItems = db.tbl_BP_Partner.Where(d => d.IsDeleted == false).Select(s => new DTO_BP_Partner()
                {
                    Id = s.Id,
                    IDParent = s.IDParent
                }).ToList();
                ...
                foreach (var item in query)
                {
                    ...
                    query = db.tbl_BP_Partner.Where(d => Ids.Contains(d.Id));
                }
        ...
        public static List<int> findParent(List<DTO_BP_Partner> allItems, int Id)
        {
        ...
        public static List<int> findChildrent(List<DTO_BP_Partner> allItems, int Id)
        {
            var Ids = new List<int>();
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS BP_Partner";
		...
		public static DTO_BP_Partner getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_BP_Partner.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_BP_Partner getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_BP_Partner
        ...
        {
            bool result = false;
            var dbitem = db.tbl_BP_Partner.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_BP_Partner",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_BP_Partner dbitem, string Username)
        {
            Type type = typeof(tbl_BP_Partner);
        ...
        public static void patchDTOtoDBValue( DTO_BP_Partner item, tbl_BP_Partner dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_BP_Partner post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_BP_Partner dbitem = new tbl_BP_Partner();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_BP_Partner.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_BP_Partner",e);
                    return null;
                }
            }
            return toDTO(dbitem);
        ...
        public static dynamic import(ExcelPackage package, AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings, string Username)
        {
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    {
                        dbitem = new tbl_BP_Partner();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_BP_Partner.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_BP_Partner.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_BP_Partner.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_BP_Partner", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_BP_Partner",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_BP_Partner",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_BP_Partner.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_BP_Partner.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_BP_Partner> toDTO(IQueryable<tbl_BP_Partner> query)
        {
			return query
			.Select(s => new DTO_BP_Partner(){							
				IDParent = s.IDParent,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
		...
		public static DTO_BP_Partner toDTO(tbl_BP_Partner dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_BP_Partner()
				{							
		...
		public static IQueryable<tbl_BP_Partner> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_BP_Partner> query = db.tbl_BP_Partner.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
        ...
        public static IQueryable<tbl_BP_Partner> _sortBuilder(IQueryable<tbl_BP_Partner> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_BP_Partner>;
        ...
        public static IQueryable<tbl_BP_Partner> _pagingBuilder(IQueryable<tbl_BP_Partner> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
