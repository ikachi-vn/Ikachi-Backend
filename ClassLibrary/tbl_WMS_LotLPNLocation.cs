...
namespace ClassLibrary
{
    ...
    public partial class tbl_WMS_LotLPNLocation
    {
        ...
        public decimal QuantityPickInProcess { get; set; }
        public decimal QuantityPendingMoveIn { get; set; }
...
{
	using System;
	public partial class DTO_WMS_LotLPNLocation
	{
		public int IDBranch { get; set; }
		public int IDStorer { get; set; }
		public int IDItem { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_WMS_LotLPNLocation 
    {
		...
		public static IQueryable<DTO_WMS_LotLPNLocation> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS WMS_LotLPNLocation";
		...
		public static DTO_WMS_LotLPNLocation getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_WMS_LotLPNLocation.Find(id);
        ...
        {
            bool result = false;
            var dbitem = db.tbl_WMS_LotLPNLocation.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_WMS_LotLPNLocation",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_WMS_LotLPNLocation dbitem, string Username)
        {
            Type type = typeof(tbl_WMS_LotLPNLocation);
        ...
        public static void patchDTOtoDBValue( DTO_WMS_LotLPNLocation item, tbl_WMS_LotLPNLocation dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDLot = item.IDLot;							
			dbitem.IDLPN = item.IDLPN;							
			...
			dbitem.QuantityPickInProcess = item.QuantityPickInProcess;							
			dbitem.QuantityPendingMoveIn = item.QuantityPendingMoveIn;							
			dbitem.IsDisabled = item.IsDisabled;							
			dbitem.IsDeleted = item.IsDeleted;        }

		public static DTO_WMS_LotLPNLocation post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_WMS_LotLPNLocation dbitem = new tbl_WMS_LotLPNLocation();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                {
					errorLog.logMessage("post_WMS_LotLPNLocation",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_WMS_LotLPNLocation();
                    ...
                    {
                        dbitem = db.tbl_WMS_LotLPNLocation.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_WMS_LotLPNLocation.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_WMS_LotLPNLocation.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_WMS_LotLPNLocation", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_WMS_LotLPNLocation",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_WMS_LotLPNLocation",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_WMS_LotLPNLocation.Any(e => e.Id == id && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_WMS_LotLPNLocation> toDTO(IQueryable<tbl_WMS_LotLPNLocation> query)
        {
			return query
			.Select(s => new DTO_WMS_LotLPNLocation(){							
				IDBranch = s.IDBranch,							
				IDStorer = s.IDStorer,							
				IDItem = s.IDItem,							
				Id = s.Id,							
		...
		public static DTO_WMS_LotLPNLocation toDTO(tbl_WMS_LotLPNLocation dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_WMS_LotLPNLocation()
				{							
					IDBranch = dbResult.IDBranch,							
					IDStorer = dbResult.IDStorer,							
					IDItem = dbResult.IDItem,							
					Id = dbResult.Id,							
					IDLot = dbResult.IDLot,							
					IDLPN = dbResult.IDLPN,							
					...
					QuantityPickInProcess = dbResult.QuantityPickInProcess,							
					QuantityPendingMoveIn = dbResult.QuantityPendingMoveIn,							
		...
		public static IQueryable<tbl_WMS_LotLPNLocation> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_WMS_LotLPNLocation> query = db.tbl_WMS_LotLPNLocation.AsNoTracking();//.Where(d => d.IsDeleted == false);
			...
			//Query IDLot (int)
			if (QueryStrings.Any(d => d.Key == "IDLot"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDLot").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDLot));
            }

			//Query IDLPN (int)
			if (QueryStrings.Any(d => d.Key == "IDLPN"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDLPN").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDLPN));
            }
			...
			//Query QuantityPickInProcess (decimal)
			if (QueryStrings.Any(d => d.Key == "QuantityPickInProcessFrom") && QueryStrings.Any(d => d.Key == "QuantityPickInProcessTo"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "QuantityPickInProcessFrom").Value, out decimal fromVal) && decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "QuantityPickInProcessTo").Value, out decimal toVal))
                    query = query.Where(d => fromVal <= d.QuantityPickInProcess && d.QuantityPickInProcess <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "QuantityPickInProcess"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "QuantityPickInProcess").Value, out decimal val))
                    query = query.Where(d => val == d.QuantityPickInProcess);


			//Query QuantityPendingMoveIn (decimal)
			if (QueryStrings.Any(d => d.Key == "QuantityPendingMoveInFrom") && QueryStrings.Any(d => d.Key == "QuantityPendingMoveInTo"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "QuantityPendingMoveInFrom").Value, out decimal fromVal) && decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "QuantityPendingMoveInTo").Value, out decimal toVal))
                    query = query.Where(d => fromVal <= d.QuantityPendingMoveIn && d.QuantityPendingMoveIn <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "QuantityPendingMoveIn"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "QuantityPendingMoveIn").Value, out decimal val))
                    query = query.Where(d => val == d.QuantityPendingMoveIn);
        ...
        public static IQueryable<tbl_WMS_LotLPNLocation> _sortBuilder(IQueryable<tbl_WMS_LotLPNLocation> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_WMS_LotLPNLocation>;
                        ...
                        switch (item)
                        {
							...
							case "Id_desc":
                                ...
                                break;
							case "IDLot":
								query = isOrdered ? ordered.ThenBy(o => o.IDLot) : query.OrderBy(o => o.IDLot);
								 break;
							case "IDLot_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDLot) : query.OrderByDescending(o => o.IDLot);
                                break;
							case "IDLPN":
								query = isOrdered ? ordered.ThenBy(o => o.IDLPN) : query.OrderBy(o => o.IDLPN);
								 break;
							case "IDLPN_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDLPN) : query.OrderByDescending(o => o.IDLPN);
                                break;
							case "IDLocation":
							...
							case "QuantityOnHold_desc":
                                ...
                                break;
							case "QuantityPickInProcess":
								query = isOrdered ? ordered.ThenBy(o => o.QuantityPickInProcess) : query.OrderBy(o => o.QuantityPickInProcess);
								 break;
							case "QuantityPickInProcess_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.QuantityPickInProcess) : query.OrderByDescending(o => o.QuantityPickInProcess);
                                break;
							case "QuantityPendingMoveIn":
								query = isOrdered ? ordered.ThenBy(o => o.QuantityPendingMoveIn) : query.OrderBy(o => o.QuantityPendingMoveIn);
								 break;
							case "QuantityPendingMoveIn_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.QuantityPendingMoveIn) : query.OrderByDescending(o => o.QuantityPendingMoveIn);
                                break;
							case "IsDisabled":
        ...
        public static IQueryable<tbl_WMS_LotLPNLocation> _pagingBuilder(IQueryable<tbl_WMS_LotLPNLocation> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
