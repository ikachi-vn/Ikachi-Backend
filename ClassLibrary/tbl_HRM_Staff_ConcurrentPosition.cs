...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_Staff_ConcurrentPosition
    {
...
{
	using System;
	public partial class DTO_HRM_Staff_ConcurrentPosition
	{
		public int IDStaff { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_Staff_ConcurrentPosition 
    {
		...
		public static IQueryable<DTO_HRM_Staff_ConcurrentPosition> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_Staff_ConcurrentPosition";
		...
		public static DTO_HRM_Staff_ConcurrentPosition getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_Staff_ConcurrentPosition.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_Staff_ConcurrentPosition getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_Staff_ConcurrentPosition
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_Staff_ConcurrentPosition.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_Staff_ConcurrentPosition",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_Staff_ConcurrentPosition dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_Staff_ConcurrentPosition);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_Staff_ConcurrentPosition item, tbl_HRM_Staff_ConcurrentPosition dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_HRM_Staff_ConcurrentPosition post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_Staff_ConcurrentPosition dbitem = new tbl_HRM_Staff_ConcurrentPosition();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_Staff_ConcurrentPosition.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_Staff_ConcurrentPosition",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_Staff_ConcurrentPosition();
                    ...
                    {
                        dbitem = new tbl_HRM_Staff_ConcurrentPosition();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_Staff_ConcurrentPosition.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_Staff_ConcurrentPosition.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_Staff_ConcurrentPosition.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_Staff_ConcurrentPosition", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_Staff_ConcurrentPosition",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_Staff_ConcurrentPosition",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_Staff_ConcurrentPosition.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_Staff_ConcurrentPosition.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_Staff_ConcurrentPosition> toDTO(IQueryable<tbl_HRM_Staff_ConcurrentPosition> query)
        {
			return query
			.Select(s => new DTO_HRM_Staff_ConcurrentPosition(){							
				IDStaff = s.IDStaff,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
		...
		public static DTO_HRM_Staff_ConcurrentPosition toDTO(tbl_HRM_Staff_ConcurrentPosition dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_Staff_ConcurrentPosition()
				{							
		...
		public static IQueryable<tbl_HRM_Staff_ConcurrentPosition> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_Staff_ConcurrentPosition> query = db.tbl_HRM_Staff_ConcurrentPosition.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
        ...
        public static IQueryable<tbl_HRM_Staff_ConcurrentPosition> _sortBuilder(IQueryable<tbl_HRM_Staff_ConcurrentPosition> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_Staff_ConcurrentPosition>;
        ...
        public static IQueryable<tbl_HRM_Staff_ConcurrentPosition> _pagingBuilder(IQueryable<tbl_HRM_Staff_ConcurrentPosition> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
