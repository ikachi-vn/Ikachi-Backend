...
namespace ClassLibrary
{
    ...
    public partial class tbl_PROD_ItemInVendor
    {
...
{
	using System;
	public partial class DTO_PROD_ItemInVendor
	{
		public int Id { get; set; }
		public int IDVendor { get; set; }
		public int IDItem { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_PROD_ItemInVendor 
    {
		...
		public static IQueryable<DTO_PROD_ItemInVendor> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS PROD_ItemInVendor";
		...
		public static DTO_PROD_ItemInVendor getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_PROD_ItemInVendor.Find(id);
        ...
        {
            bool result = false;
            var dbitem = db.tbl_PROD_ItemInVendor.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_PROD_ItemInVendor",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_PROD_ItemInVendor dbitem, string Username)
        {
            Type type = typeof(tbl_PROD_ItemInVendor);
        ...
        public static void patchDTOtoDBValue( DTO_PROD_ItemInVendor item, tbl_PROD_ItemInVendor dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_PROD_ItemInVendor post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_PROD_ItemInVendor dbitem = new tbl_PROD_ItemInVendor();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_PROD_ItemInVendor.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_PROD_ItemInVendor",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_PROD_ItemInVendor();
                    ...
                    {
                        dbitem = new tbl_PROD_ItemInVendor();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_PROD_ItemInVendor.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_PROD_ItemInVendor.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_PROD_ItemInVendor.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_PROD_ItemInVendor", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_PROD_ItemInVendor",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_PROD_ItemInVendor",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_PROD_ItemInVendor.Any(e => e.Id == id && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_PROD_ItemInVendor> toDTO(IQueryable<tbl_PROD_ItemInVendor> query)
        {
			return query
			.Select(s => new DTO_PROD_ItemInVendor(){							
				Id = s.Id,							
				IDVendor = s.IDVendor,							
				IDItem = s.IDItem,							
				Name = s.Name,							
		...
		public static DTO_PROD_ItemInVendor toDTO(tbl_PROD_ItemInVendor dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_PROD_ItemInVendor()
				{							
					Id = dbResult.Id,							
		...
		public static IQueryable<tbl_PROD_ItemInVendor> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_PROD_ItemInVendor> query = db.tbl_PROD_ItemInVendor.AsNoTracking();//.Where(d => d.IsDeleted == false );
        ...
        public static IQueryable<tbl_PROD_ItemInVendor> _sortBuilder(IQueryable<tbl_PROD_ItemInVendor> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_PROD_ItemInVendor>;
        ...
        public static IQueryable<tbl_PROD_ItemInVendor> _pagingBuilder(IQueryable<tbl_PROD_ItemInVendor> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
