...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_PayrollPaySheetMaster
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_HRM_PayrollPaySheetMaster()
        {
            this.tbl_HRM_PayrollElement = new HashSet<tbl_HRM_PayrollElement>();
        ...
        public Nullable<decimal> TotalOfIncome { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<tbl_HRM_PayrollElement> tbl_HRM_PayrollElement { get; set; }
        //List 0:1
...
{
	using System;
	public partial class DTO_HRM_PayrollPaySheetMaster
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_PayrollPaySheetMaster 
    {
		...
		public static IQueryable<DTO_HRM_PayrollPaySheetMaster> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_PayrollPaySheetMaster";
		...
		public static DTO_HRM_PayrollPaySheetMaster getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_PayrollPaySheetMaster.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_PayrollPaySheetMaster getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_PayrollPaySheetMaster
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_PayrollPaySheetMaster.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_PayrollPaySheetMaster",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_PayrollPaySheetMaster dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_PayrollPaySheetMaster);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_PayrollPaySheetMaster item, tbl_HRM_PayrollPaySheetMaster dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.TotalOfIncome = item.TotalOfIncome;        }

		public static DTO_HRM_PayrollPaySheetMaster post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_PayrollPaySheetMaster dbitem = new tbl_HRM_PayrollPaySheetMaster();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_PayrollPaySheetMaster.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_PayrollPaySheetMaster",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_PayrollPaySheetMaster();
                    ...
                    {
                        dbitem = new tbl_HRM_PayrollPaySheetMaster();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_PayrollPaySheetMaster.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_PayrollPaySheetMaster.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_PayrollPaySheetMaster.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_PayrollPaySheetMaster", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_PayrollPaySheetMaster",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_PayrollPaySheetMaster",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_PayrollPaySheetMaster.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_PayrollPaySheetMaster.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_PayrollPaySheetMaster> toDTO(IQueryable<tbl_HRM_PayrollPaySheetMaster> query)
        {
			return query
			.Select(s => new DTO_HRM_PayrollPaySheetMaster(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				TotalOfIncome = s.TotalOfIncome,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_HRM_PayrollPaySheetMaster toDTO(tbl_HRM_PayrollPaySheetMaster dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_PayrollPaySheetMaster()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					TotalOfIncome = dbResult.TotalOfIncome,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_HRM_PayrollPaySheetMaster> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_PayrollPaySheetMaster> query = db.tbl_HRM_PayrollPaySheetMaster.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query TotalOfIncome (Nullable<decimal>)
			if (QueryStrings.Any(d => d.Key == "TotalOfIncomeFrom") && QueryStrings.Any(d => d.Key == "TotalOfIncomeTo"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "TotalOfIncomeFrom").Value, out decimal fromVal) && decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "TotalOfIncomeTo").Value, out decimal toVal))
                    query = query.Where(d => fromVal <= d.TotalOfIncome && d.TotalOfIncome <= toVal);
            return query;
        }
		
        public static IQueryable<tbl_HRM_PayrollPaySheetMaster> _sortBuilder(IQueryable<tbl_HRM_PayrollPaySheetMaster> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_PayrollPaySheetMaster>;
                        ...
                        switch (item)
                        {
							...
							case "IDPayPeriod_desc":
                                ...
                                break;
							case "TotalOfIncome":
								query = isOrdered ? ordered.ThenBy(o => o.TotalOfIncome) : query.OrderBy(o => o.TotalOfIncome);
								 break;
							case "TotalOfIncome_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.TotalOfIncome) : query.OrderByDescending(o => o.TotalOfIncome);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_PayrollPaySheetMaster> _pagingBuilder(IQueryable<tbl_HRM_PayrollPaySheetMaster> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
