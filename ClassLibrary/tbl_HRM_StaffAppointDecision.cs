...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffAppointDecision
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_HRM_StaffAppointDecision()
        {
        ...
        public Nullable<int> IDJobTitleType { get; set; }
        public Nullable<int> IDSalaryDecision { get; set; }
        //List 0:1
...
{
	using System;
	public partial class DTO_HRM_StaffAppointDecision
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffAppointDecision 
    {
		...
		public static IQueryable<DTO_HRM_StaffAppointDecision> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffAppointDecision";
		...
		public static DTO_HRM_StaffAppointDecision getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffAppointDecision.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffAppointDecision getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffAppointDecision
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffAppointDecision.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffAppointDecision",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffAppointDecision dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffAppointDecision);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffAppointDecision item, tbl_HRM_StaffAppointDecision dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDJobTitleType = item.IDJobTitleType;							
			dbitem.IDSalaryDecision = item.IDSalaryDecision;        }

		public static DTO_HRM_StaffAppointDecision post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffAppointDecision dbitem = new tbl_HRM_StaffAppointDecision();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffAppointDecision.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffAppointDecision",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffAppointDecision();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffAppointDecision();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffAppointDecision.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffAppointDecision.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffAppointDecision.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffAppointDecision", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffAppointDecision",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_StaffAppointDecision",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffAppointDecision.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffAppointDecision.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffAppointDecision> toDTO(IQueryable<tbl_HRM_StaffAppointDecision> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffAppointDecision(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				IDJobTitleType = s.IDJobTitleType,							
				IDSalaryDecision = s.IDSalaryDecision,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_HRM_StaffAppointDecision toDTO(tbl_HRM_StaffAppointDecision dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffAppointDecision()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IDJobTitleType = dbResult.IDJobTitleType,							
					IDSalaryDecision = dbResult.IDSalaryDecision,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_HRM_StaffAppointDecision> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffAppointDecision> query = db.tbl_HRM_StaffAppointDecision.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query IDJobTitleType (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDJobTitleType"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDJobTitleType").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDJobTitleType));
////                    query = query.Where(d => IDs.Contains(d.IDJobTitleType));
//                    
            }

			//Query IDSalaryDecision (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDSalaryDecision"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDSalaryDecision").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDSalaryDecision));
////                    query = query.Where(d => IDs.Contains(d.IDSalaryDecision));
//                    
            }
            return query;
        ...
        public static IQueryable<tbl_HRM_StaffAppointDecision> _sortBuilder(IQueryable<tbl_HRM_StaffAppointDecision> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffAppointDecision>;
                        ...
                        switch (item)
                        {
							...
							case "IDJobTitle_desc":
                                ...
                                break;
							case "IDJobTitleType":
								query = isOrdered ? ordered.ThenBy(o => o.IDJobTitleType) : query.OrderBy(o => o.IDJobTitleType);
								 break;
							case "IDJobTitleType_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDJobTitleType) : query.OrderByDescending(o => o.IDJobTitleType);
                                break;
							case "IDSalaryDecision":
								query = isOrdered ? ordered.ThenBy(o => o.IDSalaryDecision) : query.OrderBy(o => o.IDSalaryDecision);
								 break;
							case "IDSalaryDecision_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDSalaryDecision) : query.OrderByDescending(o => o.IDSalaryDecision);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_StaffAppointDecision> _pagingBuilder(IQueryable<tbl_HRM_StaffAppointDecision> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
