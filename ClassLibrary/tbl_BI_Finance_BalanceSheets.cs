...
namespace ClassLibrary
{
    ...
    public partial class tbl_BI_Finance_BalanceSheets
    {
        public int IDBranch { get; set; }
        public string BalanceCateCode { get; set; }
...
{
	using System;
	public partial class DTO_BI_Finance_BalanceSheets
	{
		public int IDBranch { get; set; }
		public string BalanceCateCode { get; set; }
		public int Id { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_BI_Finance_BalanceSheets 
    {
		...
		public static IQueryable<DTO_BI_Finance_BalanceSheets> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS BI_Finance_BalanceSheets";
		...
		public static DTO_BI_Finance_BalanceSheets getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_BI_Finance_BalanceSheets.Find(id);
        ...
        {
            bool result = false;
            var dbitem = db.tbl_BI_Finance_BalanceSheets.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_BI_Finance_BalanceSheets",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_BI_Finance_BalanceSheets dbitem, string Username)
        {
            Type type = typeof(tbl_BI_Finance_BalanceSheets);
        ...
        public static void patchDTOtoDBValue( DTO_BI_Finance_BalanceSheets item, tbl_BI_Finance_BalanceSheets dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.BalanceCateCode = item.BalanceCateCode;							
		...
		public static DTO_BI_Finance_BalanceSheets post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_BI_Finance_BalanceSheets dbitem = new tbl_BI_Finance_BalanceSheets();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_BI_Finance_BalanceSheets.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_BI_Finance_BalanceSheets",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_BI_Finance_BalanceSheets();
                    ...
                    {
                        dbitem = new tbl_BI_Finance_BalanceSheets();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_BI_Finance_BalanceSheets.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_BI_Finance_BalanceSheets.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_BI_Finance_BalanceSheets.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_BI_Finance_BalanceSheets", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_BI_Finance_BalanceSheets",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_BI_Finance_BalanceSheets.Any(e => e.Id == id && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_BI_Finance_BalanceSheets> toDTO(IQueryable<tbl_BI_Finance_BalanceSheets> query)
        {
			return query
			.Select(s => new DTO_BI_Finance_BalanceSheets(){							
				IDBranch = s.IDBranch,							
				BalanceCateCode = s.BalanceCateCode,							
				Id = s.Id,							
		...
		public static DTO_BI_Finance_BalanceSheets toDTO(tbl_BI_Finance_BalanceSheets dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_BI_Finance_BalanceSheets()
				{							
					IDBranch = dbResult.IDBranch,							
					BalanceCateCode = dbResult.BalanceCateCode,							
					Id = dbResult.Id,							
		...
		public static IQueryable<tbl_BI_Finance_BalanceSheets> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_BI_Finance_BalanceSheets> query = db.tbl_BI_Finance_BalanceSheets.AsNoTracking();//.Where(d => d.IsDeleted == false);
			...
			//Query BalanceCateCode (string)
			if (QueryStrings.Any(d => d.Key == "BalanceCateCode_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "BalanceCateCode_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "BalanceCateCode_eq").Value;
                query = query.Where(d=>d.BalanceCateCode == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "BalanceCateCode") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "BalanceCateCode").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "BalanceCateCode").Value;
                query = query.Where(d=>d.BalanceCateCode.Contains(keyword));
            }
        ...
        public static IQueryable<tbl_BI_Finance_BalanceSheets> _sortBuilder(IQueryable<tbl_BI_Finance_BalanceSheets> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_BI_Finance_BalanceSheets>;
                        ...
                        switch (item)
                        {
							...
							case "IDBranch_desc":
                                ...
                                break;
							case "BalanceCateCode":
								query = isOrdered ? ordered.ThenBy(o => o.BalanceCateCode) : query.OrderBy(o => o.BalanceCateCode);
								 break;
							case "BalanceCateCode_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.BalanceCateCode) : query.OrderByDescending(o => o.BalanceCateCode);
                                break;
							case "Id":
        ...
        public static IQueryable<tbl_BI_Finance_BalanceSheets> _pagingBuilder(IQueryable<tbl_BI_Finance_BalanceSheets> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
