...
namespace ClassLibrary
{
    ...
    public partial class tbl_AC_JournalEntry
    {
...
{
	using System;
	public partial class DTO_AC_JournalEntry
	{
		public Nullable<int> IDBranch { get; set; }
		public int Id { get; set; }
		public Nullable<int> RefID { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_AC_JournalEntry 
    {
		...
		public static IQueryable<DTO_AC_JournalEntry> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS AC_JournalEntry";
		...
		public static DTO_AC_JournalEntry getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_AC_JournalEntry.Find(id);
		...
		public static DTO_AC_JournalEntry getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_AC_JournalEntry
        ...
        {
            bool result = false;
            var dbitem = db.tbl_AC_JournalEntry.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_AC_JournalEntry",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_AC_JournalEntry dbitem, string Username)
        {
            Type type = typeof(tbl_AC_JournalEntry);
        ...
        public static void patchDTOtoDBValue( DTO_AC_JournalEntry item, tbl_AC_JournalEntry dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_AC_JournalEntry post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_AC_JournalEntry dbitem = new tbl_AC_JournalEntry();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_AC_JournalEntry.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_AC_JournalEntry",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_AC_JournalEntry();
                    ...
                    {
                        dbitem = new tbl_AC_JournalEntry();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_AC_JournalEntry.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_AC_JournalEntry.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_AC_JournalEntry.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_AC_JournalEntry", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_AC_JournalEntry",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_AC_JournalEntry",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_AC_JournalEntry.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_AC_JournalEntry.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_AC_JournalEntry> toDTO(IQueryable<tbl_AC_JournalEntry> query)
        {
			return query
			.Select(s => new DTO_AC_JournalEntry(){							
				IDBranch = s.IDBranch,							
				Id = s.Id,							
				RefID = s.RefID,							
				Code = s.Code,							
		...
		public static DTO_AC_JournalEntry toDTO(tbl_AC_JournalEntry dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_AC_JournalEntry()
				{							
		...
		public static IQueryable<tbl_AC_JournalEntry> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_AC_JournalEntry> query = db.tbl_AC_JournalEntry.AsNoTracking();//.Where(d => d.IsDeleted == false);
...
//                    
        ...
        public static IQueryable<tbl_AC_JournalEntry> _sortBuilder(IQueryable<tbl_AC_JournalEntry> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_AC_JournalEntry>;
        ...
        public static IQueryable<tbl_AC_JournalEntry> _pagingBuilder(IQueryable<tbl_AC_JournalEntry> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
