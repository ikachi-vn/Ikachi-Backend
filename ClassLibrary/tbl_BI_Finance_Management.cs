...
namespace ClassLibrary
{
    ...
    public partial class tbl_BI_Finance_Management
    {
        public int IDBranch { get; set; }
        public int IDPnLComponent { get; set; }
...
{
	using System;
	public partial class DTO_BI_Finance_Management
	{
		public int IDBranch { get; set; }
		public int IDPnLComponent { get; set; }
		public int Id { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_BI_Finance_Management 
    {
		...
		public static IQueryable<DTO_BI_Finance_Management> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS BI_Finance_Management";
		...
		public static DTO_BI_Finance_Management getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_BI_Finance_Management.Find(id);
        ...
        {
            bool result = false;
            var dbitem = db.tbl_BI_Finance_Management.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_BI_Finance_Management",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_BI_Finance_Management dbitem, string Username)
        {
            Type type = typeof(tbl_BI_Finance_Management);
        ...
        public static void patchDTOtoDBValue( DTO_BI_Finance_Management item, tbl_BI_Finance_Management dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDPnLComponent = item.IDPnLComponent;							
		...
		public static DTO_BI_Finance_Management post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_BI_Finance_Management dbitem = new tbl_BI_Finance_Management();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_BI_Finance_Management.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_BI_Finance_Management",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_BI_Finance_Management();
                    ...
                    {
                        dbitem = new tbl_BI_Finance_Management();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_BI_Finance_Management.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_BI_Finance_Management.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_BI_Finance_Management.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_BI_Finance_Management", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_BI_Finance_Management",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_BI_Finance_Management.Any(e => e.Id == id && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_BI_Finance_Management> toDTO(IQueryable<tbl_BI_Finance_Management> query)
        {
			return query
			.Select(s => new DTO_BI_Finance_Management(){							
				IDBranch = s.IDBranch,							
				IDPnLComponent = s.IDPnLComponent,							
				Id = s.Id,							
		...
		public static DTO_BI_Finance_Management toDTO(tbl_BI_Finance_Management dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_BI_Finance_Management()
				{							
					IDBranch = dbResult.IDBranch,							
					IDPnLComponent = dbResult.IDPnLComponent,							
					Id = dbResult.Id,							
		...
		public static IQueryable<tbl_BI_Finance_Management> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_BI_Finance_Management> query = db.tbl_BI_Finance_Management.AsNoTracking();//.Where(d => d.IsDeleted == false);
			...
			//Query IDPnLComponent (int)
			if (QueryStrings.Any(d => d.Key == "IDPnLComponent"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDPnLComponent").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDPnLComponent));
            }
        ...
        public static IQueryable<tbl_BI_Finance_Management> _sortBuilder(IQueryable<tbl_BI_Finance_Management> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_BI_Finance_Management>;
                        ...
                        switch (item)
                        {
							...
							case "IDBranch_desc":
                                ...
                                break;
							case "IDPnLComponent":
								query = isOrdered ? ordered.ThenBy(o => o.IDPnLComponent) : query.OrderBy(o => o.IDPnLComponent);
								 break;
							case "IDPnLComponent_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDPnLComponent) : query.OrderByDescending(o => o.IDPnLComponent);
                                break;
							case "Id":
        ...
        public static IQueryable<tbl_BI_Finance_Management> _pagingBuilder(IQueryable<tbl_BI_Finance_Management> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
