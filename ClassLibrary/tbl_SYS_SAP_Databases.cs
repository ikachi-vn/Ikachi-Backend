...
namespace ClassLibrary
{
    ...
    public partial class tbl_SYS_SAP_Databases
    {
...
{
	using System;
	public partial class DTO_SYS_SAP_Databases
	{
		public int Id { get; set; }
		public Nullable<int> IDBranch { get; set; }
		public string DatabaseName { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_SYS_SAP_Databases 
    {
		...
		public static IQueryable<DTO_SYS_SAP_Databases> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS SYS_SAP_Databases";
		...
		public static DTO_SYS_SAP_Databases getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_SYS_SAP_Databases.Find(id);
        ...
        {
            bool result = false;
            var dbitem = db.tbl_SYS_SAP_Databases.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_SYS_SAP_Databases",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_SYS_SAP_Databases dbitem, string Username)
        {
            Type type = typeof(tbl_SYS_SAP_Databases);
        ...
        public static void patchDTOtoDBValue( DTO_SYS_SAP_Databases item, tbl_SYS_SAP_Databases dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.DatabaseName = item.DatabaseName;							
			dbitem.Username = item.Username;							
			dbitem.Password = item.Password;							
			dbitem.APIBaseUrl = item.APIBaseUrl;        }

		public static DTO_SYS_SAP_Databases post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_SYS_SAP_Databases dbitem = new tbl_SYS_SAP_Databases();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_SYS_SAP_Databases.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_SYS_SAP_Databases",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_SYS_SAP_Databases();
                    ...
                    {
                        dbitem = new tbl_SYS_SAP_Databases();
                                            }
                    else
                    {
                        dbitem = db.tbl_SYS_SAP_Databases.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_SYS_SAP_Databases.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_SYS_SAP_Databases.Where(d => IDs.Contains(d.Id));
            ...
            foreach (var dbitem in dbitems)
            {
				...
				db.tbl_SYS_SAP_Databases.Remove(dbitem);
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_SYS_SAP_Databases", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_SYS_SAP_Databases",e);
                result = false;
            }
		...
		{
            			return db.tbl_SYS_SAP_Databases.Any(e => e.Id == id);
            
		}
		
		//---//
		public static IQueryable<DTO_SYS_SAP_Databases> toDTO(IQueryable<tbl_SYS_SAP_Databases> query)
        {
			return query
			.Select(s => new DTO_SYS_SAP_Databases(){							
				Id = s.Id,							
				IDBranch = s.IDBranch,							
		...
		public static DTO_SYS_SAP_Databases toDTO(tbl_SYS_SAP_Databases dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_SYS_SAP_Databases()
				{							
					Id = dbResult.Id,							
					...
					APIBaseUrl = dbResult.APIBaseUrl,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_SYS_SAP_Databases> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_SYS_SAP_Databases> query = db.tbl_SYS_SAP_Databases.AsNoTracking();//;
...
//                    
			...
			if (QueryStrings.Any(d => d.Key == "DatabaseName_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "DatabaseName_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "DatabaseName_eq").Value;
                query = query.Where(d=>d.DatabaseName == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "DatabaseName") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "DatabaseName").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "DatabaseName").Value;
                query = query.Where(d=>d.DatabaseName.Contains(keyword));
            }
            

			//Query Username (string)
			if (QueryStrings.Any(d => d.Key == "Username_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Username_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Username_eq").Value;
                query = query.Where(d=>d.Username == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "Username") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Username").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Username").Value;
                query = query.Where(d=>d.Username.Contains(keyword));
            }
            

			//Query Password (string)
			if (QueryStrings.Any(d => d.Key == "Password_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Password_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Password_eq").Value;
                query = query.Where(d=>d.Password == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "Password") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "Password").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "Password").Value;
                query = query.Where(d=>d.Password.Contains(keyword));
            }
            

			//Query APIBaseUrl (string)
			if (QueryStrings.Any(d => d.Key == "APIBaseUrl_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "APIBaseUrl_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "APIBaseUrl_eq").Value;
                query = query.Where(d=>d.APIBaseUrl == keyword);
            }
            if (QueryStrings.Any(d => d.Key == "APIBaseUrl") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "APIBaseUrl").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "APIBaseUrl").Value;
                query = query.Where(d=>d.APIBaseUrl.Contains(keyword));
            }
            
            return query;
        }
		
        public static IQueryable<tbl_SYS_SAP_Databases> _sortBuilder(IQueryable<tbl_SYS_SAP_Databases> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_SYS_SAP_Databases>;
                        ...
                        switch (item)
                        {
							...
							case "IDBranch_desc":
                                ...
                                break;
							case "DatabaseName":
								query = isOrdered ? ordered.ThenBy(o => o.DatabaseName) : query.OrderBy(o => o.DatabaseName);
								 break;
							case "DatabaseName_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.DatabaseName) : query.OrderByDescending(o => o.DatabaseName);
                                break;
							case "Username":
								query = isOrdered ? ordered.ThenBy(o => o.Username) : query.OrderBy(o => o.Username);
								 break;
							case "Username_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Username) : query.OrderByDescending(o => o.Username);
                                break;
							case "Password":
								query = isOrdered ? ordered.ThenBy(o => o.Password) : query.OrderBy(o => o.Password);
								 break;
							case "Password_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Password) : query.OrderByDescending(o => o.Password);
                                break;
							case "APIBaseUrl":
								query = isOrdered ? ordered.ThenBy(o => o.APIBaseUrl) : query.OrderBy(o => o.APIBaseUrl);
								 break;
							case "APIBaseUrl_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.APIBaseUrl) : query.OrderByDescending(o => o.APIBaseUrl);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_SYS_SAP_Databases> _pagingBuilder(IQueryable<tbl_SYS_SAP_Databases> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
