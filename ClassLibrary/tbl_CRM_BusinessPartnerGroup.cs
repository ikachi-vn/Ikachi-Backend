...
namespace ClassLibrary
{
    ...
    public partial class tbl_CRM_BusinessPartnerGroup
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public tbl_CRM_BusinessPartnerGroup()
        {
        ...
        public Nullable<int> IDEffectiveDiscount { get; set; }
        //List 0:1
...
{
	using System;
	public partial class DTO_CRM_BusinessPartnerGroup
	{
		public Nullable<int> IDBranch { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_CRM_BusinessPartnerGroup 
    {
		...
		public static IQueryable<DTO_CRM_BusinessPartnerGroup> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS CRM_BusinessPartnerGroup";
		...
		public static DTO_CRM_BusinessPartnerGroup getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_CRM_BusinessPartnerGroup.Find(id);
		...
		public static DTO_CRM_BusinessPartnerGroup getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_CRM_BusinessPartnerGroup
        ...
        {
            bool result = false;
            var dbitem = db.tbl_CRM_BusinessPartnerGroup.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_CRM_BusinessPartnerGroup",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_CRM_BusinessPartnerGroup dbitem, string Username)
        {
            Type type = typeof(tbl_CRM_BusinessPartnerGroup);
        ...
        public static void patchDTOtoDBValue( DTO_CRM_BusinessPartnerGroup item, tbl_CRM_BusinessPartnerGroup dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDGroupType = item.IDGroupType;							
			dbitem.IsLocked = item.IsLocked;							
			dbitem.IDPriceList = item.IDPriceList;							
			dbitem.IDEffectiveDiscount = item.IDEffectiveDiscount;        }

		public static DTO_CRM_BusinessPartnerGroup post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_CRM_BusinessPartnerGroup dbitem = new tbl_CRM_BusinessPartnerGroup();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_CRM_BusinessPartnerGroup.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_CRM_BusinessPartnerGroup",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_CRM_BusinessPartnerGroup();
                    ...
                    {
                        dbitem = new tbl_CRM_BusinessPartnerGroup();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_CRM_BusinessPartnerGroup.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_CRM_BusinessPartnerGroup.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_CRM_BusinessPartnerGroup.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_CRM_BusinessPartnerGroup", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_CRM_BusinessPartnerGroup",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_CRM_BusinessPartnerGroup",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_CRM_BusinessPartnerGroup.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_CRM_BusinessPartnerGroup.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_CRM_BusinessPartnerGroup> toDTO(IQueryable<tbl_CRM_BusinessPartnerGroup> query)
        {
			return query
			.Select(s => new DTO_CRM_BusinessPartnerGroup(){							
				IDBranch = s.IDBranch,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				...
				IDGroupType = s.IDGroupType,							
				IsLocked = s.IsLocked,							
				IDPriceList = s.IDPriceList,							
				IDEffectiveDiscount = s.IDEffectiveDiscount,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_CRM_BusinessPartnerGroup toDTO(tbl_CRM_BusinessPartnerGroup dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_CRM_BusinessPartnerGroup()
				{							
					...
					IDGroupType = dbResult.IDGroupType,							
					IsLocked = dbResult.IsLocked,							
					IDPriceList = dbResult.IDPriceList,							
					IDEffectiveDiscount = dbResult.IDEffectiveDiscount,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_CRM_BusinessPartnerGroup> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_CRM_BusinessPartnerGroup> query = db.tbl_CRM_BusinessPartnerGroup.AsNoTracking();//.Where(d => d.IsDeleted == false);
...
//                    
			...
			//Query IDGroupType (int)
			if (QueryStrings.Any(d => d.Key == "IDGroupType"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDGroupType").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDGroupType));
            }
...
//                    
			...
			//Query IDEffectiveDiscount (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDEffectiveDiscount"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDEffectiveDiscount").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDEffectiveDiscount));
////                    query = query.Where(d => IDs.Contains(d.IDEffectiveDiscount));
//                    
            }
            return query;
        ...
        public static IQueryable<tbl_CRM_BusinessPartnerGroup> _sortBuilder(IQueryable<tbl_CRM_BusinessPartnerGroup> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_CRM_BusinessPartnerGroup>;
                        ...
                        switch (item)
                        {
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "IDGroupType":
								query = isOrdered ? ordered.ThenBy(o => o.IDGroupType) : query.OrderBy(o => o.IDGroupType);
								 break;
							case "IDGroupType_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDGroupType) : query.OrderByDescending(o => o.IDGroupType);
                                break;
							case "IsLocked":
							...
							case "IDPriceList_desc":
                                ...
                                break;
							case "IDEffectiveDiscount":
								query = isOrdered ? ordered.ThenBy(o => o.IDEffectiveDiscount) : query.OrderBy(o => o.IDEffectiveDiscount);
								 break;
							case "IDEffectiveDiscount_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDEffectiveDiscount) : query.OrderByDescending(o => o.IDEffectiveDiscount);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_CRM_BusinessPartnerGroup> _pagingBuilder(IQueryable<tbl_CRM_BusinessPartnerGroup> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
