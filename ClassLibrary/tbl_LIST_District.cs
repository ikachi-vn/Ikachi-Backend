...
namespace ClassLibrary
{
    ...
    public partial class tbl_LIST_District
    {
        ...
        {
            this.tbl_LIST_Ward = new HashSet<tbl_LIST_Ward>();
        }
        ...
        //List 0:1
        public virtual tbl_LIST_Province tbl_LIST_Province { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<tbl_LIST_Ward> tbl_LIST_Ward { get; set; }
    }
}
...
{
	using System;
	public partial class DTO_LIST_District
	{
		public int IDProvince { get; set; }
		public int Id { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_LIST_District 
    {
		...
		public static IQueryable<DTO_LIST_District> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS LIST_District";
            ...
            if (workBook != null)
            {
                ...
                //var LIST_ProvinceList = BS_LIST_Province.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_LIST_District getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_LIST_District.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_LIST_District getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_LIST_District
        ...
        {
            bool result = false;
            var dbitem = db.tbl_LIST_District.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_LIST_District",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_LIST_District dbitem, string Username)
        {
            Type type = typeof(tbl_LIST_District);
        ...
        public static void patchDTOtoDBValue( DTO_LIST_District item, tbl_LIST_District dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_LIST_District post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_LIST_District dbitem = new tbl_LIST_District();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_LIST_District.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_LIST_District",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_LIST_District();
                    ...
                    {
                        dbitem = new tbl_LIST_District();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_LIST_District.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_LIST_District.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_LIST_District.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_LIST_District", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_LIST_District",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_LIST_District",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_LIST_District.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_LIST_District.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_LIST_District> toDTO(IQueryable<tbl_LIST_District> query)
        {
			return query
			.Select(s => new DTO_LIST_District(){							
		...
		public static DTO_LIST_District toDTO(tbl_LIST_District dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_LIST_District()
				{							
		...
		public static IQueryable<tbl_LIST_District> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_LIST_District> query = db.tbl_LIST_District.AsNoTracking();//.Where(d => d.IsDeleted == false );
        ...
        public static IQueryable<tbl_LIST_District> _sortBuilder(IQueryable<tbl_LIST_District> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_LIST_District>;
        ...
        public static IQueryable<tbl_LIST_District> _pagingBuilder(IQueryable<tbl_LIST_District> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
