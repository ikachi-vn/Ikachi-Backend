...
namespace ClassLibrary
{
    ...
    public partial class tbl_CRM_RouteDetail
    {
        ...
        //List 0:1
        public virtual tbl_CRM_Route tbl_CRM_Route { get; set; }
    }
}
...
{
	using System;
	public partial class DTO_CRM_RouteDetail
	{
		public int IDRoute { get; set; }
		public int IDContact { get; set; }
		public int Id { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_CRM_RouteDetail 
    {
		...
		public static IQueryable<DTO_CRM_RouteDetail> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS CRM_RouteDetail";
            ...
            if (workBook != null)
            {
                ...
                //var CRM_RouteList = BS_CRM_Route.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_CRM_RouteDetail getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_CRM_RouteDetail.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_CRM_RouteDetail getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_CRM_RouteDetail
        ...
        {
            bool result = false;
            var dbitem = db.tbl_CRM_RouteDetail.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_CRM_RouteDetail",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_CRM_RouteDetail dbitem, string Username)
        {
            Type type = typeof(tbl_CRM_RouteDetail);
        ...
        public static void patchDTOtoDBValue( DTO_CRM_RouteDetail item, tbl_CRM_RouteDetail dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDRoute = item.IDRoute;							
			dbitem.IDContact = item.IDContact;							
			dbitem.Code = item.Code;							
			dbitem.Name = item.Name;							
			dbitem.Remark = item.Remark;							
			dbitem.Week1 = item.Week1;							
			dbitem.Week2 = item.Week2;							
			dbitem.Week3 = item.Week3;							
			dbitem.Week4 = item.Week4;							
			dbitem.Monday = item.Monday;							
			dbitem.Tuesday = item.Tuesday;							
			dbitem.Wednesday = item.Wednesday;							
			dbitem.Thursday = item.Thursday;							
			dbitem.Friday = item.Friday;							
			dbitem.Saturday = item.Saturday;							
			dbitem.Sunday = item.Sunday;							
			dbitem.Frequency = item.Frequency;							
			dbitem.Sort = item.Sort;							
			dbitem.IsDisabled = item.IsDisabled;							
			dbitem.IsDeleted = item.IsDeleted;        }

		public static DTO_CRM_RouteDetail post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_CRM_RouteDetail dbitem = new tbl_CRM_RouteDetail();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_CRM_RouteDetail.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_CRM_RouteDetail",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_CRM_RouteDetail();
                    ...
                    {
                        dbitem = new tbl_CRM_RouteDetail();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_CRM_RouteDetail.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_CRM_RouteDetail.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_CRM_RouteDetail.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_CRM_RouteDetail", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_CRM_RouteDetail",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_CRM_RouteDetail",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_CRM_RouteDetail.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_CRM_RouteDetail.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_CRM_RouteDetail> toDTO(IQueryable<tbl_CRM_RouteDetail> query)
        {
			return query
			.Select(s => new DTO_CRM_RouteDetail(){							
				IDRoute = s.IDRoute,							
		...
		public static DTO_CRM_RouteDetail toDTO(tbl_CRM_RouteDetail dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_CRM_RouteDetail()
				{							
					IDRoute = dbResult.IDRoute,							
					IDContact = dbResult.IDContact,							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					Remark = dbResult.Remark,							
					Week1 = dbResult.Week1,							
					Week2 = dbResult.Week2,							
					Week3 = dbResult.Week3,							
					Week4 = dbResult.Week4,							
					Monday = dbResult.Monday,							
					Tuesday = dbResult.Tuesday,							
					Wednesday = dbResult.Wednesday,							
					Thursday = dbResult.Thursday,							
					Friday = dbResult.Friday,							
					Saturday = dbResult.Saturday,							
					Sunday = dbResult.Sunday,							
		...
		public static IQueryable<tbl_CRM_RouteDetail> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_CRM_RouteDetail> query = db.tbl_CRM_RouteDetail.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query IDRoute (int)
			if (QueryStrings.Any(d => d.Key == "IDRoute"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDRoute").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDRoute));
            }
			...
			//Query Week1 (bool)
			if (QueryStrings.Any(d => d.Key == "Week1"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "Week1").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.Week1);
            }

			//Query Week2 (bool)
			if (QueryStrings.Any(d => d.Key == "Week2"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "Week2").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.Week2);
            }

			//Query Week3 (bool)
			if (QueryStrings.Any(d => d.Key == "Week3"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "Week3").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.Week3);
            }

			//Query Week4 (bool)
			if (QueryStrings.Any(d => d.Key == "Week4"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "Week4").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.Week4);
            }

			//Query Monday (bool)
			if (QueryStrings.Any(d => d.Key == "Monday"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "Monday").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.Monday);
            }

			//Query Tuesday (bool)
			if (QueryStrings.Any(d => d.Key == "Tuesday"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "Tuesday").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.Tuesday);
            }

			//Query Wednesday (bool)
			if (QueryStrings.Any(d => d.Key == "Wednesday"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "Wednesday").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.Wednesday);
            }

			//Query Thursday (bool)
			if (QueryStrings.Any(d => d.Key == "Thursday"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "Thursday").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.Thursday);
            }

			//Query Friday (bool)
			if (QueryStrings.Any(d => d.Key == "Friday"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "Friday").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.Friday);
            }

			//Query Saturday (bool)
			if (QueryStrings.Any(d => d.Key == "Saturday"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "Saturday").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.Saturday);
            }

			//Query Sunday (bool)
			if (QueryStrings.Any(d => d.Key == "Sunday"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "Sunday").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.Sunday);
            }
            ...
            if (QueryStrings.Any(d => d.Key == "Sort"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "Sort").Value, out int val))
                    query = query.Where(d => val == d.Sort);
        ...
        public static IQueryable<tbl_CRM_RouteDetail> _sortBuilder(IQueryable<tbl_CRM_RouteDetail> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_CRM_RouteDetail>;
                        bool isOrdered = ordered.ToString().IndexOf("ORDER BY") !=-1;

                        switch (item)
                        {
							case "IDRoute":
								query = isOrdered ? ordered.ThenBy(o => o.IDRoute) : query.OrderBy(o => o.IDRoute);
								 break;
							case "IDRoute_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDRoute) : query.OrderByDescending(o => o.IDRoute);
                                break;
							case "IDContact":
							...
							case "Remark_desc":
                                ...
                                break;
							case "Week1":
								query = isOrdered ? ordered.ThenBy(o => o.Week1) : query.OrderBy(o => o.Week1);
								 break;
							case "Week1_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Week1) : query.OrderByDescending(o => o.Week1);
                                break;
							case "Week2":
								query = isOrdered ? ordered.ThenBy(o => o.Week2) : query.OrderBy(o => o.Week2);
								 break;
							case "Week2_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Week2) : query.OrderByDescending(o => o.Week2);
                                break;
							case "Week3":
								query = isOrdered ? ordered.ThenBy(o => o.Week3) : query.OrderBy(o => o.Week3);
								 break;
							case "Week3_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Week3) : query.OrderByDescending(o => o.Week3);
                                break;
							case "Week4":
								query = isOrdered ? ordered.ThenBy(o => o.Week4) : query.OrderBy(o => o.Week4);
								 break;
							case "Week4_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Week4) : query.OrderByDescending(o => o.Week4);
                                break;
							case "Monday":
								query = isOrdered ? ordered.ThenBy(o => o.Monday) : query.OrderBy(o => o.Monday);
								 break;
							case "Monday_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Monday) : query.OrderByDescending(o => o.Monday);
                                break;
							case "Tuesday":
								query = isOrdered ? ordered.ThenBy(o => o.Tuesday) : query.OrderBy(o => o.Tuesday);
								 break;
							case "Tuesday_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Tuesday) : query.OrderByDescending(o => o.Tuesday);
                                break;
							case "Wednesday":
								query = isOrdered ? ordered.ThenBy(o => o.Wednesday) : query.OrderBy(o => o.Wednesday);
								 break;
							case "Wednesday_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Wednesday) : query.OrderByDescending(o => o.Wednesday);
                                break;
							case "Thursday":
								query = isOrdered ? ordered.ThenBy(o => o.Thursday) : query.OrderBy(o => o.Thursday);
								 break;
							case "Thursday_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Thursday) : query.OrderByDescending(o => o.Thursday);
                                break;
							case "Friday":
								query = isOrdered ? ordered.ThenBy(o => o.Friday) : query.OrderBy(o => o.Friday);
								 break;
							case "Friday_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Friday) : query.OrderByDescending(o => o.Friday);
                                break;
							case "Saturday":
								query = isOrdered ? ordered.ThenBy(o => o.Saturday) : query.OrderBy(o => o.Saturday);
								 break;
							case "Saturday_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Saturday) : query.OrderByDescending(o => o.Saturday);
                                break;
							case "Sunday":
								query = isOrdered ? ordered.ThenBy(o => o.Sunday) : query.OrderBy(o => o.Sunday);
								 break;
							case "Sunday_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Sunday) : query.OrderByDescending(o => o.Sunday);
                                break;
							case "Frequency":
        ...
        public static IQueryable<tbl_CRM_RouteDetail> _pagingBuilder(IQueryable<tbl_CRM_RouteDetail> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
