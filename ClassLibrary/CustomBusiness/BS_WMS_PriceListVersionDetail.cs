    ...
    public static partial class BS_WMS_PriceListVersionDetail
    {
        public static string exportCustom(ExcelPackage package, AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            ...
            if (workBook != null)
            {
                ...
                var data = _queryBuilder(db, IDBranch, StaffID, QueryStrings).Select(s => new
                {
                    s.Id,
                    ItemCode = s.tbl_WMS_Item.Code,
                    ItemName = s.tbl_WMS_Item.Name,
                    UoM = s.tbl_WMS_ItemUoM.Name,
                    s.Price
                }).ToList();
                ...
                var range = ws.Cells[4, firstColInex == 0 ? 1 : firstColInex, rowid - 1, SheetColumnsCount];
