...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_Shift
    {
...
{
	using System;
	public partial class DTO_HRM_Shift
	{
		public int IDBranch { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_Shift 
    {
		...
		public static IQueryable<DTO_HRM_Shift> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_Shift";
		...
		public static DTO_HRM_Shift getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_Shift.Find(id);
		...
		public static DTO_HRM_Shift getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_Shift
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_Shift.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_Shift",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_Shift dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_Shift);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_Shift item, tbl_HRM_Shift dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_HRM_Shift post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_Shift dbitem = new tbl_HRM_Shift();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_Shift.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_Shift",e);
                    return null;
                }
            }
            return toDTO(dbitem);
        ...
        public static dynamic import(ExcelPackage package, AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings, string Username)
        {
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    {
                        dbitem = new tbl_HRM_Shift();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_Shift.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_Shift.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_Shift.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_HRM_Shift", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_Shift",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_Shift",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_Shift.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_Shift.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_Shift> toDTO(IQueryable<tbl_HRM_Shift> query)
        {
			return query
			.Select(s => new DTO_HRM_Shift(){							
				IDBranch = s.IDBranch,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
		...
		public static DTO_HRM_Shift toDTO(tbl_HRM_Shift dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_Shift()
				{							
		...
		public static IQueryable<tbl_HRM_Shift> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_Shift> query = db.tbl_HRM_Shift.AsNoTracking();//.Where(d => d.IsDeleted == false);
        ...
        public static IQueryable<tbl_HRM_Shift> _sortBuilder(IQueryable<tbl_HRM_Shift> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_Shift>;
        ...
        public static IQueryable<tbl_HRM_Shift> _pagingBuilder(IQueryable<tbl_HRM_Shift> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
