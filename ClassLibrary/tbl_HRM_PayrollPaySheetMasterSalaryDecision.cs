...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_PayrollPaySheetMasterSalaryDecision
    {
        ...
        public Nullable<int> IDPaySheetMaster { get; set; }
        public Nullable<int> IDSalaryDecision { get; set; }
        //List 0:1
        public virtual tbl_HRM_PayrollPaySheetMaster tbl_HRM_PayrollPaySheetMaster { get; set; }
        //List 0:1
        public virtual tbl_HRM_StaffSalaryDecision tbl_HRM_StaffSalaryDecision { get; set; }
    }
}
...
{
	using System;
	public partial class DTO_HRM_PayrollPaySheetMasterSalaryDecision
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_PayrollPaySheetMasterSalaryDecision 
    {
		...
		public static IQueryable<DTO_HRM_PayrollPaySheetMasterSalaryDecision> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_PayrollPaySheetMasterSalaryDecision";
            ...
            if (workBook != null)
            {
                ...
                //var HRM_PayrollPaySheetMasterList = BS_HRM_PayrollPaySheetMaster.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
                //var HRM_StaffSalaryDecisionList = BS_HRM_StaffSalaryDecision.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_HRM_PayrollPaySheetMasterSalaryDecision getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_PayrollPaySheetMasterSalaryDecision.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_PayrollPaySheetMasterSalaryDecision getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_PayrollPaySheetMasterSalaryDecision
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_PayrollPaySheetMasterSalaryDecision.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_PayrollPaySheetMasterSalaryDecision",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_PayrollPaySheetMasterSalaryDecision dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_PayrollPaySheetMasterSalaryDecision);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_PayrollPaySheetMasterSalaryDecision item, tbl_HRM_PayrollPaySheetMasterSalaryDecision dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDPaySheetMaster = item.IDPaySheetMaster;							
			dbitem.IDSalaryDecision = item.IDSalaryDecision;        }

		public static DTO_HRM_PayrollPaySheetMasterSalaryDecision post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_PayrollPaySheetMasterSalaryDecision dbitem = new tbl_HRM_PayrollPaySheetMasterSalaryDecision();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_PayrollPaySheetMasterSalaryDecision.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_PayrollPaySheetMasterSalaryDecision",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_PayrollPaySheetMasterSalaryDecision();
                    ...
                    {
                        dbitem = new tbl_HRM_PayrollPaySheetMasterSalaryDecision();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_PayrollPaySheetMasterSalaryDecision.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_PayrollPaySheetMasterSalaryDecision.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_PayrollPaySheetMasterSalaryDecision.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_PayrollPaySheetMasterSalaryDecision", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_PayrollPaySheetMasterSalaryDecision",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_PayrollPaySheetMasterSalaryDecision",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_PayrollPaySheetMasterSalaryDecision.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_PayrollPaySheetMasterSalaryDecision.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_PayrollPaySheetMasterSalaryDecision> toDTO(IQueryable<tbl_HRM_PayrollPaySheetMasterSalaryDecision> query)
        {
			return query
			.Select(s => new DTO_HRM_PayrollPaySheetMasterSalaryDecision(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				IDPaySheetMaster = s.IDPaySheetMaster,							
				IDSalaryDecision = s.IDSalaryDecision,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_HRM_PayrollPaySheetMasterSalaryDecision toDTO(tbl_HRM_PayrollPaySheetMasterSalaryDecision dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_PayrollPaySheetMasterSalaryDecision()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IDPaySheetMaster = dbResult.IDPaySheetMaster,							
		...
		public static IQueryable<tbl_HRM_PayrollPaySheetMasterSalaryDecision> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_PayrollPaySheetMasterSalaryDecision> query = db.tbl_HRM_PayrollPaySheetMasterSalaryDecision.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query IDPaySheetMaster (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDPaySheetMaster"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDPaySheetMaster").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDPaySheetMaster));
////                    query = query.Where(d => IDs.Contains(d.IDPaySheetMaster));
//                    
            }
...
//                    
        ...
        public static IQueryable<tbl_HRM_PayrollPaySheetMasterSalaryDecision> _sortBuilder(IQueryable<tbl_HRM_PayrollPaySheetMasterSalaryDecision> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_PayrollPaySheetMasterSalaryDecision>;
                        ...
                        switch (item)
                        {
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "IDPaySheetMaster":
								query = isOrdered ? ordered.ThenBy(o => o.IDPaySheetMaster) : query.OrderBy(o => o.IDPaySheetMaster);
								 break;
							case "IDPaySheetMaster_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDPaySheetMaster) : query.OrderByDescending(o => o.IDPaySheetMaster);
                                break;
        ...
        public static IQueryable<tbl_HRM_PayrollPaySheetMasterSalaryDecision> _pagingBuilder(IQueryable<tbl_HRM_PayrollPaySheetMasterSalaryDecision> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
