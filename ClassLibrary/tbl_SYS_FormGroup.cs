...
namespace BaseBusiness
{
    ...
    public static partial class BS_SYS_FormGroup 
    {
		...
		public static IQueryable<DTO_SYS_FormGroup> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS SYS_FormGroup";
            ...
            if (workBook != null)
            {
                ...
                //var SYS_AppsList = BS_SYS_Apps.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_SYS_FormGroup getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
		...
		public static DTO_SYS_FormGroup getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
        ...
        public static void patchDynamicToDB(dynamic item, tbl_SYS_FormGroup dbitem, string Username)
        {
            Type type = typeof(tbl_SYS_FormGroup);
        ...
        public static void patchDTOtoDBValue( DTO_SYS_FormGroup item, tbl_SYS_FormGroup dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_SYS_FormGroup post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_SYS_FormGroup();
                    ...
                    {
                        dbitem = new tbl_SYS_FormGroup();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_SYS_FormGroup.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_SYS_FormGroup.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_SYS_FormGroup.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_SYS_FormGroup", updateDate, Username);
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_SYS_FormGroup",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_SYS_FormGroup.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_SYS_FormGroup.Any(e => e.Code == Code && e.IsDeleted == false);
		...
		public static IQueryable<tbl_SYS_FormGroup> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_SYS_FormGroup> query = db.tbl_SYS_FormGroup.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			if (QueryStrings.Any(d => d.Key == "ClassName_eq") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "ClassName_eq").Value))
            {
                var keyword = QueryStrings.FirstOrDefault(d => d.Key == "ClassName_eq").Value;
            ...
            if (QueryStrings.Any(d => d.Key == "ClassName") && !string.IsNullOrEmpty(QueryStrings.FirstOrDefault(d => d.Key == "ClassName").Value))
            {
                ...
                query = query.Where(d=>d.ClassName.Contains(keyword));
            }
            
            return query;
        }
		
        public static IQueryable<tbl_SYS_FormGroup> _sortBuilder(IQueryable<tbl_SYS_FormGroup> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_SYS_FormGroup>;
                        bool isOrdered = ordered.ToString().IndexOf("ORDER BY") !=-1;

                        switch (item)
                        {
							case "IDApp":
								query = isOrdered ? ordered.ThenBy(o => o.IDApp) : query.OrderBy(o => o.IDApp);
								 break;
							case "IDApp_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDApp) : query.OrderByDescending(o => o.IDApp);
                                break;
							case "Id":
							...
							case "Icon_desc":
                                ...
                                break;
							case "ClassName":
								query = isOrdered ? ordered.ThenBy(o => o.ClassName) : query.OrderBy(o => o.ClassName);
								 break;
							case "ClassName_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.ClassName) : query.OrderByDescending(o => o.ClassName);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_SYS_FormGroup> _pagingBuilder(IQueryable<tbl_SYS_FormGroup> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
