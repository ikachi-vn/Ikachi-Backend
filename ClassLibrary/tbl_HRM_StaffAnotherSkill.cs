...
namespace ClassLibrary
{
    ...
    public partial class tbl_HRM_StaffAnotherSkill
    {
        ...
        public Nullable<int> IDAnotherSkill { get; set; }
        //List 0:1
...
{
	using System;
	public partial class DTO_HRM_StaffAnotherSkill
	{
		public int Id { get; set; }
		public string Code { get; set; }
		public string Name { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_HRM_StaffAnotherSkill 
    {
		...
		public static IQueryable<DTO_HRM_StaffAnotherSkill> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS HRM_StaffAnotherSkill";
		...
		public static DTO_HRM_StaffAnotherSkill getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_HRM_StaffAnotherSkill.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_HRM_StaffAnotherSkill getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_HRM_StaffAnotherSkill
        ...
        {
            bool result = false;
            var dbitem = db.tbl_HRM_StaffAnotherSkill.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_HRM_StaffAnotherSkill",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_HRM_StaffAnotherSkill dbitem, string Username)
        {
            Type type = typeof(tbl_HRM_StaffAnotherSkill);
        ...
        public static void patchDTOtoDBValue( DTO_HRM_StaffAnotherSkill item, tbl_HRM_StaffAnotherSkill dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDAnotherSkill = item.IDAnotherSkill;        }

		public static DTO_HRM_StaffAnotherSkill post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_HRM_StaffAnotherSkill dbitem = new tbl_HRM_StaffAnotherSkill();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_HRM_StaffAnotherSkill.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_HRM_StaffAnotherSkill",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_HRM_StaffAnotherSkill();
                    ...
                    {
                        dbitem = new tbl_HRM_StaffAnotherSkill();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_HRM_StaffAnotherSkill.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_HRM_StaffAnotherSkill.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_HRM_StaffAnotherSkill.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_HRM_StaffAnotherSkill", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_HRM_StaffAnotherSkill",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_HRM_StaffAnotherSkill",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_HRM_StaffAnotherSkill.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_HRM_StaffAnotherSkill.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_HRM_StaffAnotherSkill> toDTO(IQueryable<tbl_HRM_StaffAnotherSkill> query)
        {
			return query
			.Select(s => new DTO_HRM_StaffAnotherSkill(){							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
				Remark = s.Remark,							
				...
				IDStaff = s.IDStaff,							
				IDAnotherSkill = s.IDAnotherSkill,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_HRM_StaffAnotherSkill toDTO(tbl_HRM_StaffAnotherSkill dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_HRM_StaffAnotherSkill()
				{							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					IDAnotherSkill = dbResult.IDAnotherSkill,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_HRM_StaffAnotherSkill> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_HRM_StaffAnotherSkill> query = db.tbl_HRM_StaffAnotherSkill.AsNoTracking();//.Where(d => d.IsDeleted == false );
...
//                    
			...
			//Query IDAnotherSkill (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDAnotherSkill"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDAnotherSkill").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDAnotherSkill));
////                    query = query.Where(d => IDs.Contains(d.IDAnotherSkill));
//                    
            }
            return query;
        ...
        public static IQueryable<tbl_HRM_StaffAnotherSkill> _sortBuilder(IQueryable<tbl_HRM_StaffAnotherSkill> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_HRM_StaffAnotherSkill>;
                        ...
                        switch (item)
                        {
							...
							case "IDStaff_desc":
                                ...
                                break;
							case "IDAnotherSkill":
								query = isOrdered ? ordered.ThenBy(o => o.IDAnotherSkill) : query.OrderBy(o => o.IDAnotherSkill);
								 break;
							case "IDAnotherSkill_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDAnotherSkill) : query.OrderByDescending(o => o.IDAnotherSkill);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_HRM_StaffAnotherSkill> _pagingBuilder(IQueryable<tbl_HRM_StaffAnotherSkill> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
