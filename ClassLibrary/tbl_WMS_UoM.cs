...
namespace ClassLibrary
{
    ...
    public partial class tbl_WMS_UoM
    {
...
{
	using System;
	public partial class DTO_WMS_UoM
	{
		public int IDBranch { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_WMS_UoM 
    {
		...
		public static IQueryable<DTO_WMS_UoM> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS WMS_UoM";
		...
		public static DTO_WMS_UoM getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_WMS_UoM.Find(id);
		...
		public static DTO_WMS_UoM getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_WMS_UoM
        ...
        {
            bool result = false;
            var dbitem = db.tbl_WMS_UoM.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_WMS_UoM",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_WMS_UoM dbitem, string Username)
        {
            Type type = typeof(tbl_WMS_UoM);
        ...
        public static void patchDTOtoDBValue( DTO_WMS_UoM item, tbl_WMS_UoM dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_WMS_UoM post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_WMS_UoM dbitem = new tbl_WMS_UoM();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_WMS_UoM.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_WMS_UoM",e);
                    return null;
                }
            }
            return toDTO(dbitem);
        ...
        public static dynamic import(ExcelPackage package, AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings, string Username)
        {
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    {
                        dbitem = new tbl_WMS_UoM();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_WMS_UoM.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_WMS_UoM.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_WMS_UoM.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_WMS_UoM", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_WMS_UoM",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_WMS_UoM",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_WMS_UoM.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_WMS_UoM.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_WMS_UoM> toDTO(IQueryable<tbl_WMS_UoM> query)
        {
			return query
			.Select(s => new DTO_WMS_UoM(){							
				IDBranch = s.IDBranch,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
		...
		public static DTO_WMS_UoM toDTO(tbl_WMS_UoM dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_WMS_UoM()
				{							
		...
		public static IQueryable<tbl_WMS_UoM> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_WMS_UoM> query = db.tbl_WMS_UoM.AsNoTracking();//.Where(d => d.IsDeleted == false);
        ...
        public static IQueryable<tbl_WMS_UoM> _sortBuilder(IQueryable<tbl_WMS_UoM> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_WMS_UoM>;
        ...
        public static IQueryable<tbl_WMS_UoM> _pagingBuilder(IQueryable<tbl_WMS_UoM> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
