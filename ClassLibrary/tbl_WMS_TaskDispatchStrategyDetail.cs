...
namespace ClassLibrary
{
    ...
    public partial class tbl_WMS_TaskDispatchStrategyDetail
    {
        ...
        public bool ProximityInterleaving { get; set; }
        //List 0:1
        public virtual tbl_WMS_TaskDispatchStrategy tbl_WMS_TaskDispatchStrategy { get; set; }
    }
}
...
{
	using System;
	public partial class DTO_WMS_TaskDispatchStrategyDetail
	{
		public int IDStrategy { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_WMS_TaskDispatchStrategyDetail 
    {
		...
		public static IQueryable<DTO_WMS_TaskDispatchStrategyDetail> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS WMS_TaskDispatchStrategyDetail";
            ...
            if (workBook != null)
            {
                ...
                //var WMS_TaskDispatchStrategyList = BS_WMS_TaskDispatchStrategy.get(db,IDBranch, StaffID,QueryStrings).Select(s => new ItemModel { Id = s.Id, Name = s.Name });
		...
		public static DTO_WMS_TaskDispatchStrategyDetail getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_WMS_TaskDispatchStrategyDetail.Find(id);

			return toDTO(dbResult);
			
        }
		
		public static DTO_WMS_TaskDispatchStrategyDetail getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_WMS_TaskDispatchStrategyDetail
        ...
        {
            bool result = false;
            var dbitem = db.tbl_WMS_TaskDispatchStrategyDetail.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_WMS_TaskDispatchStrategyDetail",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_WMS_TaskDispatchStrategyDetail dbitem, string Username)
        {
            Type type = typeof(tbl_WMS_TaskDispatchStrategyDetail);
        ...
        public static void patchDTOtoDBValue( DTO_WMS_TaskDispatchStrategyDetail item, tbl_WMS_TaskDispatchStrategyDetail dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDStrategy = item.IDStrategy;							
			...
			dbitem.TaskType = item.TaskType;							
			dbitem.ProximityInterleaving = item.ProximityInterleaving;        }

		public static DTO_WMS_TaskDispatchStrategyDetail post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_WMS_TaskDispatchStrategyDetail dbitem = new tbl_WMS_TaskDispatchStrategyDetail();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_WMS_TaskDispatchStrategyDetail.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_WMS_TaskDispatchStrategyDetail",e);
                    return null;
                }
            }
            return toDTO(dbitem);
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    for (int c = firstColIndex; c <= SheetColumnsCount; c++)
                    {
                        ...
                        var target = new tbl_WMS_TaskDispatchStrategyDetail();
                    ...
                    {
                        dbitem = new tbl_WMS_TaskDispatchStrategyDetail();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_WMS_TaskDispatchStrategyDetail.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_WMS_TaskDispatchStrategyDetail.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_WMS_TaskDispatchStrategyDetail.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                BS_Version.update_Version(db, null, "DTO_WMS_TaskDispatchStrategyDetail", updateDate, Username);
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_WMS_TaskDispatchStrategyDetail",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_WMS_TaskDispatchStrategyDetail",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_WMS_TaskDispatchStrategyDetail.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_WMS_TaskDispatchStrategyDetail.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_WMS_TaskDispatchStrategyDetail> toDTO(IQueryable<tbl_WMS_TaskDispatchStrategyDetail> query)
        {
			return query
			.Select(s => new DTO_WMS_TaskDispatchStrategyDetail(){							
				...
				TaskType = s.TaskType,							
				ProximityInterleaving = s.ProximityInterleaving,					
			});//.OrderBy(o => o.Sort == null).ThenBy(u => u.Sort);
                              
        }

		public static DTO_WMS_TaskDispatchStrategyDetail toDTO(tbl_WMS_TaskDispatchStrategyDetail dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_WMS_TaskDispatchStrategyDetail()
				{							
					IDStrategy = dbResult.IDStrategy,							
					Id = dbResult.Id,							
					Code = dbResult.Code,							
					Name = dbResult.Name,							
					...
					TaskType = dbResult.TaskType,							
					ProximityInterleaving = dbResult.ProximityInterleaving,
				};
			}
			else
				return null; 
		...
		public static IQueryable<tbl_WMS_TaskDispatchStrategyDetail> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_WMS_TaskDispatchStrategyDetail> query = db.tbl_WMS_TaskDispatchStrategyDetail.AsNoTracking();//.Where(d => d.IsDeleted == false );
			...
			//Query IDStrategy (int)
			if (QueryStrings.Any(d => d.Key == "IDStrategy"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDStrategy").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDStrategy));
            }
			...
			//Query TaskType (int)
			if (QueryStrings.Any(d => d.Key == "TaskTypeFrom") && QueryStrings.Any(d => d.Key == "TaskTypeTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "TaskTypeFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "TaskTypeTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.TaskType && d.TaskType <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "TaskType"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "TaskType").Value, out int val))
                    query = query.Where(d => val == d.TaskType);


			//Query ProximityInterleaving (bool)
			if (QueryStrings.Any(d => d.Key == "ProximityInterleaving"))
            {
                var qValue = QueryStrings.FirstOrDefault(d => d.Key == "ProximityInterleaving").Value;
                if (bool.TryParse(qValue, out bool qBoolValue))
                    query = query.Where(d => qBoolValue == d.ProximityInterleaving);
            }
            return query;
        }
		
        public static IQueryable<tbl_WMS_TaskDispatchStrategyDetail> _sortBuilder(IQueryable<tbl_WMS_TaskDispatchStrategyDetail> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_WMS_TaskDispatchStrategyDetail>;
                        bool isOrdered = ordered.ToString().IndexOf("ORDER BY") !=-1;

                        switch (item)
                        {
							case "IDStrategy":
								query = isOrdered ? ordered.ThenBy(o => o.IDStrategy) : query.OrderBy(o => o.IDStrategy);
								 break;
							case "IDStrategy_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDStrategy) : query.OrderByDescending(o => o.IDStrategy);
                                break;
							case "Id":
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "TaskType":
								query = isOrdered ? ordered.ThenBy(o => o.TaskType) : query.OrderBy(o => o.TaskType);
								 break;
							case "TaskType_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.TaskType) : query.OrderByDescending(o => o.TaskType);
                                break;
							case "ProximityInterleaving":
								query = isOrdered ? ordered.ThenBy(o => o.ProximityInterleaving) : query.OrderBy(o => o.ProximityInterleaving);
								 break;
							case "ProximityInterleaving_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.ProximityInterleaving) : query.OrderByDescending(o => o.ProximityInterleaving);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_WMS_TaskDispatchStrategyDetail> _pagingBuilder(IQueryable<tbl_WMS_TaskDispatchStrategyDetail> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
