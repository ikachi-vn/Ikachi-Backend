...
{
	using System;
	public partial class DTO_Version
	{
		public Nullable<int> IDBranch { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_Version 
    {
		...
		public static IQueryable<DTO_Version> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS Version";
		...
		public static DTO_Version getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_Version.Find(id);
		...
		public static DTO_Version getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_Version
        ...
        {
            bool result = false;
            var dbitem = db.tbl_Version.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_Version",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_Version dbitem, string Username)
        {
            Type type = typeof(tbl_Version);
        ...
        public static void patchDTOtoDBValue( DTO_Version item, tbl_Version dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_Version post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_Version dbitem = new tbl_Version();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                {
					errorLog.logMessage("post_Version",e);
                    return null;
                }
            }
            return toDTO(dbitem);
        ...
        public static dynamic import(ExcelPackage package, AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings, string Username)
        {
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    {
                        dbitem = db.tbl_Version.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_Version.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_Version.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_Version", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_Version",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_Version",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_Version.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_Version.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_Version> toDTO(IQueryable<tbl_Version> query)
        {
			return query
			.Select(s => new DTO_Version(){							
				IDBranch = s.IDBranch,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
		...
		public static DTO_Version toDTO(tbl_Version dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_Version()
				{							
		...
		public static IQueryable<tbl_Version> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_Version> query = db.tbl_Version.AsNoTracking();//.Where(d => d.IsDeleted == false);
...
//                    
            ...
            if (QueryStrings.Any(d => d.Key == "VersionDate"))
                if (DateTime.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "VersionDate").Value, out DateTime val))
                    query = query.Where(d => d.VersionDate != null && DbFunctions.TruncateTime(val) == DbFunctions.TruncateTime(d.VersionDate));

            return query;
        }
		
        public static IQueryable<tbl_Version> _sortBuilder(IQueryable<tbl_Version> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_Version>;
                        ...
                        switch (item)
                        {
							...
							case "ModifiedDate_desc":
                                ...
                                break;
							case "VersionDate":
								query = isOrdered ? ordered.ThenBy(o => o.VersionDate) : query.OrderBy(o => o.VersionDate);
								 break;
							case "VersionDate_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.VersionDate) : query.OrderByDescending(o => o.VersionDate);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_Version> _pagingBuilder(IQueryable<tbl_Version> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
