...
namespace ClassLibrary
{
    ...
    public partial class tbl_CRM_Config
    {
...
{
	using System;
	public partial class DTO_CRM_Config
	{
		public Nullable<int> IDBranch { get; set; }
		public Nullable<int> IDParent { get; set; }
		public int Id { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_CRM_Config 
    {
		...
		public static IQueryable<DTO_CRM_Config> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            ...
            if (QueryStrings.Any(d => d.Key == "AllChildren")  || QueryStrings.Any(d => d.Key == "AllParent"))
            {
                ...
                List<DTO_CRM_Config> allItems = db.tbl_CRM_Config.Where(d => d.IsDeleted == false).Select(s => new DTO_CRM_Config()
                {
                    Id = s.Id,
                    IDParent = s.IDParent
                }).ToList();
                ...
                foreach (var item in query)
                {
                    ...
                    query = db.tbl_CRM_Config.Where(d => Ids.Contains(d.Id));
                }
        ...
        public static List<int> findParent(List<DTO_CRM_Config> allItems, int Id)
        {
        ...
        public static List<int> findChildrent(List<DTO_CRM_Config> allItems, int Id)
        {
            var Ids = new List<int>();
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS CRM_Config";
		...
		public static DTO_CRM_Config getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_CRM_Config.Find(id);
		...
		public static DTO_CRM_Config getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_CRM_Config
        ...
        {
            bool result = false;
            var dbitem = db.tbl_CRM_Config.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_CRM_Config",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_CRM_Config dbitem, string Username)
        {
            Type type = typeof(tbl_CRM_Config);
        ...
        public static void patchDTOtoDBValue( DTO_CRM_Config item, tbl_CRM_Config dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
		...
		public static DTO_CRM_Config post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_CRM_Config dbitem = new tbl_CRM_Config();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_CRM_Config.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_CRM_Config",e);
                    return null;
                }
            }
            return toDTO(dbitem);
        ...
        public static dynamic import(ExcelPackage package, AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings, string Username)
        {
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    {
                        dbitem = new tbl_CRM_Config();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_CRM_Config.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_CRM_Config.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_CRM_Config.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_CRM_Config", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_CRM_Config",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_CRM_Config",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_CRM_Config.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_CRM_Config.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_CRM_Config> toDTO(IQueryable<tbl_CRM_Config> query)
        {
			return query
			.Select(s => new DTO_CRM_Config(){							
				IDBranch = s.IDBranch,							
				IDParent = s.IDParent,							
				Id = s.Id,							
				Code = s.Code,							
		...
		public static DTO_CRM_Config toDTO(tbl_CRM_Config dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_CRM_Config()
				{							
		...
		public static IQueryable<tbl_CRM_Config> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_CRM_Config> query = db.tbl_CRM_Config.AsNoTracking();//.Where(d => d.IsDeleted == false);
...
//                    
        ...
        public static IQueryable<tbl_CRM_Config> _sortBuilder(IQueryable<tbl_CRM_Config> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_CRM_Config>;
        ...
        public static IQueryable<tbl_CRM_Config> _pagingBuilder(IQueryable<tbl_CRM_Config> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
