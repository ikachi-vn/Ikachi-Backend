...
namespace ClassLibrary
{
    ...
    public partial class tbl_PM_Task
    {
...
{
	using System;
	public partial class DTO_PM_Task
	{
		public int IDBranch { get; set; }
		public int Id { get; set; }
		public string Code { get; set; }
...
namespace BaseBusiness
{
    ...
    public static partial class BS_PM_Task 
    {
		...
		public static IQueryable<DTO_PM_Task> get(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
        ...
        {
            package.Workbook.Properties.Title = "ART-DMS PM_Task";
		...
		public static DTO_PM_Task getAnItem(AppEntities db, int IDBranch, int StaffID, int id)
        {
            var dbResult = db.tbl_PM_Task.Find(id);
		...
		public static DTO_PM_Task getAnItemByCode(AppEntities db, int IDBranch, int StaffID, string code)
        {
            var dbResult = db.tbl_PM_Task
        ...
        {
            bool result = false;
            var dbitem = db.tbl_PM_Task.Find(Id);
            ...
            if (dbitem != null)
            {
                ...
                {
					errorLog.logMessage("put_PM_Task",e);
                    result = false;
                }
            }
            else
        ...
        public static void patchDynamicToDB(dynamic item, tbl_PM_Task dbitem, string Username)
        {
            Type type = typeof(tbl_PM_Task);
        ...
        public static void patchDTOtoDBValue( DTO_PM_Task item, tbl_PM_Task dbitem)
        {
            if (item == null){
                dbitem = null;
                return;
			...
			dbitem.IDProject = item.IDProject;							
			dbitem.IDTaskStatus = item.IDTaskStatus;							
			dbitem.ExpectedRevenue = item.ExpectedRevenue;							
			dbitem.BudgetedCost = item.BudgetedCost;							
			dbitem.ActualCost = item.ActualCost;							
			dbitem.ActualRevenue = item.ActualRevenue;        }

		public static DTO_PM_Task post(AppEntities db, int IDBranch, int StaffID, dynamic item, string Username)
        {
            tbl_PM_Task dbitem = new tbl_PM_Task();
            if (item != null)
            {
                patchDynamicToDB(item, dbitem, Username);
                ...
                try
                {
					db.tbl_PM_Task.Add(dbitem);
                    db.SaveChanges();
				
                }
                catch (DbEntityValidationException e)
                {
					errorLog.logMessage("post_PM_Task",e);
                    return null;
                }
            }
            return toDTO(dbitem);
        ...
        public static dynamic import(ExcelPackage package, AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings, string Username)
        {
            ...
            if (workBook != null)
            {
                ...
                for (int r = firstRow; r <= SheetRowCount; r++){
                    ...
                    {
                        dbitem = new tbl_PM_Task();
                        dbitem.CreatedBy = Username;
				        dbitem.CreatedDate = DateTime.Now;
                                            }
                    else
                    {
                        dbitem = db.tbl_PM_Task.Find((int)it.Id);
                    }
                    ...
                    try
                    {
                        ...
                        if (dbitem.Id == 0) db.tbl_PM_Task.Add(dbitem);
                    }
                    catch (Exception ex)
                    {
                        errCount++;
		...
		public static bool delete(AppEntities db, string ids, string Username)
        {
            ...
            var dbitems = db.tbl_PM_Task.Where(d => IDs.Contains(d.Id));
            ...
            try
            {
                ...
                {
                    BS_Version.update_Version(db, IDBranch, "DTO_PM_Task", updateDate, Username);
                }
			
            }
            catch (DbEntityValidationException e)
            {
				errorLog.logMessage("delete_PM_Task",e);
                result = false;
            }
        ...
        public static bool disable(AppEntities db, string ids, bool isDisable, string Username)
        {
            ...
            {
				errorLog.logMessage("disable_PM_Task",e);
                result = false;
            }
		...
		public static bool check_Exists(AppEntities db, int id)
		{
            ...
            return db.tbl_PM_Task.Any(e => e.Id == id && e.IsDeleted == false);
		...
		public static bool check_Exists(AppEntities db, string Code)
		{
            ...
            return db.tbl_PM_Task.Any(e => e.Code == Code && e.IsDeleted == false);
            
		}
		
		//---//
		public static IQueryable<DTO_PM_Task> toDTO(IQueryable<tbl_PM_Task> query)
        {
			return query
			.Select(s => new DTO_PM_Task(){							
				IDBranch = s.IDBranch,							
				Id = s.Id,							
				Code = s.Code,							
				Name = s.Name,							
		...
		public static DTO_PM_Task toDTO(tbl_PM_Task dbResult)
        {
			if (dbResult != null)
			{
				return new DTO_PM_Task()
				{							
		...
		public static IQueryable<tbl_PM_Task> _queryBuilder(AppEntities db, int IDBranch, int StaffID, Dictionary<string, string> QueryStrings)
        {
            
			IQueryable<tbl_PM_Task> query = db.tbl_PM_Task.AsNoTracking();//.Where(d => d.IsDeleted == false);
...
//                    
			...
			//Query Duration (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "DurationFrom") && QueryStrings.Any(d => d.Key == "DurationTo"))
                if (int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "DurationFrom").Value, out int fromVal) && int.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "DurationTo").Value, out int toVal))
                    query = query.Where(d => fromVal <= d.Duration && d.Duration <= toVal);
...
//                    
			...
			//Query IDProject (Nullable<int>)
			if (QueryStrings.Any(d => d.Key == "IDProject"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDProject").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int?> IDs = new List<int?>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
						...
						IDs.Add(null);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDProject));
////                    query = query.Where(d => IDs.Contains(d.IDProject));
//                    
            }

			//Query IDTaskStatus (int)
			if (QueryStrings.Any(d => d.Key == "IDTaskStatus"))
            {
                var IDList = QueryStrings.FirstOrDefault(d => d.Key == "IDTaskStatus").Value.Replace("[", "").Replace("]", "").Split(',');
                List<int> IDs = new List<int>();
                foreach (var item in IDList)
                    if (int.TryParse(item, out int i))
                        IDs.Add(i);
                if (IDs.Count > 0)
                    query = query.Where(d => IDs.Contains(d.IDTaskStatus));
            }
			...
			//Query ActualRevenue (decimal)
			if (QueryStrings.Any(d => d.Key == "ActualRevenueFrom") && QueryStrings.Any(d => d.Key == "ActualRevenueTo"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ActualRevenueFrom").Value, out decimal fromVal) && decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ActualRevenueTo").Value, out decimal toVal))
                    query = query.Where(d => fromVal <= d.ActualRevenue && d.ActualRevenue <= toVal);
            
            if (QueryStrings.Any(d => d.Key == "ActualRevenue"))
                if (decimal.TryParse(QueryStrings.FirstOrDefault(d => d.Key == "ActualRevenue").Value, out decimal val))
                    query = query.Where(d => val == d.ActualRevenue);

            return query;
        }
		
        public static IQueryable<tbl_PM_Task> _sortBuilder(IQueryable<tbl_PM_Task> query, Dictionary<string, string> QueryStrings)
        {
            if (QueryStrings.Any(d => d.Key == "SortBy"))
            {
                ...
                foreach (var item in sorts)
                    ...
                    {
                        var ordered = query as IOrderedQueryable<tbl_PM_Task>;
                        ...
                        switch (item)
                        {
							...
							case "PredictedClosingDate_desc":
                                ...
                                break;
							case "Duration":
								query = isOrdered ? ordered.ThenBy(o => o.Duration) : query.OrderBy(o => o.Duration);
								 break;
							case "Duration_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.Duration) : query.OrderByDescending(o => o.Duration);
                                break;
							case "IDOpportunity":
							...
							case "IDLead_desc":
                                ...
                                break;
							case "IDProject":
								query = isOrdered ? ordered.ThenBy(o => o.IDProject) : query.OrderBy(o => o.IDProject);
								 break;
							case "IDProject_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDProject) : query.OrderByDescending(o => o.IDProject);
                                break;
							case "IDTaskStatus":
								query = isOrdered ? ordered.ThenBy(o => o.IDTaskStatus) : query.OrderBy(o => o.IDTaskStatus);
								 break;
							case "IDTaskStatus_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.IDTaskStatus) : query.OrderByDescending(o => o.IDTaskStatus);
                                break;
							...
							case "ActualCost_desc":
                                ...
                                break;
							case "ActualRevenue":
								query = isOrdered ? ordered.ThenBy(o => o.ActualRevenue) : query.OrderBy(o => o.ActualRevenue);
								 break;
							case "ActualRevenue_desc":
                                query = isOrdered ? ordered.ThenByDescending(o => o.ActualRevenue) : query.OrderByDescending(o => o.ActualRevenue);
                                break;
                            default:
                                if(!isOrdered)
        ...
        public static IQueryable<tbl_PM_Task> _pagingBuilder(IQueryable<tbl_PM_Task> query, Dictionary<string, string> QueryStrings)
        {
            int skip = 0;
            int take = 200;
